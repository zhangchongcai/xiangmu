...
export default function $axios (options) {
  return new Promise((resolve, reject) => {
    let baseURL = store.state.default.apiConfig.baseURL;
    ...
    instance.interceptors.request.use(
      config => {
        if (store.state.CpmUserKey) {
          config.headers['Cpm-User-Key'] = store.state.CpmUserKey;
        }
        if (store.state.loginToken) {
          ...
          config.headers['sign'] = md5(
            store.state.loginToken + store.state.signKey + timestamp
          );
        }
    ...
    instance.interceptors.response.use(
      response => {
        ...
        if (data.code == -301) {
          MessageBox.alert('您没有当前页面权限', '提示', {
            confirmButtonText: '返回',
            callback: action => {
              let obj =
                store.state.tagNav.openedPageList[
                  store.state.tagNav.openedPageList.length - 1
                ];
              store.commit('removeTabCache', obj);
            }
          });
        }
        if (data) {
