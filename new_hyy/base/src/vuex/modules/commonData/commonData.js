...
let state = {
  ...
  perms:
    JSON.parse(
      sessionStorage.getItem('perms') ? sessionStorage.getItem('perms') : '[]'
    ) || [],
  cache: {},
  level: {
    level1: sessionStorage.getItem('level1')
      ? sessionStorage.getItem('level3')
      : '',
    level2: sessionStorage.getItem('level2')
      ? sessionStorage.getItem('level3')
      : '',
    level3: sessionStorage.getItem('level3')
      ? sessionStorage.getItem('level3')
      : ''
  }
};
  ...
  getLevel (state, data) {
    if (localStorage.getItem('leftTreeList')) {
      ...
      if (data) {
        ...
        let str = level3Arr.find(item => item.menuCode == data.menuCode)
          .parentId;
