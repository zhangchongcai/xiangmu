const state = {
  ...
  cachePage: false,
  // 已经打开的页面
  openedPageList: sessionStorage.getItem('tagNavList')
    ? JSON.parse(sessionStorage.getItem('tagNavList'))
    : [],
  // 缓存的页面
  cachedPageName: [],
  closedItem: {},
  recentVisits: sessionStorage.getItem('recentVisits')
    ? JSON.parse(sessionStorage.getItem('recentVisits'))
    : []
};
const mutations = {
    ...
    if (data) {
      if (state.recentVisits.some(page => page.menuCode == data.menuCode)) {
        ...
        sessionStorage.setItem(
          'recentVisits',
          JSON.stringify(state.recentVisits)
        );
      }
    }
  ...
  removeRecentVisits (state, data) {
    let index = state.recentVisits.findIndex(
      page => page.menuCode == data.menuCode
    );
    if (index > -1) {
  ...
  addTagNav (state, data) {
    ...
    if (state.cachedPageName.includes(data.name)) {
      console.error(
        `${data.name} 组件出现命名重复，请检查组件中的name字段。当前组件所在的路由地址为：${data.path}`
      );
      return;
    }
    ...
    if (data) {
      ...
      for (let [i, v] of state.openedPageList.entries()) {
        if (v.path === data.path) {
          ...
          sessionStorage.setItem(
            'tagNavList',
            JSON.stringify(state.openedPageList)
          );
        }
      }
      if (state.cachePage) {
        let index = state.cachedPageName.findIndex(
