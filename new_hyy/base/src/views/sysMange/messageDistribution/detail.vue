<template>
  <div class="content-wrapper distribution-page">
    <el-form ref="listQuery" :model="listQuery" label-width="124px" label-position="right">
      <div class="form-group">
        ...
        <el-form-item label="发送结果：" class="width100" prop="fullName" style="width:100%;">
          <el-select v-model="listQuery.status" style="width:180px;" filterable clearable placeholder="发送状态：">
      ...
      <div class="customer-list" style="margin-bottom:36px;">
        ...
        <!-- footer 分页条 -->
        <div class="page-wrap">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="listQuery.pageNum" :page-sizes="[10, 25, 50, 100]" :page-size="listQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
          </el-pagination>
        </div>
      </div>
...
<script>
import { querySmsRecordBatch, reSendSameContentBatchMsg, querySmsRecord } from 'baseSrc/http/interface.js';
export default {
  ...
  methods: {
    ...
    async getInfo () {
      ...
      if (res.result) {
        ...
        let postParams = { batchNumber: this.$route.query.batchNumber, mark: 0, ...this.listQuery };
        //  let postParams = {status: this.listQuery.status,batchNumber:this.$route.query.batchNumber,mark:0}
    ...
    getList () {
      this.tipMessage = '数据加载中...';
      let params = { batchNumber: this.$route.query.batchNumber, mark: 0, ...this.listQuery };
