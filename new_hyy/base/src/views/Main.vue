  ...
  <el-container>
    <el-aside :width="this.isCollapse ? '60px' : '136px'">
      <h1 class="project-title">
        <i class="iconfont icon-huiyingyunlogo"></i>
      </h1>
      <div class="gongzuo">
        <i class="iconfont icon-caidanlan-gongzuotai" style="font-size: 14px;color:#fff;"></i>
        <span class="iconfont" slot="title">工作台</span>
        <!--@click="getPlatform"-->
        <div class="border"></div>
      </div>
      <div class="xian"></div>
      <el-menu style="width: 105%" @select="handleSelect" background-color="#21376B" text-color="#fff" active-text-color="#fff" :defaultOpeneds="defaultOpenedsArr">
        <div class="second">
          <el-submenu index="2">
            <template slot="title">
              <span style="margin-left: 10px">全部功能</span>
            </template>
            <el-menu :collapse="true" style="width:105%;background-color:#21376b">
              <el-submenu :index="'2-'+i" v-for="(item, i) in list" :key="i">
                <template slot="title">
                  <p style="width:112%; text-overflow: ellipsis;white-space: nowrap; overflow: hidden;margin-left: 10px;">
                    <i :class="'iconfont '+item.icon" v-if="item.icon!=='icon-caidanlan-piaoquan'&&item.icon!=='icon-caidanlan-baobiao'&&item.icon!=='icon-caidanlan-caiwuchazhang'" style="letter-spacing: 2px;color:rgba(255,255,255,0.65);font-size: 14px;"></i>
                    <i :class="'iconfont '+item.icon" v-else style="display: inline-block;width: 16px;overflow:hidden;letter-spacing: 2px;color:rgba(255,255,255,0.65);font-size: 12px;"></i>
                    <i class="iconfont " style="margin-left:4px;font-size: 14px;letter-spacing: 2px;color:#fff;font-weight: 500">{{item.menuName}} </i>
                  </p>
                </template>
                <div style="max-height:500px;overflow-y:scroll;">
                  <el-menu-item-group :key="s" :index="'2-'+i+'-'+s" v-if="item.submenu.length>0" v-for="(sub, s) in item.submenu">
                    <span slot="title" v-if="sub.menuName" :key="s">
                      {{sub.menuName}}
                    </span>
                    <div style="display:flex;flex-wrap: wrap;border-bottom:1px solid #ccc;width:580px">
                      <el-menu-item :index="'2-'+i+'-'+s+'-'+b" v-if="sub.submenu.length!=0" v-for="(su, b) in sub.submenu" :key="b" style="width:190px;text-overflow: ellipsis;overflow: hidden">
                        <router-link @click.native="toBreadCrumb(su)" v-if="su.menuName" :to="su.resUrl?su.resUrl:''" :title="su.menuName">
                          {{ su.menuName }}
                        </router-link>
                      </el-menu-item>
                    </div>
          ...
          <el-submenu index="3">
            ...
            <el-menu-item style="height:40px;line-height:40px;min-width: 0; padding: 0 0 0 33px" :index="'3-'+item.seq" v-for="(item, id) in commonList" :key="id">
              <div class="divTitle" @click="selectRoutes(item)">
                <i class="yuan"></i>
                <span slot="title" style="font-weight: 500;"> {{item.resourceName}}</span>
              </div>
            </el-menu-item>
          </el-submenu>
        </div>
    ...
    <el-container>
      <el-header>
        ...
        <div class="header-user">
          <div class="tags-ctrl">
            <span class="el-icon-arrow-left" @click="scroll(-1)">
            </span>
            <span class="el-icon-arrow-right" @click="scroll(1)">
            </span>
          </div>
      ...
      <el-main>
        ...
        <el-dialog width="600px" title="修改密码" :visible.sync="dialogVisible" :before-close="closeDialog">
          <div class="film-top" style="padding:0 20px">
            <el-form ref="resetForm" :model="resetForm" :rules="rules" label-width="150px" size="mini" class="film-search">
              ...
              <el-form-item label="请输入新密码：" prop="newPassword" style="width:460px;">
                <el-input type="password" v-model.trim="resetForm.newPassword" @input="getLvl(resetForm.newPassword)" placeholder="请输入新密码"></el-input>
                <div class="progress-bar_wrap" v-show="passwordlevel>0">
                  <div class="progress-bar_item" :class="[ {'strengthLv1': passwordlevel == 1}, {'strengthLv2': passwordlevel == 2}, {'strengthLv3': passwordlevel == 3}]"></div>
                </div>
...
import { modifyUserPassword, menuList, getUrl, commonFunctions } from 'baseSrc/http/interface.js';
