<template>
  <div>
    ...
    <el-dialog title="选择影院" :close-on-click-modal="false" :visible.sync="selectedCinemaShow" append-to-body>
      <el-form :inline="true" :model="cinemaForm">
        <el-form-item style="margin-top: -6px;">
          影院名称
        </el-form-item>
        <el-form-item style="margin-top: -6px;">
          <el-input v-model="cinemaForm.cinemaName" placeholder="请输入内容" clearable></el-input>
        </el-form-item>
        <el-form-item style="margin-top: -6px;">
          <el-button type="primary" @click="btnSearchClickHandler">搜索</el-button>
        </el-form-item>
      </el-form>
      <el-tree :data="cinemaTreeArray" :props="cinemaListTreeProps" :default-expand-all="true" :default-checked-keys="selectedCinemaUidArray" node-key="uid" show-checkbox check-on-click-node @check-change="checkboxChangeHandler">
      </el-tree>
...
export default {
  ...
  methods: {
     ...
     **/
    queryCinemaTree () {
      this.$ctmList.getCinemaTree({ name: $.trim(this.cinemaForm.cinemaName) }).then(json => {
        if (json.code === 200) {
