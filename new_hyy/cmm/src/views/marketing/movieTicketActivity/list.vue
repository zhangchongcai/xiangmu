...
<script>
import commonTable from 'cmmSrc/components/Table/commonTable.vue';
import qs from "qs";
...
export default {
  ...
  data () {
    return {
      ...
      subComName: "movieTicketActivity",//缓存数据唯一标识
      bizNo: "",//审批单业务编码
  ...
  methods: {
    ...
    handleSearch () {
      ...
      let params = {
        ...
        ruleTemplateId: this.searchHeaderFormData.ruleTemplateId || "14,15,16",//模板id
    ...
    //新建活动 
    addAct () {
      this.$router.push({ path: 'movieTicketActivity/create' })
    },
    //修改活动 
    updateAct (scope) {
      this.$router.push({ path: 'movieTicketActivity/edit', query: { id: scope.row.id } })
    },
    //修订活动 
    reviseAct (scope) {
      this.$router.push({ path: 'movieTicketActivity/revise', query: { id: scope.row.id, activityState: scope.row.activityState } })
    },
    //查看活动 
    actDetail (scope) {
      this.$router.push({ path: 'movieTicketActivity/detail', query: { id: scope.row.id } })
    },
    //复制活动
    copyAct (scope) {
      this.$router.push({ path: 'movieTicketActivity/copy', query: { id: scope.row.id } })
    },
    //提交审批
    ...
    deleteHandle (id) {
      ...
      this.$cmmList.marketingDel(data).then(data => {
        if (data && data.code === 200) {
          this.$message({
            message: '操作成功',
            type: 'success',
