...
export default {
  ...
  data () {
    return {
      ...
      subComName: "salesActivity",//缓存数据唯一标识
      bizNo: "",//审批单业务编码
      ...
      searchHeaderFormData: {
        ...
        pageNo: 1,
        pageSize: 10,
        ruleTemplateId: "17,18,19,20",//模板id
  ...
  methods: {
    ...
    handleSearch () {
      ...
      let params = {
        ...
        ruleTemplateId: this.searchHeaderFormData.ruleTemplateId || "17,18,19,20",//模板id
    ...
    //新建活动
    addAct () {
      this.$router.push({ path: 'salesActivity/create' })
    },
    //修订活动
    reviseAct (scope) {
      this.$router.push({ path: 'salesActivity/revise', query: { id: scope.row.id, activityState: scope.row.activityState } })
    },
    //修改活动
    updateAct (scope) {
      this.$router.push({ path: 'salesActivity/edit', query: { id: scope.row.id } })
    },
    //查看活动
    actDetail (scope) {
      this.$router.push({ path: 'salesActivity/detail', query: { id: scope.row.id } })
    },
    //复制活动
    copyAct (scope) {
      this.$router.push({ path: 'salesActivity/copy', query: { id: scope.row.id } })
    },
    //提交审批
