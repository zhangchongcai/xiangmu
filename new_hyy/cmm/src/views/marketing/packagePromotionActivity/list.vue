...
export default {
  ...
  data () {
    return {
      ...
      subComName: "packagePromotionActivity",//缓存数据唯一标识
      bizNo: "",//审批单业务编码
      ...
      //活动类型-选项列表
      ruleTemplateOptions: [
        { label: '不限', value: '21,22' },
        { label: '组合商品立减', value: '21' },
        { label: '组合商品减至', value: '22' },
      ],
      //活动状态-选项列表
      searchStateOptions: [
        { label: '全部', value: '' },
  ...
  methods: {
    ...
    handleSearch () {
      ...
      let params = {
        ...
        ruleTemplateId: this.searchHeaderFormData.ruleTemplateId || "21,22",//模板id
    ...
    //新建活动 
    addAct () {
      this.$router.push({ path: 'packagePromotionActivity/create' })
    },
    //修订活动
    reviseAct (scope) {
      this.$router.push({ path: 'packagePromotionActivity/revise', query: { id: scope.row.id, activityState: scope.row.activityState } })
    },
    //修改活动
    updateAct (scope) {
      this.$router.push({ path: 'packagePromotionActivity/edit', query: { id: scope.row.id } })
    },
    //查看活动
    actDetail (scope) {
      this.$router.push({ path: 'packagePromotionActivity/detail', query: { id: scope.row.id } })
    },
    //复制活动 
    copyAct (scope) {
      this.$router.push({ path: 'packagePromotionActivity/copy', query: { id: scope.row.id } })
    },
    //提交审批 
