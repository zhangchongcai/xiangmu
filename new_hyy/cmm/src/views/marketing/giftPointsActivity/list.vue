...
export default {
  ...
  data () {
    return {
      ...
      subComName: "giftPointsActivity",//缓存数据唯一标识
      bizNo: "",//审批单业务编码
      ...
      searchHeaderFormData: {
        ...
        pageNo: 1,
        pageSize: 10,
        ruleTemplateId: "23,24,25,26",//模板id
      ...
      //活动类型-选项列表
      ruleTemplateOptions: [
        { label: '不限', value: '23,24,25,26' },
        { label: '开卡送积分', value: '23' },
        { label: '充值送积分', value: '24' },
        { label: '消费送积分', value: '25' },
        { label: '消费送N倍积分', value: '26' },
      ],
      //活动状态-选项列表
      searchStateOptions: [
        { label: '全部', value: '' },
  ...
  methods: {
    ...
    handleSearch () {
      ...
      let params = {
        ...
        ruleTemplateId: this.searchHeaderFormData.ruleTemplateId || "23,24,25,26",//模板id
    ...
    //新建活动
    addAct () {
      this.$router.push({ path: 'giftPointsActivity/create' })
    },
    //修订活动
    reviseAct (scope) {
      this.$router.push({ path: 'giftPointsActivity/revise', query: { id: scope.row.id, activityState: scope.row.activityState } })
    },
    //修改活动
    updateAct (scope) {
      this.$router.push({ path: 'giftPointsActivity/edit', query: { id: scope.row.id } })
    },
    //查看活动
    actDetail (scope) {
      this.$router.push({ path: 'giftPointsActivity/detail', query: { id: scope.row.id } })
    },
    //复制活动
    copyAct (scope) {
      this.$router.push({ path: 'giftPointsActivity/copy', query: { id: scope.row.id } })
    },
    //提交审批
