                ...
                <el-collapse v-model="dialogActiveNames">
                    <el-collapse-item v-for="item in dialogConfig" :key="item.title" :title="item.title" :name="item.dialogActiveName">
                        ...
                        <div v-if="item.dialogActiveName != 1" style="padding-left: 30px;">
                            <el-tabs tab-position="left" style="height:500px;" v-model="item.activekey">
                                <el-tab-pane v-for="tabItem in item.content" :key="tabItem.title" label :name="tabItem.key">
                                    <span slot="label">
...
export default {
    ...
    methods: {
        ...
        defaultTab(){
            for(let item of this.dialogConfig){
                if(item.dialogActiveName != "1"){
                    for(let conItem of item.content){
                        if(conItem.content.length >= 1){
                            item.activekey = conItem.key;
                            break;
                        }
                    }
                }
