        ...
        if (res && res.code === 200) {
          ...
          for (let item of this.config) {
            ...
            if (item.activeName == 1) {
              // 活动类型
              if (ruleGroup.templateId == 27) {
                item.content.find(item => item.key == 'templateId').value = '开卡送余额';
              } else if (ruleGroup.templateId == 28) {
                item.content.find(item => item.key == 'templateId').value = '充值送余额';
              } else if (ruleGroup.templateId == 29) {
                item.content.find(item => item.key == 'templateId').value = '消费送余额';
              }
              // 活动名称
            ...
            if (item.activeName == 2) {
              if (ruleGroup.templateId == 27) {// 开卡送余额
                item.content = [{
                  title: "注册影院",
                  key: 'cinemaCode',
                  opUniqueName: '',
              ...
              } else if (ruleGroup.templateId == 28) {// 充值送余额
                item.content = [{
                  title: "交易影院",
                  key: 'cinemaCode',
                  opUniqueName: '',
              ...
              } else if (ruleGroup.templateId == 29) { // 消费送余额
                item.content = [{
                  title: "交易影院",
                  key: 'cinemaCode',
                  opUniqueName: '包含',
            ...
            if (item.activeName == 3) {
              for (let action of actions) {
                // 优惠设置
                if (action.key == "presentMoney") {
                  item.content.find(item => item.key == 'presentMoney').value = `赠送余额${action.value}`;
                }
              }
            }
            //设置活动预算
