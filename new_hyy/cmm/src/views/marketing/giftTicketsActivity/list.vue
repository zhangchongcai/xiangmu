...
export default {
  ...
  data () {
    return {
      ...
      subComName: "giftTicketsActivity",//缓存数据唯一标识
      bizNo: "",//审批单业务编码
      ...
      //活动类型-选项列表
      ruleTemplateOptions: [
        { label: '不限', value: '30,31,32' },
        { label: '开卡送票券', value: '30' },
        { label: '充值送票券', value: '31' },
        { label: '消费送票券', value: '32' },
      ],
      //活动状态-选项列表
      searchStateOptions: [
        { label: '全部', value: '' },
  ...
  methods: {
    ...
    handleSearch () {
      ...
      let params = {
        ...
        ruleTemplateId: this.searchHeaderFormData.ruleTemplateId || "30,31,32",//模板id
    ...
    //新建活动
    addAct () {
      this.$router.push({ path: 'giftTicketsActivity/create' })
    },
    //修订活动
    reviseAct (scope) {
      this.$router.push({ path: 'giftTicketsActivity/revise', query: { id: scope.row.id, activityState: scope.row.activityState } })
    },
    //修改活动 
    updateAct (scope) {
      this.$router.push({ path: 'giftTicketsActivity/edit', query: { id: scope.row.id } })
    },
    //查看活动 
    actDetail (scope) {
      this.$router.push({ path: 'giftTicketsActivity/detail', query: { id: scope.row.id } })
    },
    //复制活动
    copyAct (scope) {
      this.$router.push({ path: 'giftTicketsActivity/copy', query: { id: scope.row.id } })
    },
    //提交审批
