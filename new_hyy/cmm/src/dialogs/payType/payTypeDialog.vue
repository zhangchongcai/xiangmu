      ...
      <div class="choose_table">
        <div>
          ...
          <el-table :data="tableList" :cell-style={padding:0} :row-style={height:30} :header-cell-style={padding:0} ref="multipleTable" @select-all="handleSelect" :row-key="getRowKeys" @select="handleSelect">
            ...
            <el-table-column property="payTypeCode" label="支付编码" width="294"></el-table-column>
            <el-table-column property="payTypeName" label="支付名称" width="294"></el-table-column>
          </el-table>
          <!-- 分页 -->
...
<script>
import { payType } from "cmmSrc/http/interface.js"
export default {
  props: {
    dialogTableVisible: {
      type: Boolean,
  ...
  methods: {
      ...
      if (selectedArr.length != 0) {
        for (let i = 0; i < selectedArr.length; i++) {
          for (let k = 0; k < this.tableList.length; k++) {
            if (selectedArr[i].payTypeCode == this.tableList[k].payTypeCode) {
    ...
    rowOneToggle (row) {
      for (let index = 0; index < this.chooseItem.length; index++) {
        if (row.payTypeCode == this.chooseItem[index].payTypeCode) {
    ...
    delateSpan (item) {
      ...
      if (item) {
        ...
        for (let i = this.chooseItem.length - 1; i >= 0; i--) {
          if (this.chooseItem[i].payTypeCode == item.payTypeCode) {
