// 支付方式
import cinemaDialog from 'cmmSrc/dialogs/cinema/cinemaDialog.vue'; // 影院弹窗
import userDialog from 'cmmSrc/dialogs/user/userDialog.vue'; // 用户弹窗
import orgStructureDialog from 'cmmSrc/dialogs/orgStructure/orgStructureSingleDialog.vue'; // 组织结构单个节点多选弹窗
...
let alertHandle = {
  ...
  methods: {
     ...
     */
    multiCinemaClick () {
      this.$refs.multiCinema.framedialogVisible = !this.$refs.multiCinema
        .framedialogVisible;
    },
    /**
    ...
    userCreaterClick (reviewData) {
      ...
      if (reviewData && reviewData.value && reviewData.text) {
        reviewData.value.split(',').map(item => {
          reviewList.push({ id: item, fullName: '', reviewFlag: 'review' });
        });
        reviewData.text.split(',').map((item, index) => {
          reviewList[index].fullName = item;
        });
      }
      console.log('reviewList', reviewList);
      // 开启弹窗
