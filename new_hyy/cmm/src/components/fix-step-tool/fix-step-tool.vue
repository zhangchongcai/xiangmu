<template>
  <div class="fix-step-tool" :style="{right: right}">
    <ul>
      <li v-for="(step, index) in stepList" :key="index" class="single-step" :class="{'isactive': step.isactive}" @click="stepPos(index)">
        <i class="fix-step-icon"></i>
        <i class="iconfont icon-neiye-yemianquyuxuanze"></i>
        <span>{{step.name}}</span>
        <span class="connect-line"></span>
        <span class="connect-line-first" v-if="!index"></span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  computed: {
    right () {
      return document.body.offsetWidth > 1440 ? document.body.offsetWidth > 1540 ? '80px' : (200 - document.body.offsetWidth + 1440) + 'px' : '80px'
    }
  },
  destroyed () {
