...
const WebpackTemplatePlugin = require('webpack-template-plugin');

const buildConfigBuilder = require('../../parent/src/build.config');
let buildConfig = buildConfigBuilder.getConfig('main');

const isProduction = process.env.NODE_ENV === 'production';
const envConfigBuilder = require('../../parent');
let envConfig = envConfigBuilder.getEnv('main');

let localEnv = require('../config/prod.env');
...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  plugins: [
    ...
    new WebpackTemplatePlugin({
      ...
      tpl: [
        {
          src: path.resolve(__dirname, '../src/assets/js/main.js'),
          dest: path.resolve(__dirname, '../src/main.js'),
          renderObject: null
        }
      ]
    }),
