...
{{#each(renderObject.plugins)~}}
  {{#if(this.folder !== 'base')~}}
import { {{this.folder}}RegComponents, {{this.folder}}Routes, {{this.folder}}Store, {{this.folder}}RouterView } from "{{this.folder}}";
Vue.use({{this.folder}}RegComponents);
