<template>
  <section class="cinemaView">
    ...
    <el-dialog class="normal-dialog" :title="isDetail?'支付方式详情':'新增支付方式'" :visible.sync="dialogVisible">
      <el-form :model="addForm" :rules="rules" ref="addForm" :label-width="formLabelWidth" style="width: 400px">
        ...
        <el-form-item v-if="isDetail" label="支付类型：">
          <span v-if="defFg">
            自定义支付方式-{{payMode === 1 ? '普通' : (payMode === 2 ? '微信' : '支付宝')}}
          </span>
    ...
    <div class="el-row-btns clearfix">
      <el-button class="normal-btn" style="float: right; padding: 8px 16px; width: auto" @click="pullPayTypeToCinema" size="small">同步总部数据</el-button>
    </div>
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="defFg" label="支付类型" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="scope.row.defFg">
            自定义支付方式-{{scope.row.payMode === 1 ? '普通' : (scope.row.payMode === 2 ? '微信' : (scope.row.payMode === 3 ? '支付宝' : ''))}}
          </span>
          <span v-else>默认支付方式</span>
        </template>
      </el-table-column>
      <el-table-column label="pos端是否显示" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="scope.row.showFlag">显示</span>
          <span v-else>不显示</span>
        </template>
      </el-table-column>
      <el-table-column prop="status" label="使用状态" width="120" show-overflow-tooltip>
        <template slot-scope="scope">
