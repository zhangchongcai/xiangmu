 <template>
  <div class="film-edit-wrap">
    ...
    <ShowBigDialog ref="bigDialog" :dialogTableVisible2.sync="dialogTableVisible2" :title="title" :showData="totalData" :gridData="gridData" :pageData="pageData" :parentDialog="parentDialog" :tipsword="tipsword" @changeDialogTableVisible2="changeDialogTableVisible2" @changeCurrentPage="changeCurrentPage" @searchInfo="searchInfo"></ShowBigDialog>
  </div>
</template>
 <script>
          ...
          if (data && data.code === 200) {
            ...
            if (countAmount < 0) {
              ...
              if (this.totalData.realAmount < 0) {
                ...
                beyondEl.style.border = "none";
                //  console.log(beyondEl.style.border)
              }
            }
      ...
      this.$csmList.actualCash({
        ...
        .then(data => {
          if (data && data.code === 200) {
            ...
            this.tipsword = data.data.totalAmount;
            //  this.$refs.showDialog.tipsword = this.tipsword;
    ...
    openAlipay (title, payTypeQuery, search, current = 1, size = this.size) {
      ...
      this.$csmList.payTotal(Object.assign({}, limit, search), current, size)
        .then(data => {
          ...
          this.$refs.showDialog.tipsword = data.data.totalAmount.toFixed(2);
      ...
      this.$csmList.goodsSell(Object.assign({}, limit, search), current, size)
        .then(data => {
          ...
          this.gridData.forEach(item => {
            ...
            // item.saleType = 2
            item.saleType = item.saleType == "1" ? "销售" : item.saleType == "2" ? "退货" : "";
            //  item.salePrice = -10;
            //  item.saleNum = -2;
            item.salePrice = Math.abs(Number(item.salePrice).toFixed(2))
    ...
    openVipDetail (search, current = this.current, size = this.size) {
      ...
      this.$csmList.vipSell(Object.assign({}, limit, search), current, size)
        .then(data => {
          ...
          this.$refs.bigDialog.totalAmount = data.data.totalCAmount // 传入的合计值
          this.$refs.bigDialog.totalNum = data.data.totalGAmount // 传入的合计值
          this.pageData = data.data.records;
          this.tips = data.data.payInfo;
          this.tips.map(item => {
            item.tip = `${item.payTypeName}:${item.totalAmount};`
