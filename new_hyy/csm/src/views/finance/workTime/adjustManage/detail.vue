 <template>
  <div class="film-edit-wrap">
    ...
    <!-- <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: 'detail' }">清机结算详情</el-breadcrumb-item>
    </el-breadcrumb> -->
        ...
        <el-collapse-item title="资金收入统计" name="3" v-show="isCanSee">
          ...
          <div class="flex_div">
            ...
            <div class="num_div">
              ...
              <p class="num_word"><span class="is_more">多</span><span class="num_span">{{countAmount.toFixed(2)}}</span></p>
              <p class="tip_word2">【收入差异=实收差异-应收金额】</p>
          ...
          <div class="table_div">
            <ul class="plan_money">
              <li class="special_li" @click="openMoney">
                <p> {{totalData.realXrmbAmount}}</p>
                <p>现金</p>
              </li>
              <li class="special_li" style="cursor:default">
                <!-- <p>{{totalData.saleBillPayFlowVo.xunpAmount}}</p> -->
                <p>{{totalData.bandCardTotal}}</p>
                <p>银行卡</p>
              </li>
              <li @click="openAlipay('支付宝收入','ALIPAY')">
                <p>{{totalData.saleBillPayFlowVo.alipayAmount}}</p>
                <p>支付宝</p>
              </li>
              <li @click="openAlipay('微信收入','WXPAY')">
                <p>{{totalData.saleBillPayFlowVo.wxpayAmount}}</p>
                <p>微信</p>
              </li>
              <li @click="openAlipay('会员收入','0X03')">
                <p>{{totalData.saleBillPayFlowVo.memPayAmount}}</p>
                <p>会员</p>
              </li>
              <li @click="openAlipay('票务收入','0X09')">
                <p>{{totalData.saleBillPayFlowVo.ticketPayAmount}}</p>
                <p>票券</p>
              </li>
              <li @click="openAlipay('其他收入','OTHERS')">
                <p>{{totalData.saleBillPayFlowVo.otherPayAmount}}</p>
                <p>其他</p>
              </li>
              <li @click="moneyAdjust">
                <p>{{totalData.fundChangeAmount}}</p>
                <p>资金调整单</p>
              </li>
              <div class="finally_div">
                <i class="iconfont icon-icon-youjiantou"></i>
                <p>
                  <span class="nums">{{totalData.realAmount}}</span>
            ...
            <ul class="actual_money">
              <li class="special_li" @click="openAlipay('现金收入','XRMB')">
                <p>{{totalData.saleBillPayFlowVo.xrmbAmount}}</p>
                <p>现金</p>
              </li>
              <li class="special_li" @click="openBankCard">
                <p>{{totalData.saleBillPayFlowVo.xunpAmount}}</p>
                <p>银行卡</p>
              </li>
              <li @click="openAlipay('支付宝收入','ALIPAY')">
              ...
              <div class="finally_div">
                <i class="iconfont icon-icon-youjiantou"></i>
                <p>
                  <span class="nums">{{totalData.shouldAmount}}</span>
            ...
            <ul class="big_ul">
              <li @click="openBoxOffice">
                <p>{{totalData.movieTicketAmount}}</p>
                <p>电影票销售</p>
              </li>
              <li @click="openGoods">
                <p>{{totalData.merAmount}}</p>
                <p>卖品销售</p>
              </li>
              <li @click="openVipDetail">
                <p>{{totalData.memberAmount}}</p>
                <p>会员卡充值</p>
              </li>
              <li @click="openTicket">
                <p>{{totalData.ticketAmount}}</p>
                <p>票券销售</p>
              </li>
              <li @click="openOther">
                <p>{{totalData.otherAmount}}</p>
                <p>其他销售</p>
              </li>
              <div class="finally_div">
                <i class="iconfont icon-icon-youjiantou"></i>
                <p>
                  <span class="nums">{{totalData.saleAmount}}</span>
          ...
          <p>备注</p>
          <el-input placeholder="请输入内容" v-model="remark" clearable style="width:918px;">
          </el-input>
        </el-collapse-item>
    ...
    <ShowDialog ref="showDialog" :dialogTableVisible.sync="dialogTableVisible" :title="title" :showData="totalData" :gridData="gridData" :pageData="pageData" :parentDialog="parentDialog" @changeDialogTableVisible="changeDialogTableVisible" @changeCurrentPage="changeCurrentPage" @searchInfo="searchInfo"></ShowDialog>
    <ShowBigDialog :dialogTableVisible2.sync="dialogTableVisible2" :title="title" :showData="totalData" :gridData="gridData" :pageData="pageData" :parentDialog="parentDialog" :tipsword="tipsword" @changeDialogTableVisible2="changeDialogTableVisible2" @changeCurrentPage="changeCurrentPage" @searchInfo="searchInfo"></ShowBigDialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
      ...
      this.$csmList.clearDetail(Object.assign({}, limit))
        .then(data => {
          if (data && data.code === 200) {
            ...
            let shouldAmount = Number(this.totalData.shouldAmount);
            // let shouldAmount = 820;
    ...
    openMoney () {
      ...
      this.$csmList.actualCash({ workTimeUid: this.$route.query.uid })
        .then(data => {
          if (data && data.code === 200) {
        ...
        .then(data => {
          ...
          if (this.tips.length > 0) {
            if (this.tips.length == 1) {
              this.tipsword = this.tips.reduce((prev, curr) => {
                return prev.tip + curr.tip
              }, { tip: "" }).slice(0, length - 1)
            } else {
              this.tipsword = this.tips.reduce((prev, curr) => {
                return prev.tip + curr.tip
              }).slice(0, length - 1)
    ...
    openGoods (search, current = 1, size = 10) {
      ...
      this.$csmList.goodsSell(Object.assign({}, limit, search), current, size)
        .then(data => {
          ...
          this.gridData.forEach(item => {
            item.saleType = item.saleType == "1" ? "销售" : item.saleType == "2" ? "退票" : "补登";
            item.numPrice = Number(item.salePrice * item.saleNum).toFixed(2); // 金额
            // item.position = `${item.seatColumnNo}排${item.seatRowNo}号` //位置
