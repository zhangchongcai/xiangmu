      ...
      <el-form :inline="true">
        ...
        <div v-else-if="parentDialog =='isAlipay'">
          <el-form-item label="交易流水号：">
            <el-input v-model="search.transactionCode" style="width:180px;"></el-input>
          </el-form-item>
          <el-form-item label="交易类型：">
            <el-select v-model="search.transactionType" style="width:160px;">
              <el-option label="全部" value>全部</el-option>
              <el-option v-for="(val,key) of transactionType" :key="key" :label="val" :value="key">{{val}}</el-option>
            </el-select>
          </el-form-item>
          <el-button type="primary" @click="searchInfo" style="margin-top:3px;">查询</el-button>
        </div>
      </el-form>
      <div class="border_div2" v-if="parentDialog=='isCash'">
        <!-- 现金收入 -->
        <el-table :data="gridData" border :cell-style={padding:0} :row-style={height:30} :header-cell-style={padding:0} highlight-current-row show-summary :summary-method="getSummaries">
          ...
          <el-table-column property="terminalCode" label="终端编号" min-width="96" show-overflow-tooltip></el-table-column>
        </el-table>
      </div>
      <div class="border_div" v-else>
        <!-- 领用明细 -->
        <el-table :data="gridData" border :cell-style={padding:0} :row-style={height:30} :header-cell-style={padding:0} highlight-current-row v-if="parentDialog=='isDetail'" :key=1>
          ...
          <el-table-column property="provideUserName" label="发送人" min-width="96" show-overflow-tooltip></el-table-column>
        </el-table>
        <!-- 支付宝 -->
        <el-table :data="gridData" border :cell-style={padding:0} :row-style={height:30} :header-cell-style={padding:0} highlight-current-row show-summary v-else-if="parentDialog=='isAlipay' && payTypeQuery!='0X03'" :summary-method="getSummaries" :key=2>
        ...
        <el-table :data="gridData" border :cell-style={padding:0} :row-style={height:30} :header-cell-style={padding:0} highlight-current-row show-summary v-else-if="parentDialog=='isAlipay' && payTypeQuery =='0X03'" :summary-method="getSummaries" :key=3>
          <el-table-column property="transactionCode" label="交易流水号" width="160" show-overflow-tooltip></el-table-column>
          <el-table-column property="transactionTime" label="交易时间" width="160" show-overflow-tooltip></el-table-column>
          <el-table-column property="payTime" label="支付时间" width="160" show-overflow-tooltip></el-table-column>
          <el-table-column property="payTypeName" label="支付方式" width="96" show-overflow-tooltip></el-table-column>
        ...
        <div class="block" v-if="parentDialog=='isDetail' ||parentDialog =='isAlipay'">
          <el-pagination @current-change="handleCurrentChange" background :current-page.sync="pageData.current" :page-size="pageData.size" layout="total,prev, pager, next, jumper" :total="pageData.total">
          </el-pagination>
        </div>
      </div>
