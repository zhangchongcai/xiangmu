  ...
  <div class="page-wrapper">
    ...
    <div class="list-wrapper detail">
      <div>
        ...
        <div>
          <el-tag size="small" v-for="tag in selectedCinemas.editData" :key="tag.id" closable :type="tag.type">
            {{tag.name}}
          </el-tag>
        </div>
        <span class="table-btn-mini" @click="chooseCinema(2,2)">编辑影院多选2</span>
        <div>
          <el-tag size="small" v-for="tag in selectedCinemas2.editData" :key="tag.id" closable :type="tag.type">
            {{tag.name}}
          </el-tag>
        </div>
      ...
      <div class="table">
        <el-table :data="userList" stripe style="width: 100%">
          <el-table-column prop="loginName" label="字段1">
          </el-table-column>
          <el-table-column prop="fullName" label="字段2">
          </el-table-column>
          <el-table-column prop="phone" label="字段3">
          </el-table-column>
        ...
        <!-- footer 分页条 -->
        <div class="page-wrap">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 25, 50, 100]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
          </el-pagination>
        </div>
      </div>
    </div>
    <el-dialog title="选择组织" :visible.sync="dialogVisible">
      <el-tree :data='orgArr' ref="tree" @node-click="handleNodeClick" node-key="code" default-expand-all :highlight-current="true" :props="defaultProps">
      </el-tree>
    ...
    <frame-multicinema :dialogVisible="multiCinemaVisible" :whereUse="whereUse" :type="cinematype" :innerData="innerData" :isListprop="true" @callBack="handleCallBack" ref='frameMultiCinema'>
      <div slot="footerId">
    ...
    <frame-singlecinema :framedialogVisible="singleCinemaVisible" :whereUse="whereUse" type="1" :innerData="innerDataSingle" @callBackSingle="handleSingleCallBack" ref='frameSingleCinema'>
      <div slot="footerId">
        <el-button @click="closeDialogCinemaSingle(false)">取 消</el-button>
        <el-button type="primary" @click="confirmCinemaSingleDialog()">确 定</el-button>
      </div>
    </frame-singlecinema>
  </div>
</template>
...
const { userList, resetPassword, orgList } = baseInterface;
export default {
  name: "userList",
  data () {
    return {
  ...
  methods: {
    ...
    chooseCinema (num, cinematype) {
      ...
      if (num == 1) {
        this.innerData.editData = this.selectedCinemas.editData
      } else if (num == 2) {
        this.innerData.editData = this.selectedCinemas2.editData
      }
      setTimeout(() => {
