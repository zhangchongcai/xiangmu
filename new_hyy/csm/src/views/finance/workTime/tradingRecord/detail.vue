  ...
  <div class="cinemaList">
      ...
      <el-form size="small">
        ...
        <el-col :span="12">
          <el-form-item label="交易流水号:">
            <span>{{dataTotal.transactionCode}}</span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
    ...
    <div class="table_div">
      ...
      <el-button plain class="title_btn" type="primary" @click="readGoodCode" style="width:100px;margin-bottom:8px;float:right" v-if="!canShow">
        查看取货码</el-button>
      <el-table :data="goodList" style="width: 100%">
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template slot-scope="{row}">
            <el-button type="text" size="small" @click.stop="readInfo(row)" v-if="row.ticketInfoCode">查看信息码
            </el-button>
            <span v-else> —— </span>
          </template>
        </el-table-column>
    ...
    <div class="table_div">
      ...
      <el-table :data="saleList" style="width: 100%">
        ...
        <el-table-column prop="saleBillCode" label="其它">
        </el-table-column>
      </el-table>
      ...
      this.$csmList.payDetail(Object.assign({}, limit))
        .then(data => {
          ...
          if (data && data.code === 200) {
            ...
            this.dataTotal.saleType = this.dataTotal.saleType == "1" ? "销售" : "退货"
            // 判断显示那些表格数据
            console.log(this.dataTotal)
            this.tableBillType = this.billType[this.dataTotal.billType]
            console.log("billTYpe" + this.tableBillType)
            this.canShow = this.tableBillType.indexOf('会员') > -1 // 不显示取货吗
            console.log(this.canShow, this.tableBillType.indexOf('会员'))
            ...
            this.goodList.map(item => {
              ...
              item.reportStatus = item.reportStatus == "NO" ? "未上报" : item.reportStatus == "FAIL" ? "上报失败" :
                item.reportStatus == "REPORTED" ?
                  "已上报" : item.rejectReportStatus == "REPORTING" ? "上报中" : "——"
              item.rejectReportStatus = item.rejectReportStatus == "NO" ? "未上报" : item.rejectReportStatus ==
                "FAIL" ? "上报失败" : item.rejectReportStatus == "REPORTED" ?
                  "已上报" : item.rejectReportStatus == "REPORTING" ? "上报中" : "——"
              item.saleTotal = item.saleNum * item.salePrice
              item.saleTotal = Number(item.saleTotal).toFixed(2)
              item.rejectTotal = item.rejectNum * item.salePrice
              item.rejectReturnStatus = item.rejectReturnStatus == "1" ? "已生效" : item.rejectReturnStatus ==
                "2" ? "未生效" : "——"
              item.rejectApplyStatus = this.rejectApplyStatus[item.rejectApplyStatus]
              item.returnStatus = item.returnStatus == "1" ? "已生效" : item.returnStatus == "2" ? "未生效" : "——"
