<template>
  <div>
    ...
    <!-- 弹窗 -->
    <getAgency ref="agency" @changeCurrentPage="changeCurrentPage" @searchUser="searchUser" @chooseUser="chooseUser">
    </getAgency>
    <!-- 多选 -->
      ...
      this.$csmList.getDitch(Object.assign({}, limit))
        .then(data => {
          if (data && data.code === 200) {
            ...
            this.rebackArr.forEach(item => {
              this.gridData.forEach((grid, index) => {
                if (JSON.stringify(grid) == JSON.stringify(item)) {
                  console.log(index)
                  this.$refs.agency.$refs.multipleTable.toggleRowSelection(grid);
                }
              })
            })
            console.log(this.rebackArr)
