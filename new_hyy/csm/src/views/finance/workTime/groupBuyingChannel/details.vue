  ...
  <div class="detailsLayer">
    <div class="searchAdition">
      ...
      <el-form class="detailsForm" size="small" label-width="140px">
        <el-form-item :rules="[ { required: true, message: '请选择客户名称', trigger: 'change' },]" label="客户名称：">
          <div v-if="pageType != 2">{{form.customerName}}</div>
          <el-input v-else @focus="$refs['custId'].openDialog(true)" :style="{width:'inherit'}" placeholder="请选择客户名称" v-model="form.customerName"></el-input>
        </el-form-item>
        <el-form-item label="客户编码：" v-if="['1','3'].includes(pageType)">
          <div v-if="pageType != 2">{{form.customerCode}}</div>
          <el-input v-else @focus="$refs['custId'].openDialog(true)" :style="{width:'inherit'}" v-model="form.customerCode"></el-input>
        </el-form-item>
        <el-form-item :rules="[ { required: true, message: '请输入团购券名称', trigger: 'change' },]" label="团购劵名称：">
          <div v-if="pageType == 1">{{form.name}}</div>
          <el-input v-else placeholder="请输入团购券名称" :style="{width:'inherit'}" v-model="form.name" maxlength="20"></el-input>
        </el-form-item>
        ...
        <el-form-item label="有效期：">
          ...
          <el-date-picker v-else :style="{width:'inherit'}" v-model="form.date" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd HH:mm:ss">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="代金劵抵用金额：" :rules="[ { required: true, message: '请输入抵扣金额', trigger: 'change' },]">
          <span v-if="pageType == 1">{{form.amount}} 元</span>
          <template v-else>
            <el-input placeholder="请输入抵扣金额" :style="{width:'inherit'}" v-model="form.amount"></el-input>
            <span :style="{marginLeft:'10px'}">元</span>
          </template>
        </el-form-item>
        <el-form-item :rules="[ { required: true, message: '请输入结算金额', trigger: 'change' },]" label="代金劵结算金额：">
          <span v-if="pageType == 1">{{form.settlementAmount}} 元</span>
          <template v-else>
            <el-input placeholder="请输入结算金额" :style="{width:'inherit'}" v-model="form.settlementAmount"></el-input>
            <span :style="{marginLeft:'10px'}">元</span>
          </template>
        </el-form-item>
    ...
    <div class="searchAdition" v-if="form.useMer != 1">
      ...
      <el-row style="padding:5px;float:right;marginBottom:15px;" v-if="pageType !=1">
        <el-button type="primary" plain class="xinjian" @click="selectedGoodsDialogVisible=true">选择商品</el-button>
        <el-button type="primary" plain class="xinjian" @click="totalDel(null)">删除</el-button>
      </el-row>
      <br style="clear:both;">
      <el-table ref="multipleTable" :data="dialogFeedbackData" stripe border style="width: 100%" @selection-change="handleSelectionChange" :key="pageType">
        <el-table-column v-if="pageType !=1" type="selection" width="55">
        </el-table-column>
    ...
    <selected-goods :dialogVisible.sync="selectedGoodsDialogVisible" :dialogFeedbackData="dialogFeedbackData" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack"></selected-goods>
    <multiCinema @frameCinemaDialogCallBack="frameCinemaDialogCallBack" :reviewData="reviewData" :dialogVisible.sync="dialogVisible" :whereUes="whereUes" :innerCinemaMultiData="innerData" :disabledData="disabledData"></multiCinema>
...
import tradeMerchantSingleDialog from 'csmSrc/dialogs/tradeMerchantSingle'
import selectedGoods from "csmSrc/dialogs/cimSelectedGoods/index.vue";
import { grouponTicketSave, grouponTicketView, grouponTicketUpdate } from "csmSrc/http/interface"
const { multiCinema2 } = requireModule('base')
export default {
  components: {
    selectedGoods,
    multiCinema: multiCinema2,
