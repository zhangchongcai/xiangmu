  ...
  <div class="groupBuyingChannelLayer">
    <div class="searchAdition">
      <el-form :inline="true" class="demo-form-inline search-form" size="small" label-width="110px">
        ...
        <el-form-item label="团购券名称：">
          <el-input v-model="name" placeholder="请输入团购券名称"></el-input>
        </el-form-item>
        ...
        <el-form-item label="适用影院：">
          <el-input class="cinemasInput" v-model="cinemasModel" @focus="inputFocus" placeholder="请选择影院"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" class="xinjian" @click="getData">查询</el-button>
        </el-form-item>
        <!-- test -->
    ...
    <div class="content">
      <el-row style="padding:5px;float:right;">
        <el-button type="primary" plain class="xinjian" @click="goDetails({grouponTicketEntity:''},'新建')">新建</el-button>
      </el-row>
      <br style="clear:both;">
      <!-- <el-row> -->
      <el-table :data="tableData" stripe border style="width: 100%">
        ...
        <el-table-column label="操作" width="170">
          <template slot-scope="scope">
            <el-button size='small' type="text" @click="goDetails(scope.row,'查看')">查看</el-button>
            <el-button size='small' type="text" @click="goDetails(scope.row,'修改')">修改</el-button>
            <el-button size='small' type="text" @click="deleteItem(scope.row.grouponTicketEntity.uid)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination v-if="total" class="pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 15, 30]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
      <!-- </el-row> -->
...
import { grouponTicketQuery, grouponTicketDelete } from "csmSrc/http/interface"
...
export default {
  ...
  methods: {
    ...
    goDetails (item, urlName) {
      this.$store.commit("tagNav/removeTagNav", { path: "/trade/groupBuyingChannel/details" });
      this.$router.push({
        path: 'details',
        query: { uid: item.grouponTicketEntity.uid, urlName }
      })
    }
  },
  watch: {
