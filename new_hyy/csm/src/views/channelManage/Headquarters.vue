<template>
  <section class="headquarters">
    ...
    <el-dialog title="院线渠道详情" :visible.sync="detailDialogVisible">
      <el-form :model="detailForm" label-width="120px">
        ...
        <el-form-item label="授权影院：">
          <span v-if="detailForm.cinemaNames && detailForm.cinemaNames.length > 0">{{detailForm.cinemaNames.join('，')}}</span>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="设置授权影院" :visible.sync="outerVisible">
      ...
      <div>
        ...
        <div style="margin-top: 20px">
          ...
          <i class="el-icon-more" style="background: #3B74FF; color: #fff; border-radius: 2px; cursor: pointer" @click="innerVisible = authorizedCinemaData.useRange === 2 ? true : false"></i>
    ...
    <el-dialog title="选择渠道" :visible.sync="dialogVisible" width="892px">
      <el-form :inline="true" :model="searchForm" :label-width="formLabelWidth" size="small">
        <el-form-item label="渠道类型">
          <el-select v-model="searchForm.type" placeholder="请选择">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="item in dialogChannelNatureOptions" :key="item.keyCode" :label="item.keyName" :value="item.keyCode">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="渠道编码">
        ...
        <el-form-item style="float: right; margin-right: 0">
          <el-button style="width: 80px; height: 32px" type="primary" @click="getChannelOtherList">查询</el-button>
        </el-form-item>
      </el-form>

      <div style="display: flex">
        <el-table :data="gridData" ref="multipleTable" height="420" @selection-change="handleSelectionChange" style="width: 70%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button :disabled="!multipleSelection.length" type="primary" @click="addHeaderChannel">保存</el-button>
    ...
    <el-form :inline="true" :model="formData" class="demo-form-inline search-form" size="small">
      ...
      <el-form-item label="渠道名称：">
        <el-input placeholder="请输入渠道名称" v-model="formData.name" clearable>
        </el-input>
      </el-form-item>
      <el-form-item>
    ...
    <div class="el-row-btns clearfix">
      <el-button class="normal-btn" style="float: right;" @click="addChannel" size="small">添加渠道</el-button>
    </div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="channelNature" label="渠道类型" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column prop="cinemaNames" label="授权影院数" width="240" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="scope.row.cinemaNames && scope.row.cinemaNames.length > 0">{{scope.row.cinemaNames.join('，')}}</span>
        </template>
      </el-table-column>
    ...
    <div class="page-wrap">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="formData.page" :page-sizes="[20, 40, 60, 80]" :page-size="formData.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
  </section>
</template>
