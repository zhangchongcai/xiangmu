<template>
  <ul class="aside-list">
    <template v-for="(item, i) in items">
      <li class="title" v-if="item.title" :key="i">{{ item.title }}</li>
      <li class="item" v-else :key="i">
        <router-link :to="item" :title="item.text"><i :class="'iconfont icon-' + item.icon"></i><span>{{ item.text }}</span></router-link>
        <ul class="list" v-if="item.children">
          <template v-for="(sub, s) in item.children">
            <li class="divider" v-if="sub.divider" :key="s"></li>
            <li class="item" v-else :key="s">
