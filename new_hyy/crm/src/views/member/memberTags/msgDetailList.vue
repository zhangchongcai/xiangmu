  ...
  <div class="member-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      ...
      <el-form-item label="手机号：" prop="mobileNum">
        <el-input v-model="formData.mobileNum" @blur="()=>{formData.mobileNum = formData.mobileNum.trim()}" placeholder="填写手机号" clearable></el-input>
      </el-form-item>
      <el-form-item label="会员ID：" prop="memberId">
        <el-input v-model="formData.memberId" @blur="()=>{formData.memberId = formData.memberId.trim()}" placeholder="填写会员ID" clearable></el-input>
      </el-form-item>
      <el-form-item label="会员姓名：" prop="memberName">
        <el-input v-model="formData.memberName" @blur="()=>{formData.memberName = formData.memberName.trim()}" placeholder="填写会员姓名" clearable></el-input>
      </el-form-item>
      <el-form-item label="短信批次号：" prop="messageBatchNo">
        <el-input v-model="formData.messageBatchNo" @blur="()=>{formData.messageBatchNo = formData.messageBatchNo.trim()}" placeholder="短信批次号" clearable>
        </el-input>
      </el-form-item>
      ...
      <el-form-item label="发送时间：" prop="date">
        <el-date-picker v-model="formData.date" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd"></el-date-picker>
      </el-form-item>
    ...
    <div class="_msg-detail-list-table _m-member-table-custom">
      <el-table ref="msgDetailTable" :data="tableData" stripe style="width: 100%" :empty-text="tipMessage" :row-key="getRowKeys" @selection-change="handleSelectMsg">
        ...
        <el-table-column prop="messageBatchNo" label="短信批次号" min-width="115" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="failReason" label="失败原因" min-width="120" :formatter="formateEmpty" show-overflow-tooltip>
          <template slot-scope="scope">
            <span v-if="scope.row.failReason != null && scope.row.failReason != ''" class="circle" :class="scope.row.failReason=='systemError'?'system-reason':scope.row.failReason=='messageTimeSpanError'? 'send-frequent':''"></span>
