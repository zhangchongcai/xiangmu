  ...
  <div class="_different-industry-goods-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      <el-form-item label="商品ID：" prop="goodsNo">
        <el-input v-model="formData.goodsNo" @blur="()=>{formData.goodsNo = formData.goodsNo.trim()}" placeholder="商品ID" clearable></el-input>
      </el-form-item>
      <el-form-item label="商品名称：" prop="goodsName">
        <el-input v-model="formData.goodsName" @blur="()=>{formData.goodsName = formData.goodsName.trim()}" placeholder="商品名称" clearable></el-input>
      </el-form-item>
      <el-form-item label="商家名称：" prop="merchantName">
        <el-input v-model="formData.merchantName" @blur="()=>{formData.merchantName = formData.merchantName.trim()}" placeholder="商家名称" clearable></el-input>
      </el-form-item>
    ...
    <div class="member-list-table _m-member-table-custom">
      <el-table :data="tableData" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column prop="merchantName" label="商家名称" min-width="120" :formatter="formateEmpty" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="goodsAttributeName" label="商品属性" min-width="100" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="status" label="状态" min-width="120" show-overflow-tooltip>
          <template slot-scope="scope">
            <span :class="scope.row.status=='not_on'?'not_on':scope.row.status=='has_been_on'?'has_been_on':'has_been_off'" style="fontSize:12px;">{{scope.row.status
              | judegStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" min-width="200">
          <template slot-scope="scope">
            ...
            <el-button @click="handleStartStopUse('has_been_on',scope.row)" type="text" size="small" class="operation-button" v-if="scope.row.status=='not_on'?true:false">上架</el-button>
            <el-button @click="handleStartStopUse('has_been_off',scope.row)" type="text" size="small" class="operation-button" v-if="scope.row.status=='has_been_on'?true:false">下架</el-button>
    ...
    <el-dialog title="请选择商品属性" :visible.sync="goodsAttributeVisible">
      <div class="_mbmber-goods-attribute-visible">
        <div class="_member-goods-item-wrap" @click="handleCreatCoupon" @mouseover="()=>{iconStatus = 1}" @mouseout="()=>{iconStatus = 0}">
          <div class="_icon-wrap"><i :class="iconStatus==2?'iconfont icon-dianziyouhuiquan-selected':'iconfont icon-dianziyouhuiquan-normal'" style="font-size:80px;color:#3B74FF;" :style="iconStatus==1?'color:#0F55FF':'color:#3B74FF'"></i></div>
    ...
    <el-dialog title="补加电子优惠券" :visible.sync="supplementVisible" @close="closeDialog">
      <div class="_mbmber-supplement-visible">
        ...
        <el-upload class="upload-demo" :action="$store.state.default.apiConfig.baseURL+$store.state.crm.fileUploadUrl" :on-success="fileUploadSuccess" :show-file-list="false" :before-upload="beforeFileUpload" name="file" :headers="uploadHeaders">
