<template>
  <div class="add-own-rights-type">
        ...
        <el-collapse-item :title="stepData.stepList[0].name" name="1">
          <div class="card-type-basic-set">
            ...
            <el-form-item label="权益说明：" prop="equityRemark">
              <el-input type="textarea" resize="none" maxlength="50" :rows="3" v-model="ruleForm.equityRemark">
              </el-input>
            </el-form-item>
                ...
                <div class="applicable-cinema-hall-wrap">
                  <div class="applicable-cinema-hall-inner">
                    ...
                    <el-form-item label="适用影厅：" :prop="'movieList.'+ index + '.hallList'" :rules="{
                      ...
                      <el-checkbox :indeterminate="domain.isIndeterminateHallList" v-model="domain.checkAllHallList" @change="handleCheckAllCinemaHall($event,index)">全选</el-checkbox>
                    ...
                    <el-form-item label="适用制式：" :prop="'movieList.'+ index + '.screenList'" :rules="{
                      ...
                      <el-checkbox :indeterminate="domain.isIndeterminateScreenList" v-model="domain.checkAllScreenList" @change="handleCheckAllStandards($event,index)">全选</el-checkbox>
                    ...
                    <div class="unified-setup">
                      ...
                      <el-form-item label="放映星期：" :prop="'movieList.'+ index + '.weeks'" :rules="{
                        ...
                        <el-checkbox :indeterminate="domain.isIndeterminateWeeks" v-model="domain.checkAllWeeks" @change="handleCheckAllWeeks($event,index)">全部时段</el-checkbox>
                      ...
                      <el-form-item label="放映时段：" :prop="'movieList.'+ index + '.dayTimesJson'" :rules="{
                        ...
                        <div v-for="(domainTime, indexTime) in domain.dayTimesJson" :key="indexTime" class="detail-time-select-item">
                          <el-time-picker is-range size="medium" v-model="domain.dayTimesJson[indexTime]" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间" placeholder="选择时间范围" value-format="HH:mm:ss"></el-time-picker>
                        ...
                        <el-row>
                          <el-col :span="1" style="width:25px">
                            <div class="grid-content bg-purple cursor" @click.prevent="handleAddTimeInterval(index)"><i class="el-icon-circle-plus-outline" style="color:#3b74ff;font-size:20px;"></i></div>
                          </el-col>
                          <el-col :span="1">
                            <div class="grid-content bg-purple-light cursor" style="font-size:12px;margin-top:-2px;color:#3b74ff" @click.prevent="handleAddTimeInterval(index)">添加</div>
                          </el-col>
                        </el-row>
                      </el-form-item>
                      <!-- 优惠方式 -->
                      <el-form-item label="优惠方式：" :prop="'movieList.'+ index" :rules="{
                        ...
                        <el-input v-model="domain.ticketSaleData" placeholder="请输入内容" :disabled="!domain.ticketSaleType" size="medium"></el-input>
                      </el-form-item>
                      <!-- 优惠限制 -->
                      <el-form-item label="优惠限制：" :prop="'movieList.'+ index" :rules="{
                        ...
                        <div class="ticket-preferential-restrictions-item">
                          ...
                          <el-input @input="changeTicketLimitSaleData($event,index)" :value="domain.ticketLimitSaleType == 'perMemberDay'?domain.ticketLimitSaleData:''" placeholder="请输入优惠张数" :disabled="!(domain.ticketLimitSaleType == 'perMemberDay')" size="medium"></el-input>
                        </div>
                        <div class="ticket-preferential-restrictions-item">
                          <el-radio v-model="domain.ticketLimitSaleType" label="perMemberWeek">每周优惠张数上限</el-radio>
                          <el-input @input="changeTicketLimitSaleData($event,index)" :value="domain.ticketLimitSaleType == 'perMemberWeek'?domain.ticketLimitSaleData:''" placeholder="请输入优惠张数" :disabled="!(domain.ticketLimitSaleType == 'perMemberWeek')" size="medium"></el-input>
                        </div>
                        <div class="ticket-preferential-restrictions-item">
                          <el-radio v-model="domain.ticketLimitSaleType" label="perMemberMonth">每月优惠张数上限</el-radio>
                          <el-input @input="changeTicketLimitSaleData($event,index)" :value="domain.ticketLimitSaleType == 'perMemberMonth'? domain.ticketLimitSaleData : ''" placeholder="请输入优惠张数" :disabled="!(domain.ticketLimitSaleType == 'perMemberMonth')" size="medium"></el-input>
                        </div>
                      </el-form-item>
                      <!-- 补贴方式 -->
                  ...
                  <div class="applicable-cinema-hall-inner">
                    <div class="unified-setup">
                      ...
                      <el-form-item label="适用影院：" class="shiyong_yingyuan" prop="goods.cinemaList" :rules="{required: true, validator: checkSaleCinema, trigger: 'blur'}">
                        <div class="select-btn" v-show="!(ruleForm.goods.cinemaList && ruleForm.goods.cinemaList.length > 0)" @click="chooseCinema('卖品')">请选择影院</div>
                        <div class="cinema-list" v-show="ruleForm.goods.cinemaList && ruleForm.goods.cinemaList.length > 0" @click="chooseCinema('卖品')">
                        ...
                        <div class="select-btn" v-show="ruleForm.goods.cinemaList && ruleForm.goods.cinemaList.length > 0" @click="chooseCinema('卖品')">
                          编辑
                        </div>
                      </el-form-item>
                      <el-form-item label="适用卖品：" class="shiyong_yingyuan" prop="goods.goodsList" :rules="{required: true, validator: checkSaleName, trigger: 'change'}">
                        <div class="select-btn" v-show="!(ruleForm.goods.goodsList && ruleForm.goods.goodsList.length > 0)" @click="chooseGoods()">请选择卖品</div>
                        <div class="cinema-list" v-show="ruleForm.goods.goodsList && ruleForm.goods.goodsList.length > 0" @click="chooseGoods()">
                          {{getArrGoodsName(ruleForm.goods.goodsList)}}
                          <i class="el-tag__close el-icon-close myclose" @click.stop="clearGoods('卖品')"></i>
                        </div>
                        <div class="select-btn" v-show="ruleForm.goods.goodsList && ruleForm.goods.goodsList.length > 0" @click="chooseGoods()">
                          编辑
                        </div>
                      </el-form-item>
                      <el-form-item label="优惠方式：" prop="goods" :rules="{
                        ...
                        <el-input v-model="ruleForm.goods.goodsSaleData" placeholder="请输入内容" :disabled="!ruleForm.goods.goodsSaleType" size="medium"></el-input>
                      </el-form-item>
                    </div>
                  </div>
                </div>
              ...
              <div class="sale-preference">
                <div class="applicable-cinema-hall-wrap">
                  <div class="applicable-cinema-hall-inner">
                    <div class="unified-setup">
                      <!-- 代金劵 -->
                      <div style="padding: 10px 0 0 0" class="dajinjuan" v-for="(item, index) in ruleForm.ticketList" :key="index">
                        <el-row :gutter="20" align="middle">
                          <el-col :span="6">
                            <el-form-item label-width="100px" label="销售单号：" :prop="'ticketList.'+index+'.ticketNo'" :rules="{
                              required: true, validator: checkticketListTicketNo, trigger: 'change'}" class="maximum-subsidy-wrap">
                              <el-input readonly v-model="item.ticketNo" placeholder="销售单号" size="medium" @focus="showccmSaleList('dai',index)">
                              </el-input>
                            </el-form-item>
                          </el-col>
                          <el-col :span="4">
                            <el-form-item label-width="50px" label="张数" class="maximum-subsidy-wrap" :prop="'ticketList.'+index+'.totalNum'" :rules="{required: true, validator: checkIntNum, trigger: 'blur'}">
                          ...
                          <el-col :span="2">
                            <div v-if="ruleForm.ticketList.length > 1" class="grid-content bg-purple font-style cursor" style="color:#3b74ff" @click="removeTicket(index)">
                              删除
                            </div>
                          </el-col>
                        </el-row>
                      ...
                      <div style="color:#3b74ff;margin-top:10px;margin-left:10px">
                        <el-row>
                          <el-col :span="1" style="width:25px">
                            <div class="grid-content bg-purple cursor" @click="addTicket"><i class="el-icon-circle-plus-outline" style="color:#3b74ff;font-size:20px;"></i></div>
                          </el-col>
                          <el-col :span="1">
                            <div class="grid-content bg-purple-light cursor" style="font-size:12px" @click="addTicket">
                              添加</div>
                          </el-col>
                        </el-row>
                      </div>
                      ...
                      <div>
                        <el-form-item label="发送时间：" prop="birthday.issueType" required class="birthday-time">
                          <el-radio v-model="ruleForm.birthday.issueType" label="BEFORE_DAY">
                            生日前 <el-input placeholder="天数" :disabled="ruleForm.birthday.issueType == 'SAME_MONTH'" v-model="ruleForm.birthday.outOfTime" style="width:60px;margin-right:10px"></el-input>天
                          </el-radio>
                        ...
                        <el-form-item label="赠送劵类型：" required prop="birthday.birthdayCodeOrId" class="birthday-type">
                          <el-checkbox-group v-model="ruleForm.birthday.birthdayCodeOrId">
                            <el-checkbox v-for="(item, index) in birthdayCardType" :key="index" :label="item">{{item}}
                            </el-checkbox>
                          </el-checkbox-group>
                        </el-form-item>
                        <el-form-item>
                          <div v-if="ruleForm.birthday.birthdayCodeOrId.indexOf('代金券') != -1" style="background:#f5f5f5;width:500px;padding-left: 13px;">
                            ...
                            <div style="margin-left:20px">
                              <el-form-item v-for="(voucheritem, index) in ruleForm.birthday.voucherList" :key="index" style="margin-top:14px">
                                <el-row>
                                  <el-col :span="14">
                                    <div class="grid-content bg-purple">
                                      <el-form-item label="销售单号" label-width="80px" :prop="'birthday.voucherList.'+index" :rules="{required: true, validator: checkvoucherListTicketNo, trigger: 'change'}">
                                        <el-input v-model="voucheritem.ticketNo" placeholder="销售单号" readonly @focus="showccmSaleList('birthdaydai',index)"></el-input>
                                      </el-form-item>
                                    </div>
                                  </el-col>
                                  <el-col :span="7">
                                    <div class="grid-content bg-purple">
                                      <div class="grid-content bg-purple">
                                        <el-form-item label="张数" label-width="50px" :prop="'birthday.voucherList.'+index" :rules="{required: true, validator: checkvoucherListNum, trigger: 'blur'}">
                                          <el-input v-model="voucheritem.num" placeholder="张数" style="width:60px">
                                          </el-input>
                                        </el-form-item>
                                      </div>
                                    </div>
                              ...
                              <div style="color:#3b74ff;margin-top:8px;margin-left:10px">
                                <el-row>
                                  <el-col :span="1" style="width:25px">
                                    <div class="grid-content bg-purple cursor" @click="addVoucher"><i class="el-icon-circle-plus-outline" style="color:#3b74ff;font-size:20px;margin-top:8px"></i></div>
                                  </el-col>
                                  <el-col :span="2">
                          ...
                          <div v-if="ruleForm.birthday.birthdayCodeOrId.indexOf('兑换码') != -1" style="background:#f5f5f5;width:500px;padding-left: 13px;">
                            ...
                            <div style="margin-left:20px">
                              <el-form-item v-for="(cdkeyitem, index) in ruleForm.birthday.cdkeyList" :key="index" style="margin-top:14px">
                                <el-row>
                                  <el-col :span="14">
                                    <div class="grid-content bg-purple">
                                      <el-form-item label="销售单号" label-width="80px" :prop="'birthday.cdkeyList.'+index" :rules="{required: true, validator: checkcdkeyListTicketNo, trigger: 'change'}">
                                        <el-input v-model="cdkeyitem.ticketNo" placeholder="销售单号" readonly @focus="showccmSaleList('birthdaycdk',index)"></el-input>
                                      </el-form-item>
                                    </div>
                                  </el-col>
                                  <el-col :span="7">
                                    <div class="grid-content bg-purple">
                                      <div class="grid-content bg-purple">
                                        <el-form-item label="张数" label-width="50px" :prop="'birthday.cdkeyList.'+index" :rules="{required: true, validator: checkcdkeyListNum, trigger: 'blur'}">
                                          <el-input v-model="cdkeyitem.num" placeholder="张数" style="width:60px">
                                          </el-input>
                                        </el-form-item>
                                      </div>
                                    </div>
                              ...
                              <div style="color:#3b74ff;margin-top:8px;margin-left:10px">
                                <el-row>
                                  <el-col :span="1" style="width:25px">
                                    <div class="grid-content bg-purple cursor" @click="addCdkey"><i class="el-icon-circle-plus-outline" style="color:#3b74ff;font-size:20px;margin-top:8px"></i></div>
                                  </el-col>
                                  <el-col :span="2">
                    ...
                    <div class="unified-setup">
                      ...
                      <el-form-item label="出票金额：" :prop="'movieList.'+ index" :rules="{
                        ...
                        <el-select v-model="domain.ticketIssueType" placehaolder="请选择" size="medium">
                          <el-option v-for="(item,index) in timeCardSetOptions" :key="index" :label="item.label" :value="item.value"></el-option>
                        </el-select>
                        <el-input v-model="domain.ticketIssueValue" placeholder="请输入内容" :disabled="!domain.ticketIssueType" size="medium"></el-input>
                      </el-form-item>
                      <!-- 优惠限制 -->
                      <!-- <el-form-item label="优惠限制：" :prop="'movieList.'+ index" :rules="{
                              required: false, validator: checkLimitSaleData, trigger: 'blur'
    ...
    <!-- 使用影院弹层 -->
    <frameMultiCinema :dialogVisible.sync="dialogVisible" :innerCinemaMultiData="{type:2}" :whereUse="whereUse" :reviewData="innerCinemaMultiData" :disabledData="disabledData" @frameCinemaDialogCallBack="handleCallBack">
    </frameMultiCinema>
    <!--选择销售单号弹窗-->
    <ccmSaleList ref="ccmSaleList" :incomeData="incomeData" :disabledData="disabledVoucherData" @ccmSaleListCallBack="ccmSaleListCallBack" />
    <!--选择卖品弹窗-->
    <cimSelectedGoods ref="cimSelectedGoods" :dialogFeedbackData="dialogFeedbackData" :dialogVisible.sync="selectedGoodsDialogVisible" :autoClose="false" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack" />
  </div>
</template>
<script>
import FixStepTool from '../../../components/fix-step-tool/fix-step-tool';
import fixStepMixin from '../../../mixins/CRM/fixStepTool.js';
import cimSelectedGoods from '../../../dialogs/cimSelectedGoods/index';
import ccmSaleList from '../../../alert/saleList/index';
...
// const { saleList } = requireModule('ccm');
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        'birthday.issueType': [
          { required: true, validator: birthdaytime, trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    getIndexOtherCinemaList (index) {
      ...
      allList = this.cinemaAddCinemaId(allList);
      return allList;
    },
    // 清空当前适用影院
    ...
    goodsAddGoodsCodeAndName (arr) {
      ...
      arr.map((item, index) => {
        if (item.skuUid) {
          item.goodsCode = item.skuUid;
        } else if (item.uid) {
          item.goodsCode = item.uid;
        } else {
