  ...
  <div class="member-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      ...
      <el-form-item label="政策名称：" prop="cardProductName">
        <el-input v-model="formData.cardProductName" @blur="()=>{formData.cardProductName = formData.cardProductName.trim()}" placeholder="填写政策名称" clearable>
        </el-input>
      </el-form-item>
    ...
    <div class="member-list-table _m-member-table-custom">
      <el-table :data="tableData" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column prop="cardTypeCode" label="卡类型" min-width="100" :formatter="formateEmpty" show-overflow-tooltip>
          <template slot-scope="scope">{{scope.row.cardTypeCode | formatCardTypeCode}}</template>
        </el-table-column>
        <el-table-column prop="channelType" label="可售渠道" min-width="120" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" min-width="165">
          <template slot-scope="scope">
            ...
            <el-button @click="handleChangeStatus('start',scope.row)" type="text" size="small" class="operation-button" v-if="scope.row.status=='unstart'?true:false">启用</el-button>
            <el-button @click="handleChangeStatus('stop',scope.row)" type="text" size="small" class="operation-button" v-if="scope.row.status=='start'?true:false">停用</el-button>
            <el-button @click="handleSetDefault(scope.row)" type="text" size="small" class="operation-button" v-if="scope.row.cardTypeCode == 'stored_card' && scope.row.status=='start' && scope.row.defaultCard == 0?true:false">
              设为默认</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 设置默认的dialog -->
    <el-dialog title="" :visible.sync="setDefaultDialog" width="320px" :show-close="false" class="_member-tip-dialog-custom-style">
      <i class="iconfont icon-danchuang-tishi _member-tip-dialog-icon"></i>
