  ...
  <div class="entity-card-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      ...
      <el-form-item label="领用影院：" prop="receiveCinemaName">
        <el-input v-model="formData.receiveCinemaName" @blur="()=>{formData.receiveCinemaName = formData.receiveCinemaName.trim()}" placeholder="请输入影院名称" clearable>
        </el-input>
      </el-form-item>
      <el-form-item>
    ...
    <div class="head-info-wrap">
      ...
      <div>
        <el-button @click="handleApplyCard('apply')" size="small" plain class="_el-btn-custom _member-custom-ghost-button">
          领用
        </el-button>
      </div>
    </div>
    <div class="entity-card-list-table _m-member-table-custom">
      <el-table :data="tableDataList" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column prop="cardProductName" label="政策名称" min-width="95" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="cinemaReceiveCount" label="已领用" min-width="65" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        <el-table-column prop="cinemaNoReceiveCount" label="未领用" min-width="65" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
        <el-table-column prop="cardBatchCinemaVOList" label="领用影院" min-width="80" show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.cardBatchCinemaVOList && scope.row.cardBatchCinemaVOList.length == 1">{{scope.row.cardBatchCinemaVOList[0].cinemaName}}</span>
            <span v-else @click="handleQueryCinema(scope.row.cardBatchCinemaVOList)" style="color:#3b74ff;cursor:pointer;">查看</span>
          </template>
        </el-table-column>
        <el-table-column prop="cinemaNeedReceiveCount" label="领用数量" min-width="95">
          <template slot-scope="scope">
            ...
            <el-input-number v-model="scope.row.cinemaNeedReceiveCount" :step="1" step-strictly :controls="false" :min='1' :max="scope.row.cinemaNoReceiveCount" class="used-num"></el-input-number>
          </template>
        </el-table-column>
        <el-table-column prop="cinemaNeedReturnCount" label="退还数量" min-width="95">
          <template slot-scope="scope">
            ...
            <el-input-number v-model="scope.row.cinemaNeedReturnCount" :step="1" step-strictly :controls="false" :min='1' :max="scope.row.cinemaReceiveCount" class="return-num"></el-input-number>
          </template>
        </el-table-column>
    ...
    <!-- 分页 end -->
    <!-- 领用、退还影院弹层 -->
    <singleCinema ref="frameSingleCinema" :framedialogVisible.sync="cinemaDialogVisible" :type="'2'" :whereUse="whereUse" @callBackSingle="handleCinemaCallBack" :innerData={} :dialogTitle="this.flag=='apply'?'选择领用影院':'选择退还影院'">
      <div slot="footerId">
      ...
      <div class="__table-wrap">
        <div class="_m-member-table-custom">
          <el-table :data="cinemaInfoList" stripe style="width: 100%" empty-text="暂无领用影院信息">
            <el-table-column prop="cinemaName" label="领用影院" min-width="110" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
            <el-table-column prop="cinemaReceiveCount" label="张数" min-width="110" :formatter="formateEmpty" show-overflow-tooltip></el-table-column>
          </el-table>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleQueryCinema (cinemaList) {
      ...
      this.cinemaInfoList = cinemaList || [];
    },
