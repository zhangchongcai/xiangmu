<template>
  <div class="_member-integral-query">
    ...
    <div class="member-level-title">
      <el-checkbox style="height:32px;line-height:32px;" v-model="checked" @change="toggleSelection(checked)">全选
      </el-checkbox>
    ...
    <div class="member-list-table _m-member-table-custom">
      <el-table :data="tableData" stripe style="width: 100%" ref="multipleTable" :empty-text="tipMessage" @selection-change="handleSelectionChange" @select-all="handleAllSelectionChange">
        ...
        <el-table-column prop="memberId" label="会员ID" min-width="100" :formatter="formateEmpty" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="memberName" label="姓名" min-width="100" :formatter="formateEmpty" show-overflow-tooltip>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" min-width="200">
          <template slot-scope="scope">
            <el-button @click="handleIntegralDetail(scope.row)" type="text" size="small" class="operation-button">查看
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    commitIntegral () {
      ...
      if (
        !this.integralParams.integral.trim() ||
        this.integralParams.integral.trim() == 0
      ) {
        this.$message({
          message: '积分不能为空',
          type: 'warn',
      ...
      if (
        this.integralParamsType == '1' &&
        minIntegralBalance != '' &&
        this.integralParams.integral.trim() > minIntegralBalance
      ) {
      ...
      if (
        !/^\+?[1-9]\d{0,3}(\.\d*)?$/.test(this.integralParams.integral.trim())
      ) {
        this.$message({
          message: '积分范围1到9999',
          type: 'warn',
    ...
    handleSearch () {
      ...
      if (this.formData.minBalance || this.formData.maxBalance) {
        ...
        if (
          !/(^[1-9]\d*$)/.test(this.formData.maxBalance.trim()) ||
