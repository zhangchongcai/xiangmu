  ...
  <div class="_edit-membership-level">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" size="medium" label-width="120px" label-position="left">
      ...
      <el-collapse v-model="activeNames" class="_edit-membership-level-content">
            ...
            <div class="_m-member-table-custom _membership-level-table">
              <el-table :data="ruleForm.memberLevelRuleVOList" stripe border style="width: 100%">
                ...
                <el-table-column label="会员等级名称" min-width="120">
                  <template slot-scope="scope">
                    <el-form-item :prop="'memberLevelRuleVOList.'+ scope.$index" :rules="rules.levelName" class="_custom-el-item">
                ...
                <el-table-column label="保级升级规则" min-width="550">
                  <template slot-scope="scope">
                    ...
                    <el-form-item v-else :prop="'memberLevelRuleVOList.'+ scope.$index" :rules="rules.growth" class="_custom-el-item">
                ...
                <el-table-column label="操作" min-width="50" fixed="right">
                  <template slot-scope="scope">
                    <el-button @click="handleDelete(scope)" v-if="scope.$index != 0" type="text" size="small" class="delete-btn">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
        ...
        <el-collapse-item title="成长值获取规则" name="2" class="get-rules">
          <div class="growth-value-options" v-for="(domain, index) in ruleForm.memberGrowthRuleAndChannelVOList" :key="index">
            <span class="remove-growth-value" v-if="ruleForm.memberGrowthRuleAndChannelVOList.length > 1" @click="removeGrowthValue(index,domain)">
              <i class="iconfont icon-neiye-danchuangquxiao" style="fontSize:12px" />
            </span>
            <el-form-item label="获取渠道：" :prop="'memberGrowthRuleAndChannelVOList.'+ index + '.memberGrowthRuleChannelVOList'" :rules="{
              ...
              <el-checkbox-group v-model="domain.memberGrowthRuleChannelVOList" @change="handleCheckedCinemaHallsChange($event,index)">
                <el-checkbox v-for="(channel,index) in AccessChannels" :label="channel.code" :key="index">
                  {{channel.desc}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-form-item label="获取规则：" :rules="{
              ...
              <div class="rules-item" v-for="(item, ruleIndex) of domain.memberGrowthRuleVOList" :key="ruleIndex">
                ...
                <div class="buy-ticket">
                  ...
                  <el-form-item :prop="'memberGrowthRuleAndChannelVOList.'+ index + '.memberGrowthRuleVOList.'+ ruleIndex" :rules="{
                          required: true, validator: checkeveryFullGrowth, trigger: 'blur'
                        }">
                    <el-input v-model="item.everyFullGrowth" :disabled="!item.checked" placeholder="请输入" class="growth-num"></el-input>
                  </el-form-item>
                  <span class="_member-level-rules-desc">成长值，每天上限</span>
                  <el-form-item :prop="'memberGrowthRuleAndChannelVOList.'+ index + '.memberGrowthRuleVOList.'+ ruleIndex" :rules="{
                          required: false, validator: checkeveryDayUpper, trigger: 'blur'
                        }">
                    <el-input v-model="item.everyDayUpper" :disabled="!item.checked" placeholder="请输入" class="growth-num"></el-input>
                  </el-form-item>
    ...
    <el-dialog title="" class="__same-equity-dialog __equity-dialog" :visible.sync="sameCategoryDialog">
      <div class="__same-equity-desc">
        ...
        <span style="color:red;">请仔细检查，避免选择重复的获取渠道</span>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="sameCategoryDialog = false" class="_el-btn-custom  _member-add-edit-save-btn">
          确 定</el-button>
        <el-button @click="sameCategoryDialog = false" class="_el-btn-custom">取 消</el-button>
      </span>
    </el-dialog>
    <!-- 删除的dialog -->
    <el-dialog title="" :visible.sync="deleteDialog" width="320px" :show-close="false" class="_member-tip-dialog-custom-style">
      <i class="iconfont icon-danchuang-tishi _member-tip-dialog-icon"></i>
      <span class="_member-tip-dialog-desc">
        是否删除"{{deleteInfo.row.levelName}}"等级？<br />删除操作不可恢复，该等级相关权益将被删除，请谨慎操作
      </span>
      <span slot="footer" class="dialog-footer">
    ...
    <!-- 该等级下有会员的dialog -->
    <el-dialog title="" :visible.sync="haveMemberDialog" width="320px" :show-close="false" class="_member-tip-dialog-custom-style">
      <i class="iconfont icon-danchuang-tishi _member-tip-dialog-icon"></i>
      <span class="_member-tip-dialog-desc">
        该"{{deleteInfo.row.levelName}}"等级下包含会员，不可删除
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="haveMemberDialog = false" class="_el-btn-custom _member-add-edit-save-btn">确定
        </el-button>
        <!-- <el-button @click="haveMemberDialog = false" class="_el-btn-custom _member-add-edit-save-btn">取消</el-button> -->
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    return {
      haveMemberDialog: false, // 该等级下有会员的dialog
  ...
  methods: {
    ...
    handleDelete (scope) {
      ...
      if (scope.row.id) {
        let params = {
          tenantId: this.$store.state.loginUser.consumerId,
          levelRuleId: scope.row.id
        };
        this.$crmList
          .isExistMemberByLevelNo(params)
          .then(res => {
            if (res) {
              this.haveMemberDialog = true;
            } else {
    ...
    formatChannel (data) {
      ...
      if (repeatArr.length > 0) {
        for (var i = 0; i < repeatArr.length; i++) {
          for (var k = 0; k < this.AccessChannels.length; k++) {
            if (repeatArr[i] == this.AccessChannels[k].code) {
              sameArr.push(this.AccessChannels[k].desc);
              break;
            }
          }
        }
        ...
        for (var i = 0; i < wrapArr.length; i++) {
          let channelInnerArr = wrapArr[i].memberGrowthRuleChannelVOList;
          let newWrapArr = [];
          for (var n = 0; n < channelInnerArr.length; n++) {
            for (var m = 0; m < this.AccessChannels.length; m++) {
              if (channelInnerArr[n] == this.AccessChannels[m].code) {
                newWrapArr.push({
                  channelName: this.AccessChannels[m].desc,
                  channelNo: this.AccessChannels[m].code
                });
                break;
              }
            }
          }
          wrapArr[i].memberGrowthRuleChannelVOList = newWrapArr;
        }
