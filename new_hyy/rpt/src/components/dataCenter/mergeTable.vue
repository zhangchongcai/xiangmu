<template>
  <div class="merge-table-wrapper" v-if="styleGroupArr.length > 0 && mergeTableObj.isMerged">
    <el-table border :stripe="false" :data="mergeTableObj.mergeTableBodyData" :span-method="mergeSpan" :show-summary="!(reportCode == 'Q02')" :summary-method="handleSummary" :style="{width: `${mergeTableObj.totalWidth + 2}px`}" class="merge-table" v-loading="mergeTableLoading">
      <el-table-column v-for="(item, index) in mergeTableObj.mergeTableHeaderData" :key="`merge_table_${index}`" :property="item.colKey" :label="item.colName" :width="item.colWidth" class="rpt-table"></el-table-column>
    </el-table>
    <div class="merge-table-pagenation" v-if="Pagination.total > 0">
      <el-pagination class="rpt-pagination" background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="Pagination.currentPage" :page-sizes="Pagination.page_sizes" :page-size="Pagination.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="Pagination.total"></el-pagination>
    </div>
    <div v-if="mergeTableObj.mergeTableBodyData.length === 0 && styleGroupArr.length > 0 && mergeTableObj.isMerged && mergeTableLoading === false" class="no-data">没有数据</div>
  </div>
</template>
  ...
  watch: {
    ...
    styleColArr: {
      handler (newVal, oldVal) {
        ...
        this.mergeTableObj.mergeTableHeaderData = newVal;
      }
    }
  },
  mounted () {
  ...
  methods: {
    ...
    handleSummary () {
      ...
      for (let i = 0; i < this.mergeTableObj.mergeTableHeaderData.length; i++) {
        ...
        if (i == 0) {
          sumArr.push('合计');
        } else if (i > 0 && i < this.mergeTableObj.groupCount) {
          sumArr.push('');
        } else {
    ...
    handleMergeData (resTitle, resList) {
      ...
      for (var i = 0; i < resList.length; i++) {
        if (i === 0) {
          ...
          if (
            resTitle.length >= 1 &&
            resList[i][`${resTitle[0].colKey}`] ===
            resList[i - 1][`${resTitle[0].colKey}`]
          ) {
            this.mergeTableObj.oneSpanArr[this.mergeTableObj.onePosition] += 1;
          ...
          if (
            resTitle.length >= 2 &&
            resList[i][`${resTitle[1].colKey}`] ===
            resList[i - 1][`${resTitle[1].colKey}`]
          ) {
            this.mergeTableObj.twoSpanArr[this.mergeTableObj.twoPosition] += 1;
          ...
          if (
            resTitle.length >= 6 &&
            resList[i][`${resTitle[5].colKey}`] ===
            resList[i - 1][`${resTitle[5].colKey}`]
          ) {
            this.mergeTableObj.sixSpanArr[this.mergeTableObj.sixPosition] += 1;
    ...
    mergeSpan ({ row, column, rowIndex, columnIndex }) {
      if (this.mergeTableObj.groupCount === 1) {
        if (columnIndex === 0) {
          return this.handleTableCell(this.mergeTableObj.oneSpanArr[rowIndex]);
        }
      }
      ...
      if (this.mergeTableObj.groupCount === 2) {
        if (columnIndex === 0) {
          ...
          return this.handleTableCell(this.mergeTableObj.twoSpanArr[rowIndex]);
        }
      }
      ...
      if (this.mergeTableObj.groupCount === 4) {
        if (columnIndex === 0) {
          ...
          return this.handleTableCell(this.mergeTableObj.fourSpanArr[rowIndex]);
        }
      }
      if (this.mergeTableObj.groupCount === 5) {
        if (columnIndex === 0) {
          ...
          return this.handleTableCell(this.mergeTableObj.fiveSpanArr[rowIndex]);
        }
      }
      if (this.mergeTableObj.groupCount === 6) {
        if (columnIndex === 0) {
          ...
          return this.handleTableCell(this.mergeTableObj.sixSpanArr[rowIndex]);
        }
      }
      if (this.mergeTableObj.groupCount === 7) {
        if (columnIndex === 0) {
    ...
    setStyle (mergeTable, tableHeader, tableBody, tableFooter) {
      if (mergeTable) {
        ...
        if (this.mergeTableObj.mergeTableBodyData.length >= 15) {
          ...
          mergeTable.style =
            String(this.mergeTableObj.totalWidth).indexOf('%') > -1
              ? `overflow: hidden; width: ${this.mergeTableObj.totalWidth};`
              : `overflow: hidden; width: ${this.mergeTableObj.totalWidth +
              2}px;`;
        } else {
          this.setTableStyle(tableHeader, tableBody, mergeTable);
        ...
        mergeTable.addEventListener('scroll', () => {
          ...
          if (this.reportCode == 'Q02') {
            tableBody.style = 'margin: 38px 0 0 0;';
          } else {
