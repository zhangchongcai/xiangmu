<template>
  <div class="query-dialog-wrapper">
    ...
    <div class="my_dialog">
      <el-dialog :title="controllerTitle" :visible.sync="controllerVisible" @close="handleControllerDialogClose">
        ...
        <div class="choose_table">
          <div>
            <el-table :data="controllerTableData" :cell-style="{padding:0}" :row-style="{height:30}" :header-cell-style="{padding:0}" :height="354" ref="multipleTable" @select="handleRowSelect" @select-all="handleSelectAll" @row-click="handleRowClick" v-loading="controllerTableLoading" class="controller-table">
              <el-table-column type="selection" width="40"></el-table-column>
              <template v-for="(item, index) of controllerTablerHeader">
                <el-table-column :property="item.property" :label="item.label" :width="item.width" :key="index"></el-table-column>
              </template>
            </el-table>
            <div class="block">
              <el-pagination @current-change="handleCurrentPageChange" background :current-page.sync="controllerPagination.pageNum" :page-size="controllerPagination.pageSize" layout="total, prev, pager, next, jumper" :total="controllerPagination.total" class="rpt-pagination"></el-pagination>
            </div>
          </div>
          <div class="choose_ul">
            <p class="ul_header">
...
export default {
  ...
  methods: {
    handleIconClick () {
      ...
      if (this.queryData.queryColKey == 'pay_type_name') this.$emit('handlePayTypeValue', this.controllerObj[this.controllerKey].vModelValue);
      this.$forceUpdate();
    },
    handleIconShow () {
    ...
    handleRowSelect (selection, row) {
      ...
      if (this.controllerObj[this.controllerKey].selectedData.length === 0) {
        addTemporaryData.push(selection[selection.length - 1]);
      } else {
        for (
          let i = 0;
    ...
    handleComfirmClick () {
      ...
      if (this.controllerObj[this.controllerKey].selectedData.length !== 0) {
        this.controllerObj[this.controllerKey].selectedData.forEach(element => {
          if (this.queryData.controlType == 'MER_GOODS') {
            if (element.merType == 4) {
              this.queryData.queryColValue += `${element[this.valueKey]},`;
            } else {
              this.queryData.queryColValue += `${element.skuUid},`;
            }
          } else {
            this.queryData.queryColValue += `${element[this.valueKey]},`;
          }
