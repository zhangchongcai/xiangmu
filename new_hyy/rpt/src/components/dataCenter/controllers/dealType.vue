<template>
  <div>
    <!-- 交易类型(含会员) -->
    <el-select v-model="dealTypeObj.dealTypeValue" placeholder="请选择" popper-class="rpt-select" @focus="getdealType('POS_SALE_BILL_BILL_TYPE_HAS_MEMBER',0)">
      <el-option label="全部" value></el-option>
      <el-option v-for="(item,index) in options[0]" :key="index" :label="item.keyName" :value="item.keyName"></el-option>
    </el-select>
    <!-- 交易类型(不包含会员) -->
    <!-- <el-select
