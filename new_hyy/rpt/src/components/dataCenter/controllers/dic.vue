<template>
  <div>
    <el-select popper-class="rpt-select" v-model="queryData.queryColValue" @change="getData" v-if="showAll">
      <el-option label="全部" value></el-option>
      <el-option v-for="item in options" :key="`dic_${item.id}`" :label="item.dicName" :value="item.dicVal"></el-option>
    </el-select>
    <el-select popper-class="rpt-select" v-model="queryData.queryColValue" @change="getData" v-if="!showAll">
      <el-option v-for="item in options" :key="`dic_${item.id}`" :label="item.dicName" :value="item.dicVal"></el-option>
    </el-select>
