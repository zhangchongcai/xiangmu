<template>
  <div>
    ...
    <div ref="myDialog" class="my-dialog" :style="{
      width: width,
      height: height
    }" v-if="visibility">
      <div class="dialog-header" :style="{marginTop: `${marginTop}px`}">
        <div class="title">{{ this.title }}</div>
        <div class="close-icon">
          <i :class="`iconfont ${iconClassName}`" @click="handleCloseClick"></i>
        </div>
      </div>
      <hr class="top-hr" v-if="marginTop !== 0">
...
.my-dialog-mask {
  ...
  opacity: 0.6;
  z-index: 499;
  animation: maskFadeIn ease 0.5s;
}
.my-dialog {
  position: fixed;
  top: 50%;
