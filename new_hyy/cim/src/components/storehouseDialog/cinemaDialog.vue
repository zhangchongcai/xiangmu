<template>
  <el-dialog class="cinema-dialog" :width="multiple ?'896px': '576px'" :title="title" :visible.sync="cinemalDialog" @open="openCallBack">
    <!-- <el-form
      :inline="true"
      :model="queryData"
      label-position="left"
    ...
    <el-row>
      <el-col :span="multiple  ? 19 : 24">
        <el-table class="table-box" ref="cinemalTable" :data="cinemalTableData" row-key="code" height="300" v-loading="tableLoding" @selection-change="handleSelectionCinemal">
          <div v-if="!check">
...
export default {
  ...
  methods: {
    init () {
      ...
      this.queryData.cinemaUid = this.cinemaUid.cinemaUid;
    ...
    deleteSelected (row, flag) {
      ...
      // 取消表格的勾选
      this.cinemalTableData.map(item => {
