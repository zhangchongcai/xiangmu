<template>
  <el-dialog class="good-select-dialog" width="1016px" :title="title" :visible="dialogVisible" @close="closeCallBack" @open="openCallBack">
    <div class="iT-style" id="iT-style-content" v-loading="indexloading">
      <!-- {{this.goodList}} -->
      <div class="content">
        ...
        <el-form :inline="true" ref="ruleForm" :model="queryData" label-position="left" label-width="100px" label-suffix=":">
          <el-collapse v-model="activeNames">
            ...
            <el-collapse-item title="基础信息" name="1">
              <div>
                ...
                <el-row>
                  <el-col :span="10">
                    <el-form-item label="调入门店" class="select-input" prop="inCinemaName" :rules="[{ required: routeQueryType==3 ? false : true, message: '选择调入门店',trigger: 'change' }]">
                      <template v-if="routeQueryType=='1' || routeQueryType=='2'">
                        <el-input v-model="queryData.inCinemaName" clearable @clear="onCinemalSumit" @focus="selectCinemalDialog" placeholder="请选择门店"></el-input>
                  ...
                  <el-col :span="10">
                    <el-form-item label="调出门店" class="select-input" prop="outCinemaName" :rules="[{ required: routeQueryType==3 ? false : true, message: '选择调出门店',trigger: 'change' }]">
                      <template v-if="routeQueryType=='1' || routeQueryType=='2'">
                        <el-input v-model="queryData.outCinemaName" clearable @clear="onCinemalSumit1" @focus="selectCinemalDialog1" placeholder="请选择门店"></el-input>
                ...
                <el-row v-if="this.routeQueryType == '2' || this.routeQueryType == '4' || this.routeQueryType == '3'">
                  <el-col :span="10">
                    <el-form-item label="制单日期">
                      <span>{{this.routeQueryType == '2' ? "-":this.queryData.billTime}}</span>
                    </el-form-item>
                  </el-col>
                  <el-col :span="10">
                    <el-form-item label="制单员">
                      <span>{{this.routeQueryType == '2' ? "-":this.queryData.billUserName}}</span>
                    </el-form-item>
                  </el-col>
                </el-row>
                <el-row v-if="this.routeQueryType == '2' || this.routeQueryType == '4' || this.routeQueryType == '3'">
            ...
            <el-collapse-item title="商品清单" name="2">
              <div>
                <div class="text-right" v-if="routeQueryType!=3">
                  <el-button @click="addPreEvevt()">添加商品</el-button>
                </div>
                <el-table :data="goodList" stripe>
                  <el-table-column v-for="item in tableColumn" :key="item.key" :prop="item.key" :label="item.label" :width="item.width" :formatter="item.formatter">
                  ...
                  <el-table-column label="操作" style="width:180px;" v-if="routeQueryType!=3">
                ...
                <el-row>
                  <el-col :span="12">
                    <el-form-item prop="remark" label-width="60px" label="备注">
                      <span v-if="routeQueryType==3">{{queryData.remark}}</span>
            ...
            <!-- 商品清单 end-->
            <!--  审批流程-->
            <el-collapse-item title="审批流程" name="3" v-if="routeQueryType=='3'">
            ...
            <!--  审批记录-->
            <el-collapse-item title="审批记录" name="4" v-if="routeQueryType=='3'">
              <div>
          ...
          <!-- 选择商品 -->
          <selected-goods :dialogVisible.sync="selectedGoodsDialogVisible" :dialogFeedbackData="goodList" :cinemaUid="queryData.outCinemaUid" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack"></selected-goods>
        </el-form>
      </div>
    </div>
    <span slot="footer">
      <el-button @click="closeCallBack">关闭</el-button>
    </span>
  </el-dialog>
</template>
...
import selectedGoods from 'cimSrc/dialogs/cimDbSelectedGoods/index.vue';
export default {
  props: {
    title: {
      type: String,
  ...
  methods: {
    openCallBack () {
      this.routeQueryType = '3';
      this.init();
    },
    init () {
      // this.routeQueryType == "3"
      if (this.routeQueryType == '1') {
        // 新增状态
        this.goodsDataQueryGoodsList();
      } else if (this.routeQueryType == '3') {
        // 查看状态
        this.resAllotBillFind(this.needData);
      ...
      } else if (this.routeQueryType == '2') {
        // 未提交状态
        this.resAllotBillFind(JSON.parse(this.routeQuery.data));
      } else if (this.routeQueryType == '4') {
        // 已提交状态
        this.resAllotBillFind(JSON.parse(this.routeQuery.data));
      }
    },
          ...
          if (valid) {
            ...
            if (this.goodList.length === 0) {
              ...
              if (newAvtive.a1 === true) {
                ...
                } else if (this.routeQueryType == '2' || this.routeQueryType == '4') {
