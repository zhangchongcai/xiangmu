  ...
  <el-dialog class="channel-dialog" :width="multiple?'750px': '576px'" :title="title" :visible.sync="channelDialog" @open="openCallBack">
    ...
    <el-row>
      <el-col :span="multiple ? 18 : 24">
        <div class="table-box">
          <el-table ref="channelTable" :data="tableData" row-key="channelUid" height="400" v-loading="tableLoding" @selection-change="handleSelectionchannel" stripe empty-text="暂无记录">
...
export default {
  ...
  methods: {
    ...
    openCallBack () {
      // this.dialogFeedbackData = this.dialogFeedbackData;
      // console.log(this.dialogFeedbackData)
      this.init();
    },
    handleDialog (flag) {
    ...
    deleteSelected (row, flag) {
      ...
      //   return item.channelUid != row.channelUid
      // })
      // 取消表格的勾选
      this.tableData.map(item => {
        if (item.channelUid == row.channelUid) {
          this.$refs.channelTable.toggleRowSelection(item, false);
        }
      });
    },
    // 清空选择
