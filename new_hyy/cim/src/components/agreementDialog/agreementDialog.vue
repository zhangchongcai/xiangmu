<template>
  <div>
    <el-dialog class="agreement-dialog" :width="multiple?'950px': '950px'" :title="title" :visible.sync="channelDialog" @open="openCallBack">
      <el-form :inline="true" :model="queryData" label-position="left" label-width="80px" label-suffix=":">
        ...
        <el-form-item label="有效期">
          <el-date-picker class="basic-input" v-model="queryData.billTimeTotal" type="daterange" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择日期"></el-date-picker>
        </el-form-item>
      ...
      <el-row>
        <el-col :span="multiple ? 18 : 24">
          <div class="table-box">
            <el-table ref="channelTable" :data="tableData" row-key="uid" height="300" v-loading="tableLoding" @selection-change="handleSelectionchannel" stripe empty-text="暂无记录">
              <el-table-column type="selection" width="40" reserve-selection v-if="!check" :selectable="selectable"></el-table-column>
            ...
            <div class="page-wrap">
              <el-pagination background :current-page="queryData.page" :page-size="queryData.pageSize" @size-change="handleSizeChange" @current-change="handleCurrentChange" :total="total" layout="total, prev, pager, next, jumper"></el-pagination>
            </div>
          </div>
        </el-col>
    ...
    <el-dialog class="agreement-dialog" title="查看采购协议" :visible.sync="checkProtocolDialog" width="800px">
      <el-form :inline="true" ref="ruleForm" label-position="left" label-suffix="：">
        <el-row>
          <el-col :span="8">
            <el-form-item label="协议编码">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="附件">
              <a target="_Blank" :href="agreementDetail.fileUrl" :download="agreementDetail.fileName" v-if="agreementDetail.fileName">
                <!--                                <span>点击下载: </span>-->
        ...
        <el-table :data="agreementDetail.agreementDetailList" stripe>
          <el-table-column v-for="item in agreementDetailTableColumn" :key="item.key" :prop="item.key" :label="item.label" :formatter="item.formatter">
          </el-table-column>
        </el-table>
      </el-form>
    ...
    return {
      ...
      agreementDetailTableColumn: [
        ...
        },
        {
          ...
          key: 'purCostPrice',
          formatter (row, column, cellValue) {
            return row.purCostPrice.toFixed(4);
          }
        },
        {
          label: '税率(%)',
