<template>
  <el-dialog title="选择品牌" :visible.sync="branddialog" width="50%" :before-close="beforeClose">
    <div class="brand-box">
      ...
      <el-table :data="brandData" stripe>
        <el-table-column align="center" width="55" label="">
          <template slot-scope="scope">
            <el-radio :label="scope.$index" v-model="BRadio" @change.native="getTemplateRow(scope.$index,scope.row)"></el-radio>
          </template>
        </el-table-column>
        <el-table-column v-for="item in brandColumn" :key="item.key" :prop="item.key" :label="item.label" :width="item.width">
