<template>
  <el-dialog id="brand-dialog" width="40%" :title="title" :visible.sync="cinemalDialog" @open="openCallBack">
    ...
    <div class="table-box">
      <el-table ref="cinemalTable" :data="tableData" height="300" row-key="brandUid" v-loading="tableLoding" @selection-change="handleSelectionCinemal" stripe empty-text="暂无记录">
        ...
        <el-table-column v-for="item in cinemalTableColumn" :key="item.key" :prop="item.key" :label="item.label" :formatter="item.formatter"></el-table-column>
      </el-table>
      <div class="page-wrap">
        <el-pagination :current-page="queryData.page" :page-size="queryData.pageSize" @size-change="handleSizeChange" @current-change="handleCurrentChange" :total="cinemalTotal" layout="total, prev, pager, next, jumper"></el-pagination>
      </div>
    </div>
    <span slot="footer">
