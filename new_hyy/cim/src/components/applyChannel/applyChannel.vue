    ...
    <el-radio-group v-model="defaultSelected" @change="handleApplyChange">
      <el-radio v-for="(item) in radios" @click.native="(item)" :label="item.type" :key="item.type">
        ...
        <span class="apply-tag" v-if="item.type!='1' && item.type == defaultSelected">
          <el-input class="input" v-model="item.name" placeholder="请选择" :disabled="item.name ?true:false" @focus="setCheckedKeys(item.type)">
            <i slot="suffix" v-if="item.name" class="el-icon-close icon" @click.stop="handleDelete(item)"></i>
          </el-input>
      ...
      <div class="apply-tree">
        <el-row>
          <el-col :span="24">
            ...
            <el-tree :data="sourceData" :props="defaultProps" show-checkbox node-key="id" :default-expanded-keys="defaultCheckedKys" :filter-node-method="filterNode" ref="applyTree"></el-tree>
          </el-col>
        </el-row>
      </div>
      <span slot="footer">
