<template>
  <div class="retail-style">
    <div class="common-header storageRack-style">
      <el-form :inline="true" :model="queryData" label-position="left" label-suffix="：">
        <el-form-item label="门店名称" class="select-input">
          <el-input v-model="queryData.cinameName" clearable @clear="onCinemalSumit" @focus="selectCinemalDialog" placeholder="请选择门店"></el-input>
        ...
        <el-form-item class="query-btn-box" style="margin-left: 0;">
    ...
    <el-dialog class="change-dialog dialog-common-style" :title="isNewBuile ? '新建货架':'修改货架'" :visible.sync="changeDialog" width="450px">
      <el-form :inline="true" ref="changeForm" :model="changeData" label-position="left" label-width="100px" label-suffix="：" :rules="changeRules" @close="candleBtn">
        ...
        <el-col>
          <el-form-item label="门店名称" class="select-input" prop="cinameName">
            ...
            <div v-else>
              <el-input v-model="newcinameName" clearable @clear="onCinemalSumit1" @focus="selectCinemalDialog1" placeholder="请选择门店"></el-input>
...
export default {
  ...
  data () {
    var codePass = (rule, value, callback) => {
      if (value === '' || value === null) {
        callback(new Error('选择门店会生成货架编码'));
      } else {
        callback();
      }
    };
  ...
  methods: {
    ...
    setCinema1 (val = []) {
      ...
      if (val.length > 0) {
        ...
        this.linshicinemaUid = val[0].uid || val[0].cinemaUid;
        this.linshicinemaName = val[0].name;
    ...
    handleNewBuilt () {
      ...
      } else if (this.linshiCode == true) {
        this.changeData = {
          status: '1',
          cinameUid: this.linshicinemaUid,
          cinameName: this.linshicinemaName,
          name: null,
          address: null,
          code: this.changeData.code
        };
        this.newcinameName = this.linshicinemaName;
        let newObj = [
          {
            cinemaUid: this.linshicinemaUid,
            cinemaName: this.linshicinemaName
          }
        ];
        this.resStoreCode(newObj);
      }
      this.changeDialog = true;
