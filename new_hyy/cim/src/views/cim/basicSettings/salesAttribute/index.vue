<template>
  <div class="retail-style sales-attribute">
    ...
    <el-dialog class="change-dialog" :title="isNewBuile ? '新建销售属性':'修改销售属性'" :visible.sync="changeDialog" width="400px">
      <el-form :inline="true" ref="changeForm" :model="changeData" label-position="left" label-width="100px" label-suffix=":" :rules="changeRules">
        ...
        <el-form-item label="选项值" v-for="(item,index) in changeData.attributeItemEntityList" :prop="'attributeItemEntityList.' + index + '.attrValue'" :key="item.id || item.uid" :class="{'no-label':index != 0}">
          <el-input v-model="item.attrValue" autosize :class="{attrIalueInput:changeData.attributeItemEntityList.length>1}"></el-input>
          <el-button type="text" size="small" @click.stop="handleAttrValueDlete(index)" v-if="changeData.attributeItemEntityList.length>1">删除</el-button>
        </el-form-item>
