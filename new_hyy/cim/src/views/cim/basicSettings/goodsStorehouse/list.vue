<template>
  <div class="retail-style">
    ...
    <el-dialog class="change-dialog dialog-common-style" :title="isNewBuile ? '新建仓库':'修改仓库'" :visible.sync="changeDialog" width="450px">
      <el-form :inline="true" ref="changeForm" :model="changeData" label-position="left" label-width="100px" label-suffix="：" :rules="changeRules">
        ...
        <el-col>
          <el-form-item label="门店名称" class="select-input" prop="cinemaName">
            ...
            <div v-else>
              <el-input v-model="changeData.cinemaName" clearable @clear="onCinemalSumit1" @focus="selectCinemalDialog1" placeholder="请选择门店"></el-input>
      ...
      <span slot="footer">
        <el-button type="primary common-new-built mgr22" @click="handleModificationSubmit">保 存</el-button>
    ...
    <!-- 选择影院弹窗 -->
    <cinemal-dialog ref="myCinemalDialog1" @onSumit="onCinemalSumit1" :dialogFeedbackData="[{cinemaUid:changeData.cinemaUid,cinemaName:changeData.cinemaName}]"></cinemal-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      changeDialog: false,
      changeRules: {
        cinemaName: [{ required: true, message: '请输入门店名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    setCinema1 (val = []) {
      if (val.length > 0) {
        ...
        this.linshicinemaUid = val[0].cinemaUid;
