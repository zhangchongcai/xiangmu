<template>
  <div class="content" id="for-cinema-common">
    ...
    <el-form :inline="true" ref="ruleForm" :model="queryData" label-position="left" label-width="80px" label-suffix="：">
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="基础信息" name="1">
          <div>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="生效日期" class="select-input" prop="startTime" :rules="[{ required: routeQuery.type==1 ? true:  false, message: '生效日期不能为空',trigger: 'change' }]">
                  <span v-if="routeQuery.type==2 || routeQuery.type==3">
                    {{queryData.startTime}}
                  </span>
                  <el-date-picker v-else v-model="queryData.startTime" type="date" :disabled="routeQuery.type!=1 ? true : false" value-format="yyyy-MM-dd" :picker-options="pickerOptions" placeholder="选择日期">
                  </el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
        ...
        <el-collapse-item title="商品调价清单" name="2">
          <div>
            ...
            <div class="recipe-box clearfix" v-for="(merItem,merGroupIndex) in queryData.billMerVoList" :key="merGroupIndex">
              <div class="right" v-if="routeQuery.type==1">
                <i class="el-icon-close right delete-recipe-group" @click.stop="handleDleteRecipeGroup(merItem,merGroupIndex)"></i>
              </div>
                  ...
                  <template slot-scope="{row}" name="header">
                    <div v-if="routeQuery.type!=3">
                      <div v-if="item.edit && item.edit(row)">
                        <el-input size="small" v-model="row[item.key]"></el-input>
                      </div>
                      <div v-else>
              ...
              <div v-if="!merItem.init">
                <div class="bill-group clearfix" v-if="routeQuery.type!=3">
                  <div class="text-right">
                    <el-button @click="handleDialogEvent('refCinemalDialog',{data:merItem,index:merGroupIndex})" type="primary" plain v-if="routeQuery.type==1">
                      调价门店
                    </el-button>
                ...
                <el-table :data="merItem.billCinemaMerVoList" stripe @selection-change="handleSelectedCinemal">
                  <el-table-column v-if="routeQuery.type!=3" type="selection" width="40" reserve-selection></el-table-column>
                  <el-table-column v-for="item in cinemaTableColumn" :key="item.key" :prop="item.key" :label="item.label" :width="item.width" :formatter="item.formatter">
                    <template slot-scope="{row}" name="header">
                      <div v-if="routeQuery.type!=3">
                        <div v-if="item.edit && item.edit(row)">
                          <el-input size="small" type="number" v-model="row[item.key]"></el-input>
                        </div>
                        <div v-else>
                          <span>{{row[item.key]}}</span>
                        </div>
                  ...
                  <el-table-column label="操作" style="width:180px;" v-if="routeQuery.type==1">
                    <template slot-scope="{row}">
                      <el-button type="text" size="small" @click.stop="handleOperateEvent(row,merGroupIndex)">删除
                      </el-button>
                    </template>
                  </el-table-column>
                </el-table>
      ...
      <el-dialog width="300px" class="batch-price-dialog" title="批量调价" :visible="batchDialogVisible" @close="batchDialogVisible=false">
        <el-form :model="batchData" label-width="90px" ref="editDataForm" label-suffix="：">
      ...
      <!-- 选择影院弹窗 -->
      <cinemal-dialog ref="refCinemalDialog" @onSumit="onCinemalSumit" multiple :dialogFeedbackData="selectedCinemaCheckedKeys"></cinemal-dialog>

      <!-- 选择商品 -->
      <selected-goods :dialogVisible.sync="selectedGoodsDialogVisible" :cinemaUid="queryData.cinemaUid" :autoClose="false" :dialogFeedbackData="selectedGoodsCheckedKeys" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack"></selected-goods>
