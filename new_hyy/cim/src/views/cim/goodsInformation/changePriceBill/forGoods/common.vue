<template>
  <div class="content" id="for-cinema-common">
    ...
    <el-form :inline="true" ref="ruleForm" :model="queryData" label-position="left" label-width="80px" label-suffix="：">
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="基础信息" name="1">
          <div>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="生效日期" class="select-input" prop="adjustTime" :rules="[{ required: routeQuery.type ==1 ? true :false, message: '生效日期不能为空',trigger: 'change' }]">
                  <span v-if="routeQuery.type!=1">
                    {{queryData.adjustTime}}
                  </span>
                  <el-date-picker v-else v-model="queryData.adjustTime" type="date" value-format="yyyy-MM-dd" :picker-options="pickerOptions" placeholder="选择日期">
                  </el-date-picker>
                </el-form-item>
              </el-col>
              <el-form-item label="调价门店" class="select-input" prop="cinemaName" :rules="[{ required: routeQuery.type ==1 ? true :false, message: '选择调价门店',trigger: 'change' }]">
        ...
        <el-collapse-item title="商品调价清单" name="2">
          <div>
            ...
            <el-table :data="queryData.list" @selection-change="handleselbtn" stripe>
              <el-table-column type="selection" v-if="routeQuery.type!=3" width="40">
              </el-table-column>
      ...
      <!-- 选择商品 -->
      <selected-goods :dialogVisible.sync="selectedGoodsDialogVisible" :cinemaUid="queryData.cinemaUid" :dialogFeedbackData="selectedGoodsCheckedKeys" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack"></selected-goods>
      <seecinemal-dialog :dialogVisible.sync="dialogVisibleseecinema" :needData="JSON.stringify(this.queryData.cinemaUids)">
      </seecinemal-dialog>
