<template>
  <div class="content serve-good">
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="基础信息" name="1">
          <div>
            ...
            <el-row>
              ...
              <div class="form-item-box left">
                <el-form-item prop="shorthandCode" label="速记代码" :rules="[{ required: routeQuery.type==3 ? false : true, message: '速记代码不能为空',trigger: 'change' }, {pattern:/^[0-9a-zA-Z_-]{6}$/,message: '请输入6位英文字母或数字!'}]">
              ...
              <div class="form-item-box left">
                <el-form-item label="品牌" class="brand">
                  ...
                  <div class="select-input" v-else>
                    <el-input class="select-input" placeholder="请选择" v-model="selectedBranchName" clearable @clear="handleDeleteBranchs" @focus="handleDialog('myBrandDialog')">
                    </el-input>
        ...
        <el-collapse-item title="销售信息" name="2">
          <div>
            <el-row>
              <div class="form-item-box left">
                <el-form-item prop="price" label="售价" :rules="[{ validator: priceValidator, trigger:'change'}]">
                  <span v-if="routeQuery.type==3">{{queryData.price}}</span>
                  <el-input class="price-inp" v-else type="number" :value="queryData.price" @input="priceInputEvent($event)"></el-input>&nbsp;元
                </el-form-item>
              </div>
              <div class="form-item-box left">
    ...
    <!-- 选择品牌弹窗 -->
    <brand-dialog ref="myBrandDialog" @onSumit="onBrandSumit" :catUid="queryData.classUid" :dialogFeedbackData="[{brandUid:queryData.brandUid,name:selectedBranchName}]"></brand-dialog>
    <!-- 选择影院弹窗 -->
    <cinemal-dialog ref="myCinemalDialog" @onSumit="onCinemalSumit" :dialogFeedbackData="queryData.cinemaEntityList" :multiple="multipleCinemal" :title="cinemalDialogTittle" :check="routeQuery.type==3 && queryData.saleCinema!=1"></cinemal-dialog>
    <!-- 选择渠道弹窗 -->
    <channel-dialog ref="myChannelDialog" @onSumit="onChanneSumit" :dialogFeedbackData="queryData.channelEntityList" :multiple="multipleCinemal" :check="routeQuery.type==3 &&  queryData.saleChannel!=1" :title="channelDialogTittle"></channel-dialog>
  </div>
</template>
