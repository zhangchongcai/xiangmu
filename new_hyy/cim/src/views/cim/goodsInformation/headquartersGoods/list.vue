<template>
  <div id="headqu-style">
    <div class="common-header newcommon-header">
      <el-form :inline="true" :model="GoodsListQueryData" label-position="left" label-suffix="：">
    ...
    <div class="common-left">
      <el-tree :data="proThreedata" :default-expanded-keys="defaultExpanded" ref="proTree" node-key="uid" :props="proThreedefaultProps" @node-click="proThreehandleNodeClick"></el-tree>
    </div>
    <div class="common-right">
      ...
      <div class="commin-new-table">
        <el-table :data="GoodsListData" stripe :height="this.defaultTableHeight">
          <el-table-column v-for="item in GoodsListDataColumn" :key="item.key" :prop="item.key" :width="item.width" :label="item.label" :formatter="item.formatter"></el-table-column>
          <el-table-column label="操作" width="240">
            <template slot-scope="{row,$index}">
              ...
              <el-button type="text" size="small" @click.stop="proStopEvent($index, row)">{{row.canSaleType == 1 ? "禁止销售":"允许销售"}}</el-button>
              <el-button type="text" size="small" @click.stop="proDelEvent($index, row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="page-wrap">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="GoodsListQueryData.page" :page-sizes="this.pageSizes" :page-size="GoodsListQueryData.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="this.total"></el-pagination>
        </div>
      </div>
    </div>
    <el-dialog class="change-dialog dialog-common-style" :title="newProtitleArr.newProtitle" :visible.sync="newProDialog" width="600px">
      <div class="newPro-box">
        ...
        <div class="newProTree">
          <el-tree :data="proThreedata" :props="proThreedefaultProps" node-key="uid" :default-expanded-keys="defaultExpanded" @row-click="addProNodeClick" @node-click="addProNodeClick">
            <span class="custom-tree-node" slot-scope="{ node, data }">
              <span v-if="data.isLeaf == 1">
                <el-radio v-model="selectRadio" :label="data.uid"></el-radio>
              </span>
...
import combBankgoods from 'cimSrc/dialogs/combBankGoods/index.vue';
  ...
  methods: {
    ...
    goodsDataQueryGoodsList () {
      ...
      this.$cimList.headquartersGoods
        ...
        .then(res => {
          if (res.code === 200) {
            this.GoodsListData = res.data.list == null ? [] : res.data.list;
    ...
    proStopEvent (index, row) {
      if (row.canSaleType == 1) {
        this.$confirm('禁止销售该商品将导致所有门店不可销售该商品，是否执行此操作', '提示', {
