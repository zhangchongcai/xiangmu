    ...
    <div class="common-header newcommon-header">
      <el-form :inline="true" :model="GoodsListQueryData" label-position="left" label-suffix="：">
        <el-form-item label="门店名称" class="select-input">
          <el-input v-model="GoodsListQueryData.cinemaName" clearable @clear="onCinemalSumit" @focus="selectCinemalDialog" placeholder="请选择门店"></el-input>
    ...
    <div class="common-right">
      <div class="commin-new-table">
        <el-table :data="GoodsListData" stripe :height="this.defaultTableHeight">
          <el-table-column v-for="item in GoodsListDataColumn" :key="item.key" :prop="item.key" :label="item.label" :formatter="item.formatter"></el-table-column>
        ...
        <div class="page-wrap">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="GoodsListQueryData.page" :page-sizes="this.pageSizes" :page-size="GoodsListQueryData.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total"></el-pagination>
        </div>
      </div>
    </div>
    <el-dialog class="change-dialog" :title="newProtitleArr.newProtitle" :visible.sync="newProDialog" width="600px">
  ...
  methods: {
    ...
    proStopEvent (index, row) {
      if (row.canSaleType == 1) {
        this.$confirm('禁止销售该商品后，将无法售卖该商品，是否执行此操作？', '提示', {
