  ...
  <el-container style="width: 100%; height: -webkit-fill-available;">
    <el-aside width="200px" style="padding-top: 30px;">
      <el-menu class="el-menu-vertical-demo">
        <template v-for="(item,index) in routerLinks">
          <el-submenu :index="index+1+''" :key="item.name" v-if="item.children && item.children.length>0">
            <template slot="title">
              <i :class="item.icon"></i>
              <span>{{item.name}}</span>
            </template>
            <el-menu-item v-for="(children,childrenIndex) in item.children" :index="childrenIndex+1+''" :key="children.name" @click="addTab(children)">{{children.name}}</el-menu-item>
          </el-submenu>
    ...
    <el-main>
      <el-tabs v-model="editableTabsValue" type="card" @tab-remove="removeTab">
        <el-tab-pane v-for="(item, index) in editableTabsArray" :key="item.name" :label="item.name" :name="item.name" :closable="item.closable">
