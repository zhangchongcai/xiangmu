  ...
  <div>
    <div class="common-header store-i-q-style">
      <el-form :inline="true" :model="queryData" label-position="left" label-suffix="：">
        ...
        <el-form-item label="仓库名称" class="select-input">
          <el-input v-model="storeHouseName" clearable placeholder="请选择仓库" @clear="onStoreHouseSumit" @focus="selectStoreHouseDialog">
          </el-input>
        ...
        <el-form-item label="货架名称" class="select-input">
          <el-input v-model="goodsName" clearable placeholder="请选择货架" @clear="ongoodsSumit" @focus="selectdoodsDialog">
          </el-input>
    ...
    <div>
      <div>
        ...
        <div class="page-wrap">
          <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryData.page" :page-sizes="this.pageSizes" :page-size="queryData.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total"></el-pagination>
        </div>
      </div>
    </div>
    <!-- 选择影院弹窗 -->
    ...
    <!-- 选择仓库弹窗 -->
    <storehouse-dialog ref="myStoreHouseDialog" @onSumit="onStoreHouseSumit" :cinemaUid="queryData" multiple :dialogFeedbackData="newstoreHouseCodeList"></storehouse-dialog>
    <!-- 选择货架弹窗 -->
...
import storehouseDialog from 'cimSrc/components/storehouseDialog/cinemaDialog.vue';
import goodsNameDialog from 'cimSrc/components/goodsNameDialog/cinemaDialog.vue';
...
export default {
  ...
  data () {
    return {
      tableLoding: false,
      cacheField: ['queryData', 'storeHouseName', 'goodsName', 'newstoreHouseCodeList', 'newRackCodeList', 'checkIsFilterZeroInventory', 'tableColumn', 'tableData'],
  ...
  methods: {
    ...
    getMerCinemaList (param) {
      ...
      if (this.queryData.storeHouseCodeList.length == 0 && this.queryData.rackCodeList.length == 0) {
        ...
        this.tableData.forEach((zongval, zongindex, zongarr) => {
          zongval.storeHouseList.forEach((newVal, newIndex, newArr) => {
            ...
            this.$set(zongarr[zongindex], sx1, newVal.storeHouseCostAmount.toFixed(4));
          });
        });
        ...
        this.tableData.forEach((zongval, zongindex, zongarr) => {
          zongval.rackList.forEach((newVal, newIndex, newArr) => {
            ...
            this.$set(zongarr[zongindex], sx, newVal.storeCount);
            this.$set(zongarr[zongindex], sx1, newVal.rackCostAmount.toFixed(4));
          });
        });
      ...
      this.tableData.forEach((sval) => {
        sval.storeCostPrice = sval.storeCostPrice.toFixed(8);
        sval.storeCostAmount = sval.storeCostAmount.toFixed(4);
      });
    }
  },
  components: {
