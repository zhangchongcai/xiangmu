    ...
    init() {
      ...
      switch (this.routeQuery.type) {
        ...
        case "2":
          ...
          this.resStoreInBillRegisterGetStoreInBillCode(JSON.parse(this.$route.query.data))
          // let newArr = []
          // let newObj = {}
      ...
      if(this.goodList.length == 0){
        ...
        this.$refs["ruleForm"].validate(valid => {
          if (valid) {
              ...
              let newAvtive = {}
              let dqActive = ""
              this.queryData.dataList.some((val,newindex,arr)=>{
                let check = /(^\d+\.[1-9]\d?$)|(^\d+\.\d[1-9]?$)|(^[1-9]\d*$)/
                if(val.storeoutCount === "" || val.storeoutCount === null || !check.test(val.storeoutCount)){
                  return newAvtive = {a1:true,a2:val}
                }
              })
              if(newAvtive.a1 === true){
                this.$message(newAvtive.a2.merName+":出库数量必须为大于0的正数");
              }else{
