  ...
  <div class="iT-style">
    <div class="content">
      ...
      <el-form :inline="true" ref="ruleForm" :model="queryData" label-position="left" label-width="100px" label-suffix="：">
          ...
          <el-collapse-item title="基础信息" name="1">
            <div>
              <el-row>
                ...
                <el-col :span="10">
                  <el-form-item label="转移门店" class="select-input" prop="cinemaName" :rules="[{ required: routeQuery.type==3 ? false : true, message: '选择转移门店',trigger: 'change' }]">
              ...
              <el-row>
                <el-col :span="10">
                  <el-form-item label="" prop="outStorehouseCode" :rules="[{ required: routeQuery.type==3 ? false : true, message: '选择转出仓库或货架',trigger: 'change' }]">
                    ...
                    <el-select v-model="queryData.outStorehouseCode" @focus="selCkORHj(queryData.outStoreType)" v-else @change="changeSelCk()">
                      <el-option v-for="item in outStorehouseArr" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :span="10">
                  <el-form-item label="">
                    ...
                    <el-select v-model="queryData.inStorehouseCode" v-else @focus="selRkORHj(queryData.inStoreType)" @change="changeSelRk()">
                      <el-option v-for="item in inStorehouseCodeArr" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
              </el-row>
          ...
          <el-collapse-item title="商品清单" name="2">
            <div>
              ...
              <el-table :data="goodList" stripe>
                <el-table-column v-for="item in routeQuery.type =='3' ? tableColumn1 : tableColumn" :key="item.key" :prop="item.key" :label="item.label" :formatter="item.formatter">
        ...
        <!-- 选择商品 -->
        <selected-goods :dialogVisible.sync="selectedGoodsDialogVisible" :dialogFeedbackData="goodList" :Qdata="queryData" @cimSelectedGoodsDialogCallBack="selectedGoodsDialogCallBack"></selected-goods>
...
import selectedGoods from 'cimSrc/dialogs/cimKcSelectedGoods/index.vue';
export default {
  mixins: [mixin],
  data () {
    return {
          ...
          if (res.code === 200) {
            if (this.routeQuery.type == '3') {
              ...
              this.goodList.forEach((val3) => {
                if (val3.costPrice != null) {
                  val3.costPrice = val3.costPrice.toFixed(8);
                } else {
                  val3.costPrice = 0;
                  val3.costPrice = val3.costPrice.toFixed(8);
                }
          ...
          if (valid) {
            ...
            if (this.goodList.length === 0) {
              ...
              if (newAvtive.a1 === true) {
                this.$message(newAvtive.a2.merName + ':转出数量必须为大于0的正数');
              } else {
