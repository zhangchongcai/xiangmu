      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="基础信息" name="1">
          <div>
            <el-row>
              <el-col :span="8">
                <el-form-item label="供应商编码" prop="code" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '供应商编码不能为空',trigger:  ['blur', 'change']  }, {pattern:/^[0-9a-zA-Z_-]{6}$/,message: '请输入6位英文字母或数字!'}]">
                  <span v-if="routeQuery.type==3">{{queryData.code}}</span>
                  <el-input v-else placeholder="请输入" prop="shorthandCode" :disabled="routeQuery.type==2  ? true: false" class="basic-input" v-model="queryData.code"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="供应商名称" prop="name" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '供应商名称不能为空',trigger: 'change' }]">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="税务登记号" prop="taxRegisterNum">
                  ...
                  <el-input placeholder="请输入" class="basic-input" v-else v-model="queryData.taxRegisterNum"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item prop="linkman" label="联系人" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '联系人不能为空',trigger: 'change' }]">
              ...
              <el-col :span="8">
                <el-form-item prop="tel" label="联系电话" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '联系电话不能为空',trigger: 'change' },{pattern:/^([0-9-]+)$/,message: '请输入数字或者-'}]">
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item prop="cooperationTime" label="合作时间" class="cooperation-time" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '合作时间不能为空',trigger: 'change' }]">
                  <span v-if="routeQuery.type==3">
                    <span v-if="queryData.startTime">{{queryData.startTime +'至' + queryData.endTime}}</span>
                  </span>
                  <el-date-picker v-else class="basic-input" v-model="queryData.cooperationTime" type="datetimerange" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" :picker-options="pickerOptions" placeholder="选择日期"></el-date-picker>
                </el-form-item>
              </el-col>
            </el-row>
          </div>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item prop="openAccountBank" label="开户行">
                  ...
                  <el-input placeholder="请输入" v-else class="basic-input" v-model="queryData.openAccountBank"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item prop="bankAccount" label="银行账号">
                  <span v-if="routeQuery.type==3">{{queryData.bankAccount}}</span>
                  <el-input placeholder="请输入" v-else class="basic-input" v-model="queryData.bankAccount"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
                <el-form-item prop="bunesRate" label="返点率">
                  ...
                  <el-input placeholder="请输入" type="number" v-else class="basic-input" v-model="queryData.bunesRate"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </div>
        ...
        <el-collapse-item title="地理信息" name="3">
          <div>
            <el-row>
              <el-col :span="8">
                <el-form-item prop="provinceId" :error="areaError" label="区域" :rules="[{ required:  routeQuery.type==3 ? false : true, message: '区域不能为空',trigger: 'change' }]">
                  <span v-if="routeQuery.type==3">{{queryData.provinceName+queryData.cityName+queryData.countyName}}</span>
                  <div v-else>
                    <el-select v-model="queryData.provinceId" @change="handleProvinceChange" placeholder="请选择省份" class="area-select">
                      <el-option v-for="item in provinceRegion" :key="item.areaCode" :label="item.areaName" :value="item.areaCode"></el-option>
                    </el-select>
                    <el-select v-model="queryData.cityId" @change="handleCityChange" placeholder="请选择市" class="area-select">
                      <el-option v-for="item in cityRegion" :key="item.areaCode" :label="item.areaName" :value="item.areaCode"></el-option>
                    </el-select>
                    <el-select v-model="queryData.countyId" placeholder="请选择县区" class="area-select">
                      <el-option v-for="item in countyRegion" :key="item.areaCode" :label="item.areaName" :value="item.areaCode"></el-option>
                    </el-select>
                  </div>
                </el-form-item>
              </el-col>
    ...
    handleSubmit () {
      ...
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          var currentStartTime = new Date(this.queryData.cooperationTime[1]).getTime();
          if (currentStartTime < nowTime) {
            this.$message({
              message: '合作结束时间须大于当前时间!'
            });
            return;
          }
          if (this.routeQuery.type == 1) {
