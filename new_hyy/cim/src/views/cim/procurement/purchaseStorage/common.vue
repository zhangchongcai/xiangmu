                        ...
                        <el-table :data="freeGoodsTableData" stripe>
                            ...
                            >
                                <template slot-scope="{row}" name="header">
                                    ...
                                    <div v-else>
                                        <div v-if="item.edit">
                                            <el-input size="small"  :value="row[item.key]"
                                                      @input="inputEvent($event,row,item.key)"  type="number" placeholder></el-input>
                                        </div>
                                        <div v-else-if="item.selsect && item.selsect(row)">
                                            <el-select v-model="row[item.key]" v-if="row.purUnitVoList"
...
<script>
    ...
    import purchaseNoteDialog from "cimSrc/components/purchaseNoteDialog/purchaseNoteDialog.vue";
                ...
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        ...
                        for(let item of param.detailVoList) {
                            ...
                            if(item.storeinCount<=0){
                                this.$message("入库数量必须大于0!");
                                flag = false
                                break;
                            }
                            if(this.routeQuery.billType == 3){
