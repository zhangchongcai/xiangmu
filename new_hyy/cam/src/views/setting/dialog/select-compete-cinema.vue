<template>
  <div class="reset-target-dialog">
    <el-dialog :visible="show" title="选择竞对影院" width="896px" :before-close="handleClose" :close-on-click-modal="false">
      <el-row class="search-section">
        <el-col :span="8">
          <span>竞对范围：</span>
          <el-select size="small" v-model="distance" @change="changeRange">
            <el-option v-for="(item,index) in distanceRange" :key="index" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-col>
        <el-col :span="7">
      ...
      <div class="select-wrap flex">
        <div class="select none-border-table">
          <el-table ref="multipleTable" height="366" :data="cinemaList" :row-key="rowKey" @clearSelection="clearSelection" @selection-change="handleSelectionChange">
          ...
          <div class="center reset-page" style="padding:11px 0;margin:0">
            <el-pagination v-if="total>15" layout="total,prev,pager,next" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="page" :page-size="pageSize" :total="total">
            </el-pagination>
