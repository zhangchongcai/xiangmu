  ...
  <div class="sale-content-wrap">
    ...
    <div class="section">
      ...
      <div class="section-select center">
        <target-label activeType="xse" :targetLabel="targetLabel" :otherLabel="otherLabel1" @getType="changeChannelType" type="channel"></target-label>
      </div>
      <!-- 饼图 -->
    ...
    <div class="section">
      <div class="section-title flex">
        <div>
          <i class="el-icon-arrow-up icon"></i>影院详情
        </div>
      ...
      <div class="section-content">
        ...
        <div class="reset-page">
          <el-pagination size="mini" v-if="total>15" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page" :page-sizes="sizes" :page-size="size" layout="total,sizes,prev, pager, next, jumper" :total="total"></el-pagination>
...
export default {
      ...
      pieExtend: {
        ...
        tooltip: {
          formatter: params => {
            ...
            let str =
              params.marker + name + '<br>' + value + '（' + percent + '%）';
            return str;
          }
        }
      },
  ...
  watch: {
    '$route': function (to, from) {
      // console.log(to,from)
      if (to.name == from.name && to.path.endsWith('area/sale/total')) {
        this.getCurrentTime();
        this.initData();
      }
    }
  ...
  methods: {
    ...
    getSaleOut () {
      ...
      if (this.cityId) {
        saleDownUrl += `&cityId=${this.cityId}`;
      }
      ...
      if (channelLine && channelLine.length > 0) {
        ...
        channelData.forEach(item => {
          if (item.name == 'pos机') {
            ...
            onlineNum = onlineNum * 1 + item.value * 1;
          }
        });
