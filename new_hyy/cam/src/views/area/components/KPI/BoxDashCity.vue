...
export default {
  ...
  methods: {
    ...
    getBoxKpiData (BoxKPIvalue) {
      // 取到时间进度
      let tool1 = BoxKPIvalue.timeRate ? BoxKPIvalue.timeRate : 0, tool2 = BoxKPIvalue.boxOfficeRate ? BoxKPIvalue.boxOfficeRate : 0;

      // 取值title
      this.BoxRate = BoxKPIvalue.boxOfficeMonthToNow;
      this.BoxKPItotal = BoxKPIvalue.boxOfficeTarget;
      // 从新复制
      this.gaugeData = {
        columns: ['name', 'value'],
        rows: [
