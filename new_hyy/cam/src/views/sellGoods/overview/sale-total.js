      ...
      lineExtend: {
        ...
        tooltip: {
          formatter: params => {
            ...
            let value =
              this.$options.filters.formatNum(params[0].value[1]) +
              this.saleUnit;
            let str =
              params[0].marker + date + '<br>' + '&nbsp&nbsp&nbsp' + value;
            return str;
          }
        }
      },
      ...
      pieExtend: {
        ...
        label: {
          formatter: params => {
            ...
            if (name == '线上' || name == '线下') {
              ...
              let value =
                this.$options.filters.formatNum(params.value) +
                this.channelUnit;
        ...
        tooltip: {
          formatter: params => {
            let name = params.name;
            let value =
              this.$options.filters.formatNum(params.value) + this.channelUnit;
            let percent = params.percent;
            let str =
              params.marker +
              name +
      ...
      barExtend: {
        ...
        tooltip: {
          formatter: params => {
            ...
            let value =
              this.$options.filters.formatNum(params[0].value) +
              this.channelUnit;
        ...
        tooltip: {
          formatter: params => {
            let str = params.map(item => {
              ...
              let unitPrice =
                '客单价:' +
                this.$options.filters.formatNum(item.value[1]) +
                '元' +
                '<br>';
              let sale =
                '销售额:' +
                this.$options.filters.formatNum(item.value[2]) +
                '元' +
                '<br>';
