    ...
    <div class="header-fixed" ref="headerFixed">
      <div class="time-wrap">
        ...
        <div class="time-item">
          ...
          <el-select v-model="commodityName" filterable remote reserve-keyword placeholder="请输入名称" :remote-method="remoteMethod" :loading="loading">
            <el-option v-for="item in optionsList" :key="item.goodSku" :label="item.goodName" :value="item.goodSku">
            </el-option>
          </el-select>
        </div>
        <div class="time-item">
            ...
            <div class="target-list">
              <div class="target-cell" v-for="(item,index) in itemList.list" :key="index">
                ...
                <div class="target-middle">
                  <i class="iconfont" :class="icons[item.name]" :style="{color:iconColors[index]}"></i>
                  <span class="money-num">{{item.value | formatMoney(item.name)}}</span>
        ...
        <div class="reset-table" v-else>
          ...
          <el-table border :data="boxTableData" height="380" class="mt10">
            ...
            <el-table-column prop="purchCount" label="采购数量(件)" min-width="110" key="purchCount">
              <template slot-scope="scope">{{scope.row.purchCount | formatNum('fixed')}}</template>
            </el-table-column>
            ...
            <el-table-column prop="startInventory" label="期初库存数量(件)" min-width="130" key="startInventory">
              <template slot-scope="scope">{{scope.row.startInventory | formatNum('fixed')}}</template>
            </el-table-column>
            <el-table-column prop="startAmount" label="期初库存成本额(元)" min-width="130" key="startAmount">
            ...
            <el-table-column prop="endInventory" label="期末库存数量(件)" min-width="130" key="endInventory">
              <template slot-scope="scope">{{scope.row.endInventory | formatNum('fixed')}}</template>
            </el-table-column>
            <el-table-column prop="endAmount" label="期末库存成本额(元)" min-width="130" key="endAmount">
