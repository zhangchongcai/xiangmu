...
export default {
  ...
  methods: {
    ...
    goDetail (row) {
      this.$router.push(
        `/analysis/common/commodity/detail?goodSku=${row.goodSku}&goodName=${row.categoryName}&startTime=${this.startTime}&endTime=${this.endTime}&timeType=${this.timeType}`
      );
    },
    // 跳转 缺货明细
    replenish () {
    ...
    getCategoryOut () {
      ...
      let downUrl = `${baseURL}/analysis/category/downloadCategoryDetail?startDate=${this.startTime}&endDate=${this.endTime}&dateType=${this.timeType}&member=${this.memberTypeVal}&token=${this.$store.state.loginToken}`;
