  ...
  <div class="sale-content-wrap">
    <div class="header-fixed">
      <div class="time-wrap clearfix">
        ...
        <div class="time-item">
          ...
          <calendar-view size="mini" v-model="time" :dateType="dateType" :dateTypeActive='dateTypeActive' @change="changeTime" @emitCalendarType="getTimeType">
          </calendar-view>
        </div>
        <div class="time-item">
          <span class="label-title">品类选择:</span>
          <el-cascader class="reset-cascader" size="small" :show-all-levels="false" placeholder="请选择" :options="categoryList" filterable clearable change-on-select @change="changeCategory"></el-cascader>
        </div>
        <div class="time-item">
          <span class="label-title">供应商:</span>
          <el-select v-model="supplierId" filterable clearable remote reserve-keyword placeholder="请输入关键词" :remote-method="getSuppliersList" @change="changeSupplier">
            <el-option v-for="item in suppliersList" :key="item.id" :label="item.name" :value="item.uid">
            </el-option>
          </el-select>
        </div>
        <div class="time-item">
      ...
      <div class="section-content" v-if="targetType == 2">
        <div class="target-flex">
          <div class="target-block" v-for="(item,index) in targetArr" :key="index">
            ...
            <div class="money">
              <i class="iconfont" :class="item.icon" :style="{color:item.color}"></i>
              <span class="money-num">{{item.amount|formatMoney(item)}}</span>
    ...
    <div class="section">
      ...
      <div class="section-content" style="padding-left:10px">
        <div v-if="isLine">
          <ve-histogram v-if="inoutType == 1" :data="inOutData1" :settings="inoutSettings" :extend="inoutExtend" :colors="colors1">
          </ve-histogram>
          <ve-histogram v-if="inoutType == 2" :data="inOutData2" :settings="inoutSettings" :extend="inoutExtend" :colors="colors1">
          </ve-histogram>
        </div>
        <div class="reset-table mt20 clearfix" v-else>
          ...
          <el-table border v-if="inoutType == 1" :data="switchData">
            ...
            <el-table-column prop="salesCount" label="销售出库数量(件)" min-width="100" align="center">
              <template slot-scope="scope">
                {{scope.row.salesCount|formatNum(0)}}
              </template>
            </el-table-column>
            <el-table-column prop="inventoryTurnoverDaysDay" label="库存数量周转天(天)" min-width="100" align='center'>
              <template slot-scope="scope">
          ...
          <el-table border v-else :data="switchData">
            <el-table-column prop="dateColumn" label="日期" min-width="120" align="center"></el-table-column>
            <el-table-column prop="purchAmount" label="采购入库金额(元)" min-width="100" align="center">
              <template slot-scope="scope">
                {{scope.row.purchAmount|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="salesAmount" label="销售出库金额(元)" min-width="100" align="center">
              <template slot-scope="scope">
                {{scope.row.salesAmount|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="inventoryTurnoverDaysAmount" label="库存金额周转天(天)" min-width="100" align="center">
              <template slot-scope="scope">
      ...
      <div class="section-title flex">
        ...
        <div>
          ...
          <template v-if="(orgType=='cinema') && currentShow">
            <el-button class="right" style="margin:0 10px" @click="goUnSale">
      ...
      <div class="section-content">
        <div class="reset-table">
          <el-table border :data="tableData">
            ...
            <el-table-column prop="goodName" label="商品名称" min-width="120">
              <template slot-scope="scope">
                <div @click="goDetail(scope.row.goodSku,scope.row.goodName)" class="text-blue cursor">
            ...
            <el-table-column prop="startInventory" label="期初库存数量(件)" min-width="140px">
              <template slot-scope="scope">
                {{scope.row.startInventory|formatNum(0)}}
              </template>
            </el-table-column>
            <el-table-column prop="startAmount" label="期初库存成本额(元)" min-width="140px">
              <template slot-scope="scope">
                {{scope.row.startAmount|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="purchAmount" label="采购数量(件)" min-width="110px">
              <template slot-scope="scope">
                {{scope.row.purchCount|formatNum(0)}}
              </template>
            </el-table-column>
            <el-table-column prop="purchCount" label="采购成本额(元)" min-width="110px">
              <template slot-scope="scope">
                {{scope.row.purchAmount|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="salesCount" label="销售数量(件)" min-width="120px">
              <template slot-scope="scope">
                {{scope.row.salesCount|formatNum(0)}}
              </template>
            </el-table-column>
            <el-table-column prop="sellingCost" label="销售成本额(元)" min-width="120px">
              <template slot-scope="scope">
                {{scope.row.sellingCost|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="endInventory" label="期末库存数量(件)" min-width="120px">
              <template slot-scope="scope">
                {{scope.row.endInventory|formatNum(0)}}
              </template>
            </el-table-column>
            <el-table-column prop="endAmount" label="期末库存成本额(元)" min-width="140px">
              <template slot-scope="scope">
                {{scope.row.endAmount|formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="inventoryTurnoverDaysDay" label="库存数量周转天(天)" min-width="140px">
              <template slot-scope="scope">
                {{scope.row.inventoryTurnoverDaysDay | formatNum}}
              </template>
            </el-table-column>
            <el-table-column prop="inventoryTurnoverDaysAmount" label="库存金额周转天(天)" min-width="140px">
              <template slot-scope="scope">
                {{scope.row.inventoryTurnoverDaysAmount | formatNum}}
              </template>
            </el-table-column>
        ...
        <div class="reset-page">
          <el-pagination size="mini" v-if="total>15" layout="total,sizes,prev, pager, next, jumper" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page" :page-sizes="sizes" :page-size="size" :total="total"></el-pagination>
...
export default {
      ...
      inoutExtend: {
        ...
        tooltip: {
          formatter: (params) => {
            ...
            let unit = '';
            params.forEach(item => {
              if (item.seriesName == '采购入库' || item.seriesName == '销售出库') {
                unit = '件';
              } else {
                unit = '天';
              }
              resStr += item.marker + item.seriesName + ':' + Global.formatNum(item.value) + unit + '<br>';
            });
            res.push(resStr);
            return res;
          }
  ...
  filters: {
    ...
    formatNum: (num, count) => {
      if (num == 99999.99) {
        // 与后端商议的无穷大的标识为：-99999.99
        return Global.formatNum('∞');
      } else {
        return Global.formatNum(num, count);
      }
    },
    // 指标里/不带单位
          ...
          if (barData && barData.yAxis) {
            ...
            this.inOutData1.rows = barData.yAxis.map(item => {
              return {
                ...
                库存周转天数: item.inventoryTurnoverDaysDay == (99999.99) ? '∞' : item.inventoryTurnoverDaysDay
              };
            });
            // 金额
            this.inOutData2.rows = barData.yAxis.map(item => {
              return {
                ...
                库存周转天数: item.inventoryTurnoverDaysAmount == (99999.99) ? '∞' : item.inventoryTurnoverDaysAmount
              };
            });
          }
        } else {
