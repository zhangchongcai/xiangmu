...
const ticket = {
  ...
  data () {
    return {
      ...
      orgName: null,
      naveList: [
        { name: '票劵使用效果分析', path: '/analysis/group/ticket/use' }
      ],
      ticketInfo: {
        // 基础信息
        name: null,
  ...
  methods: {
    ...
    init () {
      ...
      this.time = [
        this.$moment(query.startTime).format('YYYY-MM-DD'),
        this.$moment(query.endTime).format('YYYY-MM-DD')
      ];
      this.getAllData();
    },
    getAllData () {
    ...
    getOrg () {
      ...
      if (this.$route.query.type) {
        ...
        this.detailType =
          this.orgType == 'group' ? 1 : this.orgType == 'city' ? 2 : 3; // 明细列表，初始化类型
      } else {
        this.$camList.getOrgInfo({ body: {} }).then(res => {
          if (res.data) {
            ...
            this.detailType =
              this.orgType == 'group' ? 1 : this.orgType == 'city' ? 2 : 3;
          }
        });
      }
    },
    ...
    getTarget () {
      ...
      this.$camList.ticketUseTarget(params).then(response => {
        ...
        if (res) {
          ...
          this.ticketInfo.time = [
            res.ticketValidTimeStart,
            res.ticketValidTimeEnd
          ];
          // 获取时间后再获取区域列表
          this.getTableData();
        }
    ...
    goDetail (option) {
      ...
      if (type == 'city') {
        ...
        this.$router.push({
          path: '/analysis/cinema/ticket/use',
          query: {
            cinemaId: cinemaId,
            name: name,
            type: type,
