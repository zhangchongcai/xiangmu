  ...
  methods: {
    ...
    getOrg () {
      return new Promise((resolve, reject) => {
        if (this.orgName) {
          let orgType = ['group', 'area', 'cinema'].find(item =>
    ...
    getTrend () {
      ...
      // console.log(this.allTrendData,'trendData4444')
    },
    // 8.查询渠道
    getChannel () {
      let params = {
      ...
      this.$camList.memberChannelType(params).then(res => {
        ...
        //   console.log(this.allChannelData, '全部渠道5555')
      });
    },
    // 9.查询明细
    getDetail () {
    ...
    trendExport () {
      ...
      let api = `/analysis/member/downloadMemberTend?&startDate=${this.startTime}&endDate=${this.endTime}&dateType=${this.timeType}&token=${this.$store.state.loginToken}`;
    ...
    detailExport () {
      //   let baseURL = config.baseURL;
      let baseURL = this.$store.state.default.apiConfig.baseURL;
      let api = `/analysis/member/downloadRegionData?&startDate=${this.startTime}&endDate=${this.endTime}&dateType=${this.timeType}&token=${this.$store.state.loginToken}`;
      let downUrl = `${baseURL}${api}`;
      if (this.orgType == 'city') {
        if (this.cityId) {
          downUrl = `${downUrl}&cityId=${this.cityId}`;
