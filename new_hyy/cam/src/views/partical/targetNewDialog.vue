<template>
  ...
  <div class="reset-target-dialog">
    <el-dialog title="指标设置" :visible.sync="show" width="448px" :close-on-click-modal="false">
      <div class="content">
        <div class="select">
          <div class="label">选项库( {{selectList.length}} / {{total}})</div>
          <div class="detail">
            <div class="detail-item" v-for="(item,index1) in targetList" :key="index1">
              <div class="type-name">
                {{item.name}}
              </div>
              <el-checkbox-group v-model="selectList" class="type-item" :max="6">
                <el-checkbox class="item-wrap" v-for="(target) in item.list" :key="target.name" :label="target" :disabled="target.gray == 1? true:false">
