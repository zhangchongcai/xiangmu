<template>
  <div class="auto-radio-button-wrap">
    <div class="auto-radio-button" :class="{active:targetType == item.id?true:false}" v-for="(item,index) in targetLabel" :key="index" @click="changeRadio(item.id)">{{item.name}}</div>
...
export default {
  ...
  watch: {
    activeType () {
      let targetTypes = this.targetLabel.map(item => {
        return item.id;
      });
      let otherTypes = this.otherLabel.map(item => {
        return item.id;
      });
      if (targetTypes.indexOf(this.activeType) > -1) {
        this.targetType = this.activeType;
        this.otherType = null;
      } else if (otherTypes.includes(this.activeType)) {
        this.otherType = this.activeType;
