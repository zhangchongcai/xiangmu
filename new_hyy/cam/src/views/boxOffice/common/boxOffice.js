...
export default {
        ...
        tooltip: {
          formatter: params => {
            ...
            let value =
              this.$options.filters.formatNum(
                params[0].value,
                this.channelIndicatorType
              ) +
              this.$options.filters.formatTargetUnit(this.channelIndicatorType);
        ...
        tooltip: {
          formatter: params => {
            ...
            let value =
              this.$options.filters.formatNum(
                params[0].value,
                this.movieIndicatorType
              ) +
              this.$options.filters.formatTargetUnit(this.movieIndicatorType);
            let str = params[0].marker + name + '<br>' + value;
            return str;
          }
        },
  ...
  filters: {
    ...
    formatNum: (money, type, unit) => {
      if (
        ['audienceCount', 'avgPlanShowCount', 'planShowCount'].includes(type)
      ) {
  ...
  methods: {
    ...
    getTrendOut () {
      ...
      let downUrl = `${baseURL}/analysis/boxOffice/exportTrendData?userId=${this.userId}&startDate=${this.startTime}&endDate=${this.endTime}&dateType=${this.timeType}&member=${this.memberTypeVal}&token=${this.$store.state.loginToken}`;
      if (this.cinemaId) {
    ...
    getAreaOut () {
      // let baseURL = config.baseURL;
      let baseURL = this.$store.state.default.apiConfig.baseURL;
      let downUrl = `${baseURL}/analysis/boxOffice/exportRegionData?userId=${this.userId}&startDate=${this.startTime}&endDate=${this.endTime}&dateType=${this.timeType}&member=${this.memberTypeVal}&token=${this.$store.state.loginToken}`;
      if (this.cinemaId) {
        downUrl += `&brandId=${this.cinemaId}`;
      }
      window.location.href = downUrl;
    ...
    getTargetTrend () {
      ...
      this.$camList.boxOfficeTrend(params).then(res => {
        ...
        this.boxTableData = (res.data && res.data.list) || [];
      });
    },
    // 查询渠道信息
    getChannelData () {
      ...
      this.$camList.boxOfficeChannel(params).then(res => {
        ...
        this.showChannelPie = ![
          'avgTicketPrice',
          'memberBoxOfficePer'
        ].includes(this.channelIndicatorType);
      });
    },
    // 转化数据格式
    ...
    getMovieData () {
      ...
      let params = {
        body: {
          brandId: this.cinemaId,
          columnType:
            this.movieIndicatorType != 'unShowCountRate'
              ? this.transIndicator(this.movieIndicatorType)
              : 'un_plan_show_count_rate',
          dateType: this.timeType,
          startDate: this.startTime,
          endDate: this.endTime,
          pageNo: this.page,
      ...
      this.$camList.boxOfficeMovie(params).then(res => {
        ...
        this.showMoviePie = ![
          ...
          'memberBoxOfficePer',
          'marketShare'
        ].includes(this.movieIndicatorType);
      });
    },
    // 查询区域明细
    ...
    changeMemberType () {
      ...
      if (this.memberTypeVal) {
        ...
        this.otherLabel = Global.boxOfficeTrendOther.filter(
          item => item.onlyMember
        );
        this.otherLabel1 = Global.boxOfficeMovieOther.filter(
          item => item.onlyMember
        );
        this.otherLabel2 = Global.boxOfficeChannelOther.filter(
          item => item.onlyMember
        );
