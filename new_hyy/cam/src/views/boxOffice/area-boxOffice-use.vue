  ...
  <div class="boxOffice-content-wrap">
        ...
        <div class="target-flex">
          <div class="target-block" v-for="(item,index) in targetArr" :key="index" v-show="isShowTarget(item.code)">
            ...
            <div class="ratio" v-if="ratioType == 1 && !disabledChain">
              环比
              <span class="ratio-num" :class="item.valueRound*1 > 0 ? 'text-green':'text-red' ">
                <i class="iconfont" :class="[item.valueRound > 0? 'icon-neiye-shangshengjiantou':'icon-neiye-xiajiangjiantou']"></i>
            ...
            <div class="ratio" v-if="ratioType == 2 && !disabledSame">
              ...
              <span class="ratio-num" :class="item.valueSame*1 >0?'text-green':'text-red'">
                <i class="iconfont" :class="[item.valueSame > 0? 'icon-neiye-shangshengjiantou':'icon-neiye-xiajiangjiantou']"></i>
    ...
    <div class="section">
      ...
      <div class="section-select flex">
        ...
        <div>
          <target-label ref="targetLabel" activeType="boxOffice" :targetLabel="targetLabel" :otherLabel="otherLabel" @getType="changeTrendType"></target-label>
        </div>
        <div>
    ...
    <div class="section">
      ...
      <div class="flex-wraper">
        <div class="section-item">
          <target-label ref="targetLabel2" activeType="boxOffice" :targetLabel="targetLabel2" :otherLabel="otherLabel2" @getType="changeChannelType"></target-label>
          <div style="width:100%">
            <!-- <ve-pie
              :title="channelTitle"
              :data="channelData"
            ...
            <ve-ring :title="channelTitle" :settings="{roseType: 'radius'}" :data="channelData" :extend="pieExtendChannel" :colors="colors" :data-empty="channelChartEmpty" v-if="showChannelPie"></ve-ring>
            <ve-histogram :title="channelTitle1" :data="channelData" :extend="barExtend" :legend-visible="false" :data-empty="channelChartEmpty" v-else></ve-histogram>
          </div>
        </div>
        <div class="section-item">
          <target-label ref="targetLabel1" activeType="boxOffice" :targetLabel="targetLabel1" :otherLabel="otherLabel1" @getType="changeMovieType"></target-label>
          <div style="width:100%">
            <ve-ring :title="movieTitle" :settings="{roseType: 'radius'}" :data="movieData" :extend="pieExtendMovie" :colors="colors" :data-empty="movieChartEmpty" v-if="showMoviePie"></ve-ring>
            <ve-histogram :title="movieTitle1" :data="movieData" :extend="barExtend" :legend-visible="false" :data-empty="movieChartEmpty" v-else></ve-histogram>
          </div>
        </div>
      ...
      <div class="section-content">
        <div class="reset-table">
          <el-table border :data="tableData" key="tableData" @sort-change="tableSortChange">
            ...
            <el-table-column label="影院名称" min-width="120" fixed key="name">
              <template slot-scope="scope">
                <div class="cursor" @click="goDetail(scope.row.cinemaId,scope.row.name)">
