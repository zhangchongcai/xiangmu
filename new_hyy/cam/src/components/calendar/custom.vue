  ...
  <section class="calendarWarpSection">
      ...
      <div class="dayListWarp" v-if="daysList.length > 0">
        <template v-for="(item, index) in daysList">
          <template v-if="choisedDateTime.length==0">
            <div class="dayItem" :key="index" v-if="index < 42" @click="(! disabledDate(item.value)) && customClickDataItem(item)" @mouseover="divMouseOverHandle(item)">
              <span v-text="item.label" :class="{'notInMonth': !item.dayInCurrentMonth,
                            'disabled': disabledDate(item.value)
                            }"></span>
            </div>
          </template>
          <template v-else-if="choisedDateTime.length==1">
            <div class="dayItem" :key="index" v-if="index < 42" @click="(! disabledDate(item.value)) && customClickDataItem(item)" @mouseover="divMouseOverHandle(item)">
              <span v-text="item.label" :class="{'notInMonth': !item.dayInCurrentMonth , 
                                'disabled': disabledDate(item.value),
                                'selected':((choisedDateTime[0]==item.value || choisedDateTime[1]==item.value)&&item.dayInCurrentMonth)}"></span>
            </div>
          </template>
          <template v-else>
            <div class="dayItem" :key="index" v-if="index  < 42" @click="(! disabledDate(item.value)) && customClickDataItem(item)" @mouseover="divMouseOverHandle(item)" :class="{'backgroundGlay': ((item.value >= choisedDateTime[0] && item.value <= choisedDateTime[1]) || (item.value >= choisedDateTime[1] && item.value <= choisedDateTime[0]))}">
              <span v-text="item.label" :class="{'notInMonth': !item.dayInCurrentMonth , 
                            'disabled': disabledDate(item.value),
                            'selected':((choisedDateTime[0]==item.value || choisedDateTime[1]==item.value)&&item.dayInCurrentMonth)}"></span>
            </div>
    ...
    <div>
      ...
      <div class="dayListWarp" v-if="daysList.length > 0">
        <template v-for="(item, index) in daysList">
          <template v-if="choisedDateTime.length==0">
            <div class="dayItem" :key="index" v-if="index > 41" @click="(! disabledDate(item.value)) && customClickDataItem(item)" @mouseover="divMouseOverHandle(item)">
              <span v-text="item.label" :class="{'notInMonth': !item.dayInCurrentMonth,
                            'disabled': disabledDate(item.value)
                            }"></span>
            </div>
          ...
          <template v-else>
            <div class="dayItem" :key="index" v-if="index > 41" @click="(! disabledDate(item.value)) && customClickDataItem(item)" @mouseover="divMouseOverHandle(item)" :class="{'backgroundGlay': ((item.value >= choisedDateTime[0] && item.value <= choisedDateTime[1]) || (item.value >= choisedDateTime[1] && item.value <= choisedDateTime[0]))}">
              <span v-text="item.label" :class="{'notInMonth': !item.dayInCurrentMonth , 
                            'disabled': disabledDate(item.value),
                            'selected':((choisedDateTime[0]==item.value || choisedDateTime[1]==item.value)&&item.dayInCurrentMonth)}"></span>
            </div>
...
export default {
  ...
  computed: {
    ...
    disabledDate () {
      return function (val) {
        // if(!this.timeCommitted && this.choisedDateTime.length >= 1) {
        //     return (val > this.choisedDateTime[0]) || val <= (+new Date(this.choisedDateTime[0]).addDays(-60))
        // }
        // return false
        return (val > +new Date() || val <= (+new Date().addDays(-60)));
      };
    }
  },
  mounted () {
  ...
  methods: {
    ...
    customClickDataItem (item) {
      ...
      if (this.choisedDateTime.length > 1) {
        // 开始日期和结束日期不能为同一天
        if (this.choisedDateTime[0] === this.choisedDateTime[1]) {
          this.clickedLength = 1;
