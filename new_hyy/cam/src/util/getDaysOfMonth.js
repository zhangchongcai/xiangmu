...
export function getDaysListOfMonth (time) {
  ...
  let monthBegin = new Date(year, month, 1),
    ...
    monthEndTime = new Date(year, month, 1)
      .addMonths(1)
      .addDays(-1)
      .getTime();
  ...
  let daysList = [
    {
      ...
      value: _calendarEnd,
      dayInCurrentMonth:
        new Date(_calendarEnd) >= monthBeginTime &&
        new Date(_calendarEnd) <= monthEndTime,
      currendDay:
        new Date(new Date().formatDate('yyyy-MM-dd 00:00:00')).getTime() ==
        _calendarEnd
