  ...
  <div class="control">
    ...
    <el-form :inline="true" :model="formData" class="demo-form-inline search-form" size="small">
      ...
      <el-form-item>
        ...
        <el-date-picker v-model="formData.date" class="month" type="month" @change="dateChange" :clearable="false" :editable="false" placeholder="请选择">
        </el-date-picker>
    ...
    <div class="table-body clearfix">
      <div :class="['table-body-content', {'bg-holiday': index % 7 === 0 || index % 7 === 6}, {'bg-today': new Date(Date.parse(item.reportDate)).toDateString() === new Date().toDateString()}]" v-for="(item, index) in bodyData" :key="index">
        <div v-if="Date.parse(startShowDate) <= Date.parse(item.reportDate) && Date.parse(item.reportDate) < new Date().getTime() - 22*60*60*1000 && item.reportStatus != '0'" :class="['iconfont', {'icon-report-success': item.reportStatus == 3 }, {'icon-report-failed': !item.reportStatus || item.reportStatus == 2}, {'icon-reporting': item.reportStatus == 1}]"></div>
        ...
        <div @click="report(item.reportDate)" v-if="Date.parse(startShowDate) <= Date.parse(item.reportDate) && Date.parse(item.reportDate) < new Date().getTime() - 22*60*60*1000 && item.reportStatus != '0'" class="table-content-btn">重新上报</div>
        <div @click="report(item.reportDate)" v-if="Date.parse(startShowDate) <= Date.parse(item.reportDate) && Date.parse(item.reportDate) < new Date().getTime() - 22*60*60*1000 && item.reportStatus == '0'" class="table-content-btn">上报</div>
  ...
  methods: {
    ...
    getSchreportCalendar () {
      ...
      this.$ctmList.getSchreportCalendar(data).then(res => {
        console.log(res);
        if (res.code === 200) {
          this.startShowDate = res.data.startShowDate;
          res.data.calendarList.forEach(item => {
