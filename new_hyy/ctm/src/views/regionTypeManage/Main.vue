    ...
    <el-dialog :title="type === 'add' ? '新建分区类型' : '编辑分区类型'" :visible.sync="dialogVisible">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" :label-width="formLabelWidth">
        <el-form-item label="分区类型名称" prop="name">
          <el-input style="width: 200px" v-model="ruleForm.name" placeholder="请输入"></el-input>
        </el-form-item>
        <el-form-item label="可选代表颜色" prop="color">
          <div :class="[{'color-picker-border': item.isSelected}, 'color-picker']" v-for="(item, index) in colorBoxData" :key="item.color" @click="handleColorPicker(index)">
            <div :style="{
                                position: 'relative',
                                width: '20px',
                                height: '20px',
