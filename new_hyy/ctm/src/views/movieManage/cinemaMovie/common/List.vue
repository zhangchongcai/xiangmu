  ...
  <div class="cinema-stock-wrap">
    <div class="ciniema-stock-header">
      <div class="search-wrap">
        <span>影院名称：</span>
        <el-input size='small' placeholder="请输入内容" v-model="innerDataSingle.fullName">
        </el-input>
      ...
      <div class="search-wrap" style="margin-left:0px;">
        <span> 影片名称：</span>
        <el-input size='small' placeholder="请输入内容" v-model="movieName">
        </el-input>
      </div>
      <div class="search-wrap">
        <span>影片编码：</span>
        <el-input size='small' placeholder="请输入内容" v-model="movieCode">
        </el-input>
      </div>

      <div class="search-wrap">
        <span>公映日期：</span>
        <el-date-picker v-model="value7" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd">
        </el-date-picker>
      </div>
      <div class="search-wrap plan-size">
        <span>影片状态：</span>
        <el-select v-model="status" placeholder="请选择影片状态：">
          <el-option label="已下映" value='2'></el-option>
          <el-option label="上映中" value='3'></el-option>
          <el-option label="待上映" value='4'></el-option>
      ...
      <div class="button-wrap">
        <el-button type="primary" size="small" @click="getDatas('backPageOne')" class="button-reset">查询</el-button>

      </div>
      <div class="button-wrap">
        <el-button class="button-reset" @click="resetHandle" plain>重置</el-button>
    ...
    <div class="ticket-price-plan-tableNew" style="">
      <div class="creat-tickPrice" style="display:inline-block;float:right;margin-bottom:12px;margin-top:11px;">
        <el-button type="primary" size="small" plain @click="openDownloadMovie()">下载影片</el-button>
      </div>

    </div>
    <el-dialog title="下载影片" :visible.sync="dialogVisible" width="448px" :before-close="handleClose">
      <div>
        <div class="block">
          <span class="demonstration" style="margin-bottom:20px;">影片公映日期范围：</span>
          <el-date-picker v-model="downLoadMovieTime2" type="daterange" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" align="center" value-format="yyyy-MM-dd">
          </el-date-picker>
        </div>
      </div>
    ...
    <div class="cinema-stock-table">
      <el-table :data="tableData" :row-class-name="tableRowClassName" style="width: 100%">
        ...
        <el-table-column prop="movieCode" label="影片编码" width="180">
        </el-table-column>
        <el-table-column prop="movieName" label="影片名称" width="180">
        </el-table-column>
        <el-table-column prop="disVersion" label="发行版本">
        </el-table-column>
        <el-table-column prop="movieClass" label="影片分类">
        </el-table-column>
        <!-- <el-table-column
          prop="moviedesclanguage"
          label="语言"
        ...
        <el-table-column label="最低票价（元）" prop="minPrice" width="120">
        </el-table-column>
        <el-table-column prop="timeLong" label="时长（分钟）" width="100">
        </el-table-column>
        <el-table-column prop="datePublicShow" label="公映日期:" width="100">
          <template slot-scope="scope" v-if='scope.row.datePublicShow'>
            {{scope.row.datePublicShow.substr(0,10)}}
          </template>

        </el-table-column>
        <el-table-column prop="dateShowFirst" label="首映日期" width="100">
          <template slot-scope="scope" v-if='scope.row.dateShowFirst'>
            {{scope.row.dateShowFirst.substr(0,10)}}
          </template>
        </el-table-column>
        <el-table-column prop="dateShowOff" label="下映日期" width="100">
    ...
    <div class="page-wrap">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[15, 30, 45,60]" :page-size="size" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
    ...
    <frameSinglecinema :framedialogVisible="singleCinemaVisible" :whereUse="whereUse" :type="cinematype" :innerData="innerDataSingle" @callBackSingle="handleSingleCallBack" ref="frameSingleCinema">
