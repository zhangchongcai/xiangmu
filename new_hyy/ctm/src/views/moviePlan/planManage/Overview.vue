  ...
  <div>
    <el-form :inline="true" :model="formData" class="demo-form-inline search-form" size="small">
      <el-form-item label="时间范围">
        <el-date-picker v-model="date" type="daterange" @change="dateChange" value-format="yyyy-MM-dd" :clearable="false" :editable="false" unlink-panels range-separator="~" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions">
        </el-date-picker>
      </el-form-item>
      ...
      <el-form-item label="影厅">
        <el-select v-model="formData.hallUid" placeholder="请选择">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in hallUidOptions" :key="item.uid" :label="item.name" :value="item.uid">
          </el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table :data="tableData" stripe :header-cell-style="{'background': '#F2F4FD'}" style="width: 100%; margin-top: 10px">
      <el-table-column prop="movieName" label="场次名称" width="180" show-overflow-tooltip>
      </el-table-column>
      ...
      <el-table-column prop="minPrice" label="最低票价" width="180" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="priceProgramName" label="价格方案" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="address" label="票类价格">
      ...
      <el-table-column prop="addFee" label="增值服务费" show-overflow-tooltip>
      </el-table-column>
    </el-table>
...
<script>
import formatDate from 'ctmSrc/mixins/ctm/formatDate';
...
export default {
  ...
  methods: {
    ...
    search () {
      this.$ctmList.getPrizeList(this.formData).then(res => {
        console.log(res);
        if (res.code === 200) {
