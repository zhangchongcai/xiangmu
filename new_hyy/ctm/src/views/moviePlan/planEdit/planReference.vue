<template>
  <div class="plan-reference">
    <div>
      ...
      <!-- <div class="cur-vie-cinema">
                <span>当前选中: </span>
                <span class="cinema-name">{{curSelectedCinemaShow}}</span>
                <span v-if="curReCinema != curVieCinema" class="cancle-refer" @click="refSetCancel">设为参考</span>
            </div> -->
    </div>
    <div class="right-top-check">
      <div>
        <el-checkbox v-model="showGoldTimeOnly" @change="getGoldDataOrNo">只显示黄金时段场次({{goldTimeRange}})</el-checkbox>
      </div>
    </div>
    ...
    <section class="echarts-container">
      ...
      <el-radio-group v-model="curVieCinema" @change="radioChange">
        ...
        <div class="button-con" v-for="cinema in vieCinemaArr" :key="cinema.cinemaCompeteCode">
          ...
          <el-radio-button :title="cinema.cinemaName" slot="reference" :label="cinema.cinemaCompeteCode">{{cinema.cinemaName}}</el-radio-button>
        </div>

      </el-radio-group>

      <div class="echarts-con" v-loading="isLoading">
        <chart :options="bar" ref="bar" autoresize />
      </div>

    </section>

    <section>
      <el-table class="ref-table" v-loading="isLoading" :span-method="objectSpanMethod" :data="referenceTable" style="max-width: 100%;">
        <el-table-column width="112" fixed="left">
          <template slot-scope="scope">
            <div>{{tableLeftCol[Math.floor(scope.$index / 3)]}}</div>
          </template>
        </el-table-column>
        <el-table-column label="" fixed="left" width="88" prop="cinemaName">
        </el-table-column>
        <el-table-column prop="total" label="合计">
        </el-table-column>
        <!-- :min-width="item.movieName.length * 15" -->
        <el-table-column width="120" v-for="(item, index) in columnDate" :key="index" :prop="item.movieCode" :label="item.movieName">
...
// import ECharts from 'v-chart';
// import 'echarts/lib/chart/bar';
// import 'echarts/lib/component/dataZoom';
import { getRefCinema, getPlanReference } from 'ctmSrc/http/interface';
export default {
  data () {
    return {
      cinemaUid: '',
        ...
        if (res.code == 200 && res.data) {
          ...
          for (var k in table) {
            ...
            lackArr.forEach(value => {
              let pushObj = { type: value,
                total: '-',
                cinemaName: value == 'myData' ? '我的当前数据' : value == 'currentCinema' ? this.vieCinemaArr.find(item => {
  ...
  computed: {
    ...
    curSelectedCinemaShow () {
      return this.curVieCinema == '00000000' ? '全国大盘' : this.curVieCinema == '11111111' ? '竞对平均' : this.vieCinemaArr.length ? this.vieCinemaArr.filter(item => item.cinemaCompeteCode == this.curVieCinema).length ? this.vieCinemaArr.filter(item => item.cinemaCompeteCode == this.curVieCinema)[0].cinemaName : '无' : '无';
    }
  }
