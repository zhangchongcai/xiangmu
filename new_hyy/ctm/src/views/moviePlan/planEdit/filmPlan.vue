...
<script>
import {saveMoviePlan1, deletePlan, subApproves, saleOpenStop, copySinglePlan, getDecision, directApprove} from 'ctmSrc/http/interface'
        ...
        plan_list(data, old_data) {
            ...
            getDecision({
                ...
                if (res.code == 200) {
                    ...
                    res.data.planDecisionVoList.forEach(item => {
                    ...
                    this.$emit('changehallBoxOfficeVoList', res.data.hallBoxOfficeVoList)
                }
            })
    ...
    methods: {
        ...
        deleteSelectPlan(e) {
            ...
            planArray.forEach((row, rowIndex) => {
                row.forEach(plan => {
                    let deleteStatus = !plan.planStatus || plan.planStatus == 'NEW' || (plan.planStatus == 'VALID' && plan.saleStatus == 'STOP')
        ...
        goToPlanDetail(type) {
            ...
            let mode = this.mode == 1 ? 'edit': 'view'
            path += `&referer=${encodeURIComponent('/ticket/moviePlan/layout?date=' + this.baseParam.planDate + '&mode=' + mode)}`
            this.$router.push({
                    path
            })
