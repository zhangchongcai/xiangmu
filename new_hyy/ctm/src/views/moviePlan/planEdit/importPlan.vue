<template>
  <section class="import-plan">
    <el-dialog title="导入排期" :visible.sync="dialogVisible" width="892px" @close="dialogClose">
      <singleCinema ref="frameSingleCinema" :framedialogVisible.sync="singleCinemaVisible" :type="singleCinemaType" :innerData="innerData" append-to-body @callBackSingle="callBackSingle">
        <div slot="footerId">
      ...
      <el-form :inline="true" ref="ruleForm" :rules="rules" :model="formInline" class="form-inline">
        <el-form-item label="放映日期：" prop="sourceMakeDate">
          <el-date-picker v-model="formInline.sourceMakeDate" type="date" placeholder="请选择放映日期">
          </el-date-picker>
        </el-form-item>
        ...
        <div v-if="mateHallVoList.length" style="margin: 16px 85px 0">
          ...
          <div v-for="item in mateHallVoList" style="margin-top: 24px">
            ...
            <el-select style="width: 300px" v-model="item.sourceHallUid" placeholder="请选择">
              <el-option v-for="item in sourceHallUidOptions" :key="item.uid" :label="item.name" :value="item.uid">
              </el-option>
            </el-select>
