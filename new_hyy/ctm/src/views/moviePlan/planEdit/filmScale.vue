<template>
  ...
  <section>
    ...
    <div class="chaneg-scale">
      <el-tooltip class="item" effect="dark" content="时间轴设定" placement="bottom">
        <div @click="changeScaleDialogVisible = true"><i class="iconfont icon-neiye-shijianzhousheding"></i></div>
      </el-tooltip>
    </div>
    <el-dialog custom-class="changeScaleDialog" title="时间轴设置" :visible.sync="changeScaleDialogVisible" width="448px" left>
      <!-- 弹窗内容 开始-->
      <div class="flex-base">
        <div class="scale-set-div"><span>*</span><span class="scale-set-div-item">拖动精度：</span> <input type="text" placeholder="请输入" v-model="dragPrecision" title="请输入一个介于1和60之间的值"> 分钟</div>
        <div class="scale-set-div"><span>*</span><span class="scale-set-div-item">场次间隔：</span> <input type="text" placeholder="请输入" v-model="timeInterval" title="请输入一个介于1和99之间的值"> 分钟</div>
      </div>
      <!-- 弹窗内容 结束 -->
      <span slot="footer" class="dialog-footer">
        <el-button class="dialog-btn-w80" type="primary" @click="changeScale">确 定</el-button>
        <el-button class="dialog-btn-w80" @click="cancleChangeScale">取 消</el-button>
      </span>
    </el-dialog>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    changeScale () {
      ...
      if (dragPrecision >= 1 && dragPrecision <= 60 && /^\+?[1-9][0-9]*$/.test(dragPrecision)) {
        precisionFlage = true;
      }
      if (timeInterval >= 1 && timeInterval <= 99 && /^\+?[1-9][0-9]*$/.test(timeInterval)) {
    ...
    cancleChangeScale () {
      let scaleConfig = JSON.parse(JSON.stringify(this.scaleConfig));
      this.initScale(scaleConfig);
      this.changeScaleDialogVisible = false;
    },
    /* 初始化刻度尺 */
    initScale (data) {
      this.dragPrecision = data.dragPrecision;
