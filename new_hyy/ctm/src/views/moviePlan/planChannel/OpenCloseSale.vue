<template>
  <div class="open-close-sale">
    ...
    <el-dialog class="normal-dialog" title="选择渠道" @close="dialogClose" :visible.sync="dialogVisible" width="892px">
      <el-form :inline="true" :model="searchForm" :label-width="formLabelWidth" size="small">
        ...
        <el-form-item style="float: right; margin-right: 0">
          <el-button style="width: 80px; height: 32px" type="primary" @click="getBaseChannelList">查询</el-button>
        </el-form-item>
      </el-form>

      <div style="display: flex">
        <el-table :data="gridData" ref="multipleTable" height="420" @selection-change="selectedChannelChange" style="width: 70%">
    ...
    <el-form :inline="true" :model="formData" class="demo-form-inline search-form" size="small">
      <el-form-item label="影院名称：">
        <el-input style="width: 192px; margin-right: 8px" v-model="cinemaName" readonly></el-input>
        <el-button class="normal-btn" @click="$refs.frameSingleCinema.listAuthCommCinemas(), singleCinemaVisible = true">选择</el-button>
      </el-form-item>
      ...
      <el-form-item label="时间范围：">
        <el-date-picker v-model="date" type="daterange" @change="dateChange" value-format="yyyy-MM-dd" :clearable="false" :editable="false" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="开售状态：">
        <el-select v-model="formData.saleStatus" placeholder="请选择">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in saleStatusOptions" :key="item.keyCode" :label="item.keyName" :value="item.keyCode">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="影厅：">
        <el-select v-model="formData.hallUid" placeholder="请选择">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in hallUidOptions" v-if="item.status" :key="item.uid" :label="item.name" :value="item.uid">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="销售渠道：">
        <el-select v-model="formData.channelUid" placeholder="请选择">
          <el-option label="全部" value=""></el-option>
          <el-option v-for="item in channelUidOptions" :key="item.uid" :label="item.name" :value="item.uid">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="影片名称：">
        <el-input placeholder="请输入影片名称" v-model="formData.movieName" clearable>
        </el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="tableData" @selection-change="handleSelectionChange" style="width: 100%">
      ...
      <el-table-column prop="movieName" label="影片名称" width="200" show-overflow-tooltip>
        <template slot-scope="scope">
          {{`${scope.row.joinFlag ? scope.row.joinMovieName : scope.row.movieName}（${scope.row.disVersion || '--'}/${scope.row.movieDescLanguage || '--'}）`}}
        </template>
      </el-table-column>
      <el-table-column prop="showTimeStart" label="日期" width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          {{scope.row.showTimeStart.split(' ')[0]}}
        </template>
      </el-table-column>
      <el-table-column prop="showTimeStart" label="时间" width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          {{`${scope.row.showTimeStart.split(' ')[1].substring(0, 5)}-${scope.row.showTimeEnd.split(' ')[1].substring(0, 5)}`}}
        </template>
      </el-table-column>
      <el-table-column prop="hallName" label="影厅" width="180" show-overflow-tooltip>
      </el-table-column>
      <!--<el-table-column-->
      <!--prop="timeLong"-->
      <!--label="影片时长"-->
      ...
      <!--</el-table-column>-->
      <el-table-column prop="publisherRate" label="院方分账比例" width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="!scope.row.joinFlag">{{scope.row.publisherRate}}%</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column prop="priceProgramName" label="价格方案" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="!scope.row.joinFlag">{{scope.row.priceProgramName}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column prop="minPrice" label="最低票价" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="!scope.row.joinFlag">{{scope.row.minPrice}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column label="票类价格" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column label="网售价格" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column width="120" prop="priceProgramName" label="增值服务费" show-overflow-tooltip>
        <template slot-scope="scope">
          <el-popover trigger="hover" placement="bottom" v-if="(scope.row.baseTicketList && scope.row.baseTicketList.length > 0) || (scope.row.favTicketList && scope.row.favTicketList.length > 0)">
      ...
      <el-table-column label="销售渠道" show-overflow-tooltip>
        <template slot-scope="scope">
      ...
      <el-table-column prop="saleStatus" label="场次状态" show-overflow-tooltip>
      </el-table-column>
    ...
    <div class="page-wrap">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="formData.currentPage" :page-sizes="[20, 40, 60, 80]" :page-size="formData.size" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
...
import formatDate from 'ctmSrc/mixins/formatDate';
