<template>
  <div class="seatpage">
    ...
    <el-container class="seat-container-height">
      <el-main id="seatMain" style="background-color: #FFF; padding: 0;">
        ...
        <!--座位操作区-->
        <div id="scseat" class="section" @contextmenu.prevent="showMenu($event)" @mousedown="seatMousedownHandler($event)" @mousemove="seatMousemoveHandler($event)" @mouseleave="seatMouseupHandler($event)" @mouseup="seatMouseupHandler($event)">
          ...
          <table class="seatbackground">
            <tr v-for="(col,index) in messagerowArr">
              <td>
                <div class="fanzhuagn-box" v-if="index == 0" @click="rowIndexBtnEvent">
                  <svg class="icon icon-width20" aria-hidden="true">
          ...
          <!--座位区域-->
          <div class="seatarea" style="position: relative;" v-bind:style="{top: seatarea.top-this.messageRow*41 + 'px', left: seatarea.left+this.messageCol*41 + 'px', width: seatarea.width + 'px', height: seatarea.height + 'px'}">
          ...
          <!--座位-->
          <div class="seatcell" style="position: relative;" v-bind:style="{top: seatarea.top*2-this.messageRow*41 + 'px', left: seatarea.left+this.messageCol*41 + 'px', width: seatarea.width + 'px', height: seatarea.height + 'px'}">
            <table class="tdseatarea">
              <tr v-for="seatcellrow in seatCellGrid">
                <td v-for="seatcellcol in seatcellrow">
                  <div class="iconfont-text" v-model="seatlevClickActive" v-bind:class="getIcon(seatcellcol.imageType)" :sgCode="seatcellcol.sgCode">{{seatlevClickActive == false ? seatcellcol.colAlias : seatcellcol.seatLevel}}</div>
                </td>
              </tr>
            </table>
          </div>
          <!-- 画框线 -->
          <div class="drawline" v-if="drawline.drawing" v-bind:style="{left: drawline.left + 'px', top: drawline.top + 'px', width: drawline.width + 'px', height: drawline.height + 'px'}"></div>
        </div>
        <!-- {{this.messageRow}} -->
        <!-- <br> -->
        <!-- {{this.seatCellGrid}} -->
      ...
      <el-aside width="300px" style="background-color: #FFF; margin-left: 20px;position: relative;overflow: hidden;">
        <div class="h">
          ...
          <div style="margin-bottom:17px; color: #333; font-size: 14px;">设定分区</div>
          <setRegionPlan ref="setRegionPlan" :seatAreaGrid="seatAreaGrid" :seatCellGrid="seatCellGrid" @updateDrawBgData="updateDrawBgData" @updateCommonBgData="updateCommonBgData" @updateActiveType="updateActiveType" @updateSeatRegionData="updateSeatRegionData"></setRegionPlan>
          ...
          <!--            </div>-->
          <!--          </div>-->
          <div style="margin-bottom:17px;margin-top:16px; color: #333;font-size: 14px;">详情统计</div>
    ...
    <el-dialog title="设置销售区域" :visible.sync="salesArea" width="576px" :before-close="salesAreahandleClose">
      <div class="salesArea-box">
        ...
        <div class="salesArea-box-col">
          ...
          <span class="newstyle-a">
            <el-checkbox>影片开映前</el-checkbox>
          </span>
...
import setRegionPlan from 'ctmSrc/views/cinemaManage/seatInfo/SetRegionPlan';
export default {
  name: 'seatdesign',
  data () {
    ...
    this.$ctmList.ciseatLoad(data).then(data => {
      if (data && data.code === 200) {
        ...
        if (!$.isEmptyObject(this.initHallData) && $.isArray(this.initHallData.seatList)) {
          if (this.initHallData.seatList.length >= 2 && Number(this.initHallData.seatList[0].col) > Number(this.initHallData.seatList[1].col)) {
            this.turnCol = true;
          }
          if (this.initHallData.seatList.length >= 2 && Number(this.initHallData.seatList[0].row) > Number(this.initHallData.seatList[1].row)) {
            this.turnRow = true;
          }
...
/**行头、列头文本框样式**/
.seatcolheader .el-input__inner,
.seatrowheader .el-input__inner {
  height: $seatWidthHeight;
  padding: 0 8px;
  text-align: center;
  border-radius: 0;
