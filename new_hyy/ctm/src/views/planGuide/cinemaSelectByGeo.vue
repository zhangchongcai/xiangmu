<template>
  <div class="cinema-select-by-geo">
    ...
    <el-dialog custom-class="choose-cinema-by-geo" title="选择影院" :visible.sync="cinemaSelDialog" @closed="closeCb" width="890px">
      <div class="filter-tool">
        <div class="filter-options">
          <span>省市:</span>
          <el-select v-model="provinceCodeValue" placeholder="请选择" @change="provinceChange" :disabled="showType == 'view'">
            <el-option v-for="item in provinceData" :key="item.provinceCode" :label="item.provinceName" :value="item.provinceCode">
            </el-option>
          </el-select>
          <el-select v-model="cityCodeValue" placeholder="请选择" :disabled="showType == 'view'">
            <el-option v-for="item in cityData" :key="item.cityCode" :label="item.cityName" :value="item.cityCode">
            </el-option>
          </el-select>
        </div>
...
<script>
import { getCinemaAreaList, getAreaInfo } from 'ctmSrc/http/interface';
