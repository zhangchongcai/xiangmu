          ...
          <el-collapse-item title="触发条件" name="2">
            ...
            <!-- <el-form-item label="适用影院：">
              ...
              <span
                ...
                :key="index"
              >{{item.cinemaName}}{{item.name}}</span>
            </el-form-item>-->
            <el-form-item label="适用影院：">
            ...
            <el-form-item label="适用影厅：">
              ...
              <el-table :data="cinemaTypeList" v-if="applicableFilm1==1">
                ...
                <el-table-column prop="type">
                  ...
                  <template slot-scope="scope">
                    <el-select placeholder="更多" v-model="scope.row.code" multiple>
                      <el-option v-for="item in scope.row.hallList" :key="item.hallUid" :label="item.name" :value="item.hallUid">
                      </el-option>
                    </el-select>
                    <!-- <el-tag
                      class="m-l-5"
          ...
          <el-collapse-item title="价格体系" name="3">
            <el-tabs v-model="activeTab" type="card" @tab-click="handleClick">
              <el-tab-pane label="柜台/自助" name="first">
                ...
                <el-form-item label="适用渠道：" style="margin-bottom: 0;" prop="price_Channel_1">
                  ...
                  >
                    <el-checkbox
                      v-for="(chanel,index) in channelsList_1"
                      :key="index"
                      :label="chanel.uid"
            ...
            <div class="table-bottom-menu">
              <el-form-item label="分区定价：" class="m-t-10">
                ...
                <el-table :data="tableData" style="width: 302px;margin-top:10px" v-if="pricing == 1">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <el-input
                        ...
                        placeholder="0"
                        @change="changeNumer1(item.attendanceRate1,index)"
                      ></el-input>
                      ...
                      <el-input
                        ...
                        @change="changeTime(item.opening,index)"
                      ></el-input>
...
    },
    ...
    chooseCinema1() {
      ...
      this.$refs.chooseCinema1.title = "影院名称"; //  关联代码 getCinema
      ...
      this.$ctmList
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            this.cinemaTypeList.forEach(item =>{
              var a = [];
              item.hallList.forEach(i =>{
                a.push(i.hallUid)
              })
              this.$set(item,"code",a)
            })
            if(this.moren.length>0){
              var arr = [];
              this.moren.forEach(item =>{
                var a = [];
                item.hallList.forEach(i =>{
                  a.push(i.hallUid)
                })
              ...
              this.cinemaTypeList.forEach(item =>{
                this.moren.forEach(i =>{
                  if(item.cinemaUid == i.cinemaUid){
                    item.code = i.code
                  }
                })
              })
    ...
    changeNumer1(val, idx) {
      if(this.activeList[idx].attendanceRate){
          if(val*1 <= this.activeList[idx].attendanceRate*1){
              ...
              if(this.activeList[idx*1+1]){
                this.activeList[idx*1+1].attendanceRate =null
              }
              return;
          }
      }
      ...
      if(this.activeList[idx*1+1]){
        this.activeList[idx*1+1].attendanceRate = newVal.toFixed(1);
        if(this.activeList[idx*1+1].attendanceRate>=this.activeList[idx*1+1].attendanceRate1){
          this.activeList[idx*1+1].attendanceRate1 =null
        }
      }
      this.active=''
    ...
    addRuler() {
      ...
      var x = true;
      if(this.ifCheck[0]==0){
        if(this.activeList[a*1-1].attendanceRate == null){
          this.activ = a*1-1
          if(a*1-1==0){
            this.activ = '0'
          }
        ...
        if(this.activeList[a*1-1].attendanceRate1 == null){
          this.active = a*1-1
          if(a*1-1==0){
            this.active = '0'
          }
      ...
      if(this.ifCheck[0]==1||this.ifCheck[1]==1){
        if(this.activeList[a*1-1].opening == null){
          this.active2 = a*1-1
          if(a*1-1==0){
            this.active2 = '0'
          }
      ...
      if(x){
        this.$set(this.activeList, a, {
          idx: a,
          attendanceRate: a!=0?this.activeList[a*1-1].attendanceRate1:null,
          attendanceRate1: null,
          opening: null,
          type: 0
        });
    ...
    handleCheckAllChange(val) {
      if (val) {
        ...
        this.ttVoList_Fun();
        this.ttVoList.forEach(item =>{
          item.feeFlag = this.ttVoList[0].feeFlag
        })
      }
    },
    handleCheckAllChange1(val) {
      if (val) {
        ...
        this.priceNetSale.forEach(item =>{
          item.feeFlag = this.priceNetSale[0].feeFlag
        })
      }
    },
    //=========================================================================
    ...
    handleSelectionTtVoListChange(val) {
      ...
      for (var i = 0; i < val.length; i++) {
        if (!obj[val[i].movieVersion]) {
          result.push(val[i]);
          obj[val[i].movieVersion] = true;
        }
      }
      self.formTtVoList = result;
    },
    ...
    ttVoList_Fun() {
      let self = this;
      let resultArr = [];
      var oldArr = self.ttVoList;
      var arr = this.price_tickettype;
      ...
      self.ttVoList.forEach(item=>{
        oldArr.forEach(i =>{
          if(item.movieVersionName == i.movieVersionName&&item.ttName==i.ttName){
            ...
            this.$set(item,'feeFlag',i.feeFlag)
          }
        })
      })
      console.log(oldArr)
    ...
    priceChange1(val, index) {
        ...
        if (val==null||val=="") {
            this.ttVoList[index].price = "0.00";
            this.$message({
                message: "不得为空",
                type: "warning"
            });
    ...
    priceChange2(val, index) {
        ...
        if (val==null||val=="") {
            this.priceNetSale[index].price = "0.00";
            this.$message({
                message: "不得为空",
                type: "warning"
            });
    ...
    getMovieList(current = 1,name) {
      console.log(this.disVersion_checkAll_1, this.disVersion_checkAll_2);
      this.disVersion_isIndeterminate_1 = false;
      this.disVersion_isIndeterminate_2 = false;
      this.price_disVersion_1 = [];
      this.price_disVersion_2 = [];
    ...
    priceprogramSave(rulesSizeForm) {
      ...
      this.$refs[rulesSizeForm].validate(valid => {
        if (valid) {
          ...
          //影厅类型数据处理
          var cinemaTypeList = this.cinemaTypeList;
          cinemaTypeList.forEach(item => {
            item.hallList = item.hallList.filter(i =>{
              for(var j=0;j<item.code.length;j++){
                if(i.hallUid == item.code[j]){
                  return i.hallUid
                }
              }
            })
          });
          var ciPriceHall = cinemaTypeList;
          // var ciPriceHall = this.cinemaTypeList;
          // if (this.ciPriceHall.length > 0) {
          //   ciPriceHall.forEach(item => {
          //     this.ciPriceHall.forEach(i => {
          //       if (item.cinemaUid == i.cinemaUid) {
          //         item.hallList.forEach(z => {
          //           i.hallList.forEach(x => {
          //             if (z.hallUid == x.hallUid) {
          //               this.$set(z, "uid", x.uid);
          //             }
          //           });
          //         });
          //       }
          ...
          this.tableData.forEach((item, index) => {
            ...
            index.regionTypeUid = item.regionTypeUid;
            index.uid = item.uid;
          ...
          console.log(this.applicableFilm1);
          ...
          if (this.applicableFilm1 == 1) {
            var useHall = 0;
          } else if (this.applicableFilm1 == 0) {
            var useHall = 1;
          }
          ...
          ciPriceProgram.uid = self.$route.query.id;
          ciPriceProgram.code = this.code;
          ...
          console.log(self.ttVoList,self.formPriceNetSale)
          let params = {
    ...
    getDatas() {
      ...
      self.$ctmList
        ...
        .then(res => {
          ...
          if (res.code == 200) {
            this.sizeForm.ciPriceProgram = res.data;
            res.data.startDy = res.data.startDy.slice(0,10)
            this.$set(this.sizeForm.ciPriceProgram,'tiemStart_end',[res.data.startDy,res.data.endDy])
            this.dateClick = true;
            this.code = res.data.code;
            this.pricing = res.data.useRegionPrice;
            this.pricing1 = res.data.useAdjustmentPrice;
            if (res.data.useMovieLifeCycle == 1) {
              this.pricing2 = 0;
            } else if (res.data.useMovieLifeCycle == 0) {
              this.pricing2 = 1;
              // this.lifeCyclelist = [];
              var a = res.data.movieLifeCycleStr.split(",");
            ...
            if (res.data.priceAdjustmentList.length > 0) {
              ...
              this.activeList.forEach(item => {
                ...
                this.$set(item, "attendanceRate", item.occupancyMin);
                item.attendanceRate = item.attendanceRate.toFixed(1)
                this.$set(item, "attendanceRate1", item.occupancyMax);
                item.attendanceRate1 = item.attendanceRate1.toFixed(1)
                // this.$set(item, "uid", item.uid);
                if(item.price<0){
                ...
                this.$set(item, "opening", item.time);
                item.opening = item.opening.toFixed(1)
              });
            }

            this.sizeForm.ciCinemaList = res.data.ciCinemaList;
            if (res.data.priceRegionList.length > 0) {
              var data = res.data.priceRegionList;
              data.forEach(item =>{
                this.tableData.forEach(i =>{
                  if(item.regionTypeUid == i.uid){
                    this.$set(item,'name',i.name);
                    this.$set(item,'color',i.color);
                  }
                })
              })
              this.tableData = data;
            }else if(res.data.priceRegionList.length == 0){
              this.tableData.forEach(item =>{
            ...
            //处理影院Uid
            if (res.data.useCinema == 1) {
            } else if (res.data.useCinema == 0) {
              this.$refs.chooseCinema1.zx = 1;
              this.$refs.chooseCinema1.cList = res.data.ciCinemaList;
              this.chooseCinemas1 = "";
              res.data.ciCinemaList.forEach(item => {
                this.chooseCinemas1 += item.cinemaName + ",";
              });
              this.chooseCinemas1 = this.chooseCinemas1.slice(
                0,
            ...
            let UidArr = [];
            this.sizeForm.ciCinemaList.forEach((item, index) => {
              UidArr.push(item.cinemaUid);
            });
            this.cinemaUids = UidArr.join(",");
            self.sizeForm.ciCinemaList.forEach(item => {
              var a = item.cinemaUid;
              item.cinemaUid = item.uid;
              item.uid = a;
            });

            this.useMovie = res.data.useMovie;
            this.isSome = res.data.useMovie == "1" ? false : true;
            this.sizeForm.schBashMovieList = res.data.movieList;
            this.chooseCinemas = "";
            res.data.movieList.forEach(item => {
              this.chooseCinemas += item.name + ",";
            });
            this.chooseCinemas = this.chooseCinemas.slice(
              0,
              this.chooseCinemas.length - 1
            );
            if (res.data.useMovie == 2) {
              var uids = "";
              UidArr.forEach(item => {
                uids += item + ",";
              });
              ...
              this.$ctmList
                ...
                .then(ret => {
                  this.$refs.chooseCinema.gridData = ret.data.records;
                  this.$refs.chooseCinema.pageData = ret.data;
                  this.$refs.chooseCinema.zx = 1;
                  this.$refs.chooseCinema.cList = res.data.movieList;
                })
                .catch(function(error) {
                  console.log(error);
                });
            }
            this.getTickettypeList_1();

            if (res.data.ciPriceHallType.length > 0) {
              self.sizeForm.ciPriceProgram.ciPriceHallType = res.data.ciPriceHallType.map(
                (item, index) => {
                  return item.type;
                }
              );
              // res.data.ciPriceHallType.forEach(item => {
              //   this.$set(self.sizeForm.schBashHallTypeList, "keyCode", item);
              // });
              var length = res.data.ciPriceHallType.length;
              var arr = [];
              for (let index = 0; index < length; index++) {
                arr[index] = {
                  keyCode: res.data.ciPriceHallType[index]
                };
              }
              self.sizeForm.schBashHallTypeList = arr;
              console.log(self.sizeForm.schBashHallTypeList)
            } else {
              self.sizeForm.schBashHallTypeList = [];
              self.sizeForm.ciPriceProgram.ciPriceHallType = ["limit"];
            }
            if (res.data.ciPriceHall) {
              this.cinemaTypeList = res.data.ciPriceHall;
              this.ciPriceHall = res.data.ciPriceHall;
            }
            //处理影厅数据
            if (res.data.useHall == 0) {
              this.applicableFilm1 = 1;
              this.moren = res.data.ciPriceHall
              this.cinemaLists()
              // data.forEach(item =>{
              //   var a = [];
              //   item.hallList.forEach(i =>{
            ...
            } else if (res.data.useHall == 1) {
              this.applicableFilm1 = 0;
            }
            ...
            if (result.ciPriceSaleChannelVoList.length > 0) {
              result.ciPriceSaleChannelVoList.forEach((item, index) => {
                if (item.channelNature == 0) {
                  self.price_Channel_1.push(item.channelUid);
                  self.ciPriceSaleChannelVoList_1.push(item);
                } else if (item.channelNature == 2) {
                  self.price_Channel_2.push(item.channelUid);
            ...
            var val = [];
            var val1 = [];
            res.data.ciPriceSaleChannelVoList.forEach(item => {
              if (item.channelNature == 0) {
                val.push(item.channelCode);
              }
              if (item.channelNature == 1) {
                val.push(item.channelCode);
              }
              if (item.channelNature == 2) {
                val1.push(item.channelCode);
              }
            });
            this.handleCheckedChannel_change_1(val);
            this.handleCheckedChannel_change_2(val1);
            if (result.ciPriceSaleChannelVoList.length > 0) {
              if (result.ciPriceSaleChannelVoList[0].type == 2) {
                this.activeTab = "second";
              }
              res.data.ciPriceSaleChannelVoList.forEach(item => {
                if ((item.channelNature = 0 || 1)) {
                }
              });
              // ciPriceSaleChannelVoList_1
              var ciPriceSaleChannelVoList = res.data.ciPriceSaleChannelVoList;
              ciPriceSaleChannelVoList.forEach(item => {
                this.$set(item, "channelUid", item.channelCode);
              });
            }

            // result.ciPriceSaleChannelVoList.forEach(item => {
            //   this.price_Channel_1.push(item.channelCode);
            //   this.price_Channel_2.push(item.channelCode);
            // });
            //处理影片版本和适用票类---线上
            let resultItempriceNetSale = [];
            result.priceNetSale.forEach((item, index) => {
              // console.log('item99999999999999999999',item)
              resultItempriceNetSale.push(
                item.movieVersion + "," + item.movieVersionName
              );
            });
            if (resultItempriceNetSale.length > 0) {
              self.disVersion_checkAll_2 =
                resultItempriceNetSale.length == self.disVersionList.length;
              self.disVersion_isIndeterminate_2 =
                resultItempriceNetSale.length > 0 &&
                resultItempriceNetSale.length < self.disVersionList.length;
            }
            ...
            if (resultTtVoList.length > 0) {
              self.disVersion_checkAll_1 =
                resultTtVoList.length == self.disVersionList.length;
              self.disVersion_isIndeterminate_1 =
                resultTtVoList.length > 0 &&
                resultTtVoList.length < self.disVersionList.length;
            }
          ...
          //   console.log(result);
          //   this.useMovie =
          //     (result.ciPriceProgram && result.ciPriceProgram.useMovie) || "";
          ...
          //   self.getMovieList();
          //   self.sizeForm.ciPriceProgram.tiemStart_end = result.ciPriceProgram
          //     .startDy
          //     ? [result.ciPriceProgram.startDy, result.ciPriceProgram.endDy]
          //     : [];
          //   //时间段处理
          //   self.sizeForm.tmList = result.tmList.map((item, index) => {
          //     // let item2 ={}
          ...
          //   //影片量处理
          //   if (arr.length > 0) {
          //     self.sizeForm.ciPriceProgram.movieCodeList = result.schBashMovieList.map(
          //       (item, index) => {
          ...
          //     self.sizeForm.ciPriceProgram.ciPriceHallType = result.ciPriceHallType.map(
          //       (item, index) => {
          //         return item.keyCode;
          //       }
          //     );
          ...
          //         console.log(
          //           " self.走走组左走走走走走2 ",
          //           self.price_Channel_2
          //         );
          //       }
          //     });
          //   }
          //   console.log(result.ciPriceSaleChannelVoList);
          //   if (result.ciPriceSaleChannelVoList[0].type == 2) {
          //     this.activeTab = "second";
          //   }
          //   //处理影片版本和适用票类---线上
          ...
          //   console.log(
          //     "result.ciPriceProgram.status",
          //     self.sizeForm.ciPriceProgram.status
          //   );
          // }
        })
        .catch(err => {
