          ...
          <div class="basic-info">
            <el-form ref="form" :model="sizeForm" label-width="98px" size="small">
              ...
              <el-form-item label="有效日期：">
                <span class="cinema-stock-scan-text" v-if="data.startDy && data.endDy">{{data.startDy.split(" ")[0]}} 至 {{data.endDy.split(" ")[0]}}</span>
              </el-form-item>
            </el-form>
          </div>
        </el-collapse-item>
            ...
            <el-form ref="form" :model="sizeForm" label-width="98px" size="small">
                ...
                <el-table :data="tableData" v-if="tableData.length>0">
                  ...
                  <el-table-column prop="hallList" label="适用影厅">
                    <template slot-scope="scope">
                      <el-tag class="m-l-5" v-for="(tag) in scope.row.hallList" :key="tag.code" type="info">{{tag.name}}</el-tag>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-form-item label="适用时段：">
                <div v-for=" (item,index) in data.tmList" :key="index">
                  ...
                  <span class="cinema-stock-scan-text cinema-stock-scan-distant">{{item.startTm}} ~ {{item.endTm}}</span>
                </div>
              </el-form-item>
            </el-form>
          </div>
            ...
            <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
              <el-tab-pane label="柜台/自助" name="first">
                <el-form ref="form" :model="sizeForm" label-width="85px" size="small">
                  <el-form-item label="适用渠道：">
                    <template v-for="(ciPriceSaleChannelVoItem,index) in data.ciPriceSaleChannelVoList">
                      <span class="cinema-stock-scan-text" :key="index" v-if="ciPriceSaleChannelVoItem.channelNature == 0||ciPriceSaleChannelVoItem.channelNature == 1">{{ciPriceSaleChannelVoItem.channelName}}</span>
                    </template>
                  </el-form-item>
              ...
              <el-tab-pane label="线上网售" name="second">
                <el-form ref="form" :model="sizeForm" label-width="85px" size="small">
                  <el-form-item label="适用渠道：">
                    <template v-for="(ciPriceSaleChannelVoItem,index) in data.ciPriceSaleChannelVoList">
                      <span class="cinema-stock-scan-text" :key="index" v-if="ciPriceSaleChannelVoItem.channelNature == 2">{{ciPriceSaleChannelVoItem.channelName}}</span>
                    </template>
                  </el-form-item>
                  <el-form-item label="定价方案：">
              ...
              <el-form-item label="动态调价：">
                <div class="cinema-stock-scan-text" v-if="data.useAdjustmentPrice == 1">
                  <span v-for="(item,index) in data.priceAdjustmentList" :key="index">
                    <span v-if="item.occupancyMin||item.occupancyMax">{{item.occupancyMin}}% ≤ 上座率 ＜ {{item.occupancyMax}}%</span>
