...
<template>
  <el-form-item v-bind="itemProps(mItem)" :class="['my-form-item', {'is-required': !noVerify && mItem.verify && mItem.verify.canBeEmpty === undefined}, mItem.col ? 'col-' + mItem.col : '', mItem.prop]" v-loading="loading">
    <!--选择框-->
    ...
    <!--日期，范围-->
    <el-date-picker v-model="model[mItem.prop]" v-bind="mItem.props" :type="mItem.type" v-else-if="['date', 'datetime', 'daterange', 'datetimerange'].includes(mItem.type)" :value-format="datePickerFarmatter(mItem)" v-on="$listeners"></el-date-picker>
    <!--时间选择-->
    <el-time-select v-model="model[mItem.prop]" v-bind="mItem.props" v-else-if="mItem.type=== 'timeselect'" v-on="$listeners">
    </el-time-select>
    <!--时间、范围-->
    <el-time-picker v-model="model[mItem.prop]" v-bind="mItem.props" :is-range="mItem.type === 'timerange'" v-else-if="['time', 'timerange'].includes(mItem.type)" :value-format="mItem.props && mItem.props.valueFormat ? mItem.props.valueFormat : 'HH:mm:ss'" v-on="$listeners">
    </el-time-picker>
    <!--开关-->
    ...
    <!--多选-->
    <cc-checkbox-group v-model="model[mItem.prop]" v-bind="mItem.props" :options="mItem.options" :type="mItem.type" v-else-if="['checkbox', 'checkboxbutton'].includes(mItem.type)" v-on="$listeners"></cc-checkbox-group>
    <!--级联选择器-->
    <template v-else-if="mItem.type === 'cascader'">
      <el-cascader @change="val => cascaderChange(val, mItem)" @active-item-change="val=>handleItemChange(val,mItem)" v-model="model[mItem.prop]" :options="typeof mItem.options === 'string' ? [] : mItem.options" v-bind="mItem.props" v-if="mItem.options" v-on="$listeners" />
    </template>
    <!--计数器-->
    ...
    <template v-else-if="mItem.type === 'divider-cascader'">
      <cc-select v-model="model[mItem.prop]" v-bind="mItem.props" :options="mItem.options" v-if="mItem.type === 'divider-cascader'" v-on="$listeners" @change="val => selectAreaChange(val, mItem)"></cc-select>
      <cc-select v-model="model[mItem.prop1]" v-bind="mItem.props" :options="mItem.options1" v-if="mItem.type === 'divider-cascader'" v-on="$listeners" @change="val => selectChange(val, mItem)"></cc-select>
    </template>
    <template v-else>
      ...
      <el-input v-model="model[mItem.prop]" @click.native="$emit('click')" v-bind="mItem.props" :type="mItem.type" v-else clearable v-on="$listeners"></el-input>
    </template>
  </el-form-item>
</template>
...
// fetchProvinces, fetchAreaByCode,
import { queryAreaByParent } from '../../http/interface';
