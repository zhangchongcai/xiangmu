...
<template>
  <div>
    <el-popover ref="popover" placement="bottom" trigger="click" v-model="show">
      <dy-table :rows="options.rows" :columns="options.columns" v-if="forceUpdate" :row-key="valueField" v-bind="$attrs" :multiSelect="multiSelect" :selected-rows.sync="selectedRows" :height="height" :current-row.sync="currentRow">
      </dy-table>
    </el-popover>
