...
<script>
import { getMoudelData } from "ccmSrc/http/interface.js"
export default {
  props: {
    dialogTableVisible: {
      type: Boolean,
  ...
  methods: {
    ...
    getCinemaTypeList () {
      ...
      getMoudelData(params.page, params.pageSize, params.name).then(res => {
        if (res.data && res.code == 200) {
          ...
          this.pageData.total = res.data.list.length || 0
      ...
      if (selectedArr.length != 0) {
        for (let i = 0; i < selectedArr.length; i++) {
          for (let k = 0; k < this.tableList.length; k++) {
            if (selectedArr[i].keyCode == this.tableList[k].keyCode) {
    ...
    rowOneToggle (row) {
      for (let index = 0; index < this.chooseItem.length; index++) {
        if (row.keyCode == this.chooseItem[index].keyCode) {
    ...
    delateSpan (item) {
      ...
      if (item) {
        ...
        for (let i = this.chooseItem.length - 1; i >= 0; i--) {
          if (this.chooseItem[i].keyCode == item.keyCode) {
