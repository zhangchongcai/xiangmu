...
util.printTicket = function (type, printData, configData, callback) {
  ...
  if (
    !type &&
    (type != 'film_print' || type != 'bill_print' || type != 'coupon_print')
  ) {
    callback('打票类型有误');
    return;
  }
  ...
  if (type == 'film_print') {
    ...
    if (printData.ticket_element && printData.ticket_element.length > 0) {
      for (var item in printData.ticket_element) {
        printData.ticket_element[item].x =
          printData.ticket_element[item].x * zoomX + parseInt(x_w);
        printData.ticket_element[item].y =
          printData.ticket_element[item].y * zoomY + parseInt(y_w);
      }
    }
  }
  if (!configData) {
  ...
  // 向外壳发送数据
  app.sendMessage('Ticket_Print_REQUEST', [
    callbackName,
    newPrintData,
    printCode
  ]);
    ...
    getDevice(0, function (device) {
      ...
      if (device == null) {
        ...
        if (!is_reading_member) {
          ...
          app.sendMessage('Card_ReadSN_REQUEST', [
            callbackname,
            device.code.split('&')[0]
          ]);
        }
    ...
    getDevice(0, function (device) {
      ...
      if (device == null) {
        ...
        if (!is_reading_member) {
          ...
          app.sendMessage('Card_ReadNum_REQUEST', [
            callbackname,
            device.code.split('&')[1],
            device.pwd
          ]);
...
util.writeCard = function (configData, callBack) {
  ...
  // 向外壳传送数据
  app.sendMessage('Card_WriteNum_REQUEST', [
    callbackName,
    readCardCode,
    pwd,
    cardNum,
...
util.resetCardPwd = function (configData, callBack) {
  ...
  let readCardCode = getPrintCode(configData.card_reader); // 读卡器打印码
  // 向外壳传送数据
  app.sendMessage('Common_ChangeCardPwd_REQUEST', [
    callbackName,
    readCardCode,
    initPwd,
    pwd
    ...
    if (type == 0 || type == 2) {
      ...
      if (memberCardKindCodeObj == null) {
        ...
        getDeviceObject(
          type,
          result,
          memberCardKindCodeObj.result.cardAuthPwd,
          null,
          callback
        );
      }
  ...
  if (typeof app !== 'undefined' && app.setMessageCallback) {
    ...
    result = {
      ...
      bill_print: [
        ...
        { code: 'Fax', name: 'Fax', selected: false }
      ],
      card_reader: [
        { code: 'S8', name: '峰华_S8', selected: false },
        { code: 'E7', name: '峰华_E7', selected: true },
        { code: 'Z9', name: '德卡_Z9', selected: false },
        { code: 'QM8', name: '德卡_QM8', selected: false },
        { code: 'QM8_OSK', name: '德卡_QM8_OSK', selected: false },
        { code: 'FKSF2', name: '方卡_SF2', selected: false },
        { code: 'FKSF2C', name: '方卡_SF2C', selected: false },
        { code: 'TYRM', name: '明华_R330', selected: false },
        { code: 'FHJY', name: '德生F3RF', selected: false },
        {
          code: 'FHJY&GWI-60BR',
          name: '德生F3RF+长城GWI-60BR',
          selected: false
        },
        { code: 'SENOR', name: '星乔读卡器', selected: false }
      ],
      cash_box: [
        {
          code: 'Send To OneNote 2016',
        ...
        { code: 'Fax', name: 'Fax', selected: false },
        { code: 'COM3', name: 'COM3', selected: true }
      ],
      coupon_print: [
        {
          code: 'Send To OneNote 2016',
      ...
      keyboard: [
        { code: 'E7', name: '峰华_E7', selected: false },
        { code: 'Z9', name: '德卡_Z9', selected: false },
        { code: 'CM511', name: '辰明_CM511', selected: false },
        { code: 'CM512', name: '辰明_CM512', selected: false },
        { code: 'Numeric Keypad', name: '标准键盘', selected: true }
      ],
      counter_type: [
        { code: 'movie', selected: false },
        { code: 'goods', selected: false },
        { code: 'both', selected: true }
      ],
      meal_preparation: [
        { code: '0', selected: false },
        { code: '1', selected: true }
      ],
      pos_show_type: [
        { code: ' ', name: '无客显', selected: true },
        { code: 'ZHONGQI', name: '中崎', selected: false },
        { code: 'XINGQIAO', name: '星乔', selected: false }
      ],
      pos_show_type: [
        { code: 'ZHONGQI', name: '中崎', selected: false },
        { code: 'XINGQIAO', name: '星乔', selected: false }
      ...
      pos_type: [
        { code: '2', name: '不对接', selected: true },
        { code: '0', name: '快钱POS机', selected: false },
        { code: '1', name: '杉德POS机', selected: false }
      ],
      print_voucher: [
        { code: '0', selected: false },
        { code: '1', selected: true }
...
util.writeTerminalParameter = function (json, callback) {
  ...
  app.sendMessage('Common_DataWrite_REQUEST', [
    callbackname,
    type,
    requestdata
  ]);
