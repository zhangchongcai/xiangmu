<template>
  <div>
    <div v-if="model.pageName == 'saleList'">
      <el-form :model="form" :label-width="'100px'" ref="form">
        <el-collapse v-model="activeNames" ref="el-collapse-item">
          <el-collapse-item title="票劵基本资料" name="baseInfo">
            <el-form-item label="票券名称：" prop="baseInfo.name.value" :rules="[{required:true,validator:checkName}]">
            ...
            <el-form-item label="票券数量：" prop="baseInfo.couponCount.value" :rules="[{required:true,validator:checkCouponCount}]">
              <span v-if="model.type=='revise'" class="font-666">{{form.baseInfo.couponCount.value}}</span>
              <div v-else>
                <el-input v-model="form.baseInfo.couponCount.value" class="input-type-94" @change="couponCountChange">
                </el-input> <span class="tips-font">(不大于100,000)</span>
              </div>
            </el-form-item>
            <el-form-item label="销售单价：" prop="baseInfo.couponPrice.value" :rules="[{required:true,validator:checkCouponPrice}]">
            ...
            <el-form-item label="起售数量：">
              <el-form-item prop="baseInfo.startSaleNum.value" :rules="[{required:false,validator:checkStartSaleNum}]">
            ...
            <template v-if="form.baseInfo.salesMode.value == '3' || form.baseInfo.salesMode.value == '4'">
              <el-form-item label="客商名称：" prop="baseInfo.custId.value" :rules="form.baseInfo.custId.rules">
                ...
                <el-row v-else>
                  <el-input v-model="form.baseInfo.custId.text" class="input-type-124 popup-input" :readonly=true>
              ...
              <el-form-item label="合同协议号：" prop="baseInfo.contractCode.value">
                ...
                <el-row class="flex-base" v-else>
                  ...
                  <el-upload class="upload" ref="attatchFile" :action="form.baseInfo.attatchFile.action" :on-preview="handlePreview" :on-remove="removeAttatchFile" :limit="form.baseInfo.attatchFile.limit" :file-list="form.baseInfo.attatchFile.list" :on-change="setAttatchFile" :auto-upload="false">
                    <el-button slot="trigger" size="small" type="primary" style="padding:0">添加附件</el-button>
                  </el-upload>
            ...
            <el-form-item label="入账影院：" prop="baseInfo.incomeCinemaId.value" :rules="form.baseInfo.incomeCinemaId.rules">
              ...
              <el-row v-else>
                <el-input v-model="form.baseInfo.incomeCinemaId.text" class="input-type-124 popup-input" :readonly=true>
            ...
            <el-form-item label="外部导入：" prop="baseInfo.importFileName.value" v-if="form.baseInfo.couponGeneration.value == '1'&& model.type != 'revise'">
              <el-row class="flex-base">
                <el-upload class="upload" ref="importFileName" :action="form.baseInfo.importFileName.action" :limit="form.baseInfo.importFileName.limit" :on-preview="handlePreview" :on-remove="removeImportFile" :file-list="form.baseInfo.importFileName.list" :on-change="setImportFile" :auto-upload="false">
                  <el-button slot="trigger" size="small" type="primary" plain style="width:auto;">导入票券编号</el-button>
                </el-upload>
                <el-button type="primary" class="margin-left-10" plain @click="downloadTemplate" style="width:auto;">下载导入模板</el-button>
              </el-row>
            </el-form-item>
            <el-form-item label="备注：" prop="baseInfo.remark.value" :rules="[{validator:remark}]">
          ...
          <el-collapse-item title="票券消费规则" name="commonInfo">
            <el-form-item label="有效期：" prop="commonInfo.validType.value" :rules="{required: true}">
              ...
              <div>
                <el-form-item prop="commonInfo.validDate.value" :rules="form.commonInfo.validDate.rules" v-if="form.commonInfo.validType.value==0">
                  <el-date-picker v-model="form.commonInfo.validDate.value" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="disabledDate">
                  </el-date-picker>
                </el-form-item>
              </div>
              <div>
                <el-form-item prop="commonInfo.validGenType.value" :rules="form.commonInfo.validGenType.rules" v-if="form.commonInfo.validType.value==1">
                  ...
                  <el-form-item prop="commonInfo.validGenValue.value" :rules="[{required:true,validator:checkVliadDate}]" v-if="form.commonInfo.validGenType.value==1" style="display:inline-block">
                    <el-input placeholder="请输入正整数" v-model="form.commonInfo.validGenValue.value" class="input-type-124"></el-input>
                  </el-form-item>
                </el-form-item>
              </div>
            </el-form-item>
            <el-form-item label="排除日期：" prop="commonInfo.eliminationDate.value">
              ...
              <template v-if="form.commonInfo.eliminationDate.value.includes('not_DateRangeContainOperator')">
                <el-form-item style="padding-bottom:15px;" v-for="(item,index) in form.commonInfo.appointInvalidDate.value" :key="index" :prop="'commonInfo.appointInvalidDate.value.' + index + '.value'" :rules="{required: true, message: '请选择日期范围', trigger: 'blur'}">
                  <el-row class="flex-base">
                    <el-date-picker v-model="item.value" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="disabledDate">
                    </el-date-picker>
              ...
              <el-row class="flex-base">
                ...
                <template v-if="form.commonInfo.cinemaCode.value != ''">
                  <el-form-item prop="commonInfo.cinemarangev.value" :rules="form.commonInfo.cinemarangev.rules">
                    <el-row class="flex-base">
                      <el-input class="input-type-124 popup-input" v-model="form.commonInfo.cinemarangev.text" :readonly=true>
            ...
            <el-form-item label="交易渠道：" prop="commonInfo.consumeWayCode.value">
              ...
              <template v-if="form.commonInfo.consumeWayCode.value.includes('normalIn')">
                <el-form-item prop="commonInfo.consumeWayCodeOp.text" :rules="form.commonInfo.consumeWayCodeOp.rules" style="display:inline-block;margin-left:10px;">
                  <el-row class="flex-base">
                    <el-input class="input-type-124 popup-input" v-model="form.commonInfo.consumeWayCodeOp.text" :readonly=true>
                ...
                <template v-if="form.commonInfo.customerLevelType.value == 'normalIn' || form.commonInfo.customerLevelType.value == 'normalNotIn'">
                  <el-row class="flex-base margin-left-10">
                    <el-form-item prop="commonInfo.customerLevelCode.value" :rules="form.commonInfo.customerLevelCode.rules">
                      <el-input class="input-type-124 popup-input" v-model="form.commonInfo.customerLevelCode.text" :readonly=true>
                ...
                <template v-if="form.commonInfo.cardPolicyType.value == 'normalIn' || form.commonInfo.cardPolicyType.value == 'normalNotIn'">
                  <el-row class="flex-base margin-left-10">
                    <el-form-item prop="commonInfo.cardPolicyCode.value" :rules="form.commonInfo.cardPolicyCode.rules">
                      <el-input class="input-type-124 popup-input" v-model="form.commonInfo.cardPolicyCode.text" :readonly=true>
            ...
            <el-form-item label="会员生日：" prop="commonInfo.MemberInfo.value">
              <el-row class="flex-base">
                ...
                <template v-if="form.commonInfo.MemberInfo.value == 'EqualCurrentDayOffset'">
                  <el-form-item prop="commonInfo.memberBirthdayOffset.value" :rules="[{required:true,validator:checkMemberBirthdayOffset}]">
          ...
          <el-collapse-item v-if="ruleInfo.isShow" :title="ruleInfo.config.title" name="moneyInfo">
            ...
            <template v-if="ruleInfo.config.showType == '1'">
              <div class="moneyInfoClass">
                <el-form-item class="change-label inline-reset" label="代金券面值(抵用金额)：" prop="moneyInfo.voucherValue.value" :rules="[{required:true,validator:checkVoucherValue}]">
                ...
                <el-form-item class="change-label inline-reset" label="代金券消费密码：" prop="moneyInfo.voucherIsPassword.value">
                  <el-row class="flex-base">
                    ...
                    <el-form-item v-if="form.moneyInfo.voucherPasswordType.value == '2'&& form.moneyInfo.voucherIsPassword.value" prop="moneyInfo.voucherPassword.value" :rules="[{required:true,validator:checkVoucherPassword}]">
                ...
                <el-form-item label="每笔消费金额不小于：" prop="moneyInfo.sumPrice.value" :rules="[{required:true,validator:checkSunPrice}]" class="inline-reset">
      ...
      <!-- <fixStepTool :stepData="stepData.stepList" ref="FixStepTool"></fixStepTool> -->
    </div>
    <!-- 新建|编辑 规则 -->
    <div v-if="model.pageName == 'setRuleItem'">
      <setRuleItem :data="ruleChildItem.data" :limitName="ruleChildItem.limitName" :familyIdBaseNum="ruleChildItem.familyIdBaseNum" :type="ruleChildItem.type" :editIndex="ruleChildItem.editIndex" @closeItem='closeItem' @editItem='editItem'>
      </setRuleItem>
    </div>

    <!-- 弹窗 -->
    <!-- 查看预生成编号 -->
    <find-start-num ref="findStarNum" :couponCount="form.baseInfo.couponCount.value" :incomeCinemaId="form.baseInfo.incomeCinemaId.value" @batchTicketIdsCallBack="batchTicketIdsCallBack"></find-start-num>
    <!-- 交易渠道 -->
    <tradeChannelDialog :title="tradeChannelDialog.title" :dialogTableVisible.sync="tradeChannelDialog.tradeChannelDialogVisible" ref="consumeWayCodeOp" :channelNature="''" @callBack="handleOtherTradeChannelCallBack"></tradeChannelDialog>
    <!-- 入账包含组织节点影院 -->
    ...
    <!-- 影院范围 多选 -->
    <cinemaMultipDialog :title="cinemaMultipDialog.title" :dialogTableVisible.sync="cinemaMultipDialog.cinemaDialogVisible" ref="cinemarangev" @callBack="handleRegisterCinemaCallBack">
    </cinemaMultipDialog>
    <!-- 交易客商 -->
    <tradeMerchantSingleDialog ref="custId" @tradeMerchantSingleCallBack="tradeMerchantSingleCallBack" :customerType="form.baseInfo.salesMode.value" :customerId="form.baseInfo.custId.value"></tradeMerchantSingleDialog>
    <!-- 会员卡政策 -->
    <crmCardPolicyDialog @crmCardPolicyDialogCallBack="cardPolicyDialogCallBack" whereUse="mainPageUnique" :reviewData="reviewData" :dialogVisible.sync="cardPolicyDialogVisible" :unique="'id'" />
    <!-- 会员等级 -->
    <crmMemberLevelDialog @crmMemberLevelDialogCallBack="MemberLevelDialogCallBack" :reviewData="reviewData" :dialogVisible.sync="memberLevelDatadialogVisible" />
  </div>
</template>
...
import { getChannelList } from 'ccmSrc/http/interface';
...
import setRuleItem from './setRuleItem.vue';
...
export default {
  ...
  mixins: [alertHandle, cacheMixin.cacheMixin],
  data () {
    return {
      /* 缓存数据 */
      cacheField: ['activeNames', 'model', 'form', 'ruleInfo', 'ruleChild', 'showRule', 'limitName', 'ruleChildItem'],
  ...
  created () {
    ...
    // this.calcScrollTop('el-collapse-item', this.stepData.stepList)
    /**
  ...
  computed: {},
  mounted () {
    // let isAdd  = this.$route.query.isAdd ? this.$route.query.isAdd : false
    // //新建清除缓存，重置form表单
    // if(isAdd){
    //     console.log('清除表单')
  ...
  methods: {
    ...
    btnShow () {
      if (this.model.type == 'revise') {
        ...
        return (this.form.baseInfo.couponGeneration.value != 1);
      }
    },
    ...
    checkCouponPrice (rules, value, callback) {
      ...
      if (!reg.test(value)) {
        ...
        return callback(new TypeError('销售单价不能大于9999'));
      }
    ...
    remark (rules, value, callback) {
      if (value.length > 100) {
        return callback(new TypeError('限制100个字符'));
      }
      return callback();
    },
    /**
    ...
    resetcouponTypeId (couponType) {
      ...
      this.stepData.stepList.splice(2, 1);
    },
    /**
     * resetRules  - 重置规则
     */
    ...
    changeTicketType () {
      ...
      /* 添加 固定侧边栏 */
      console.log('执行票券分步函数', this.stepData.stepList);
      if (this.stepData.stepList.length > 2) {
        this.stepData.stepList.splice(2, 1, {
          name: this.ruleInfo.config.title,
          isactive: false
        });
      } else {
    ...
    editionRule (type, index) {
      ...
      if (type == 'edit') {
        ...
        // });
        // console.log(this.ruleChild[index])
        // this.ruleChildItem = this.ruleChild[index]
        this.ruleChildItem.data = JSON.parse(JSON.stringify(this.ruleChild[index]));
        this.ruleChildItem.editIndex = index;
        this.ruleChildItem.limitName = this.limitName;
      } else {
        this.ruleChildItem.data = {};
        this.ruleChildItem.editIndex = -1;
      }
      console.log('editIned', this.ruleChildItem.editIndex);
      this.ruleChildItem.type = _type;
      this.ruleChildItem.limitName = this.limitName;
      this.ruleChildItem.familyIdBaseNum = (new Date()).getTime();
      this.model.pageName = 'setRuleItem';
      // this.$router.push({
      //     path: 'setRuleItem',
      //     query: {
      //         type: _type,
    ...
    /* 取消item */
    closeItem () {
      this.model.pageName = 'saleList';
    },
    /* 编辑返回 */
    editItem (params) {
      let _ind = params.editIndex;
      if (_ind >= 0) {
        console.log('替换');
        this.ruleChild.splice(_ind, 1, JSON.parse(JSON.stringify(params.ruleChild)));
      } else {
        console.log('提娜佳');
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$ccmList.saleListSaveOrUpdate(param).then(data => {
            ...
            if (type == 'success') {
              /** 清缓存 */
              let clearData = ['form', 'ruleInfo', 'ruleChild', 'showRule', 'limitName', 'activeNames', 'model', 'stepData'];
...
.moneyInfoClass /deep/ .el-form-item__label {
  margin-left: 22px;
}
</style>
