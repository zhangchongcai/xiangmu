<template>
  <div class="table-page">
    ...
    <section v-if="table.data.length != 0" class="pagination-section flex-base flex-center pageStyle">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="pageConfig.currentPage" :page-sizes="pageConfig.pageSizes" :page-size="pageConfig.pageSize" background layout="total, prev, pager, next, jumper, sizes" :total="pageConfig.total">
      </el-pagination>
    </section>
    <!-- 弹窗 -->
...
export default {
  ...
  methods: {
    ...
    setParam (param) {
      let pointer = this;
      let exitsParam = param ? JSON.stringify(param) != '{}' : false;
      let exitsSearhParam = JSON.stringify(pointer.searchParam) != '{}';
      let _param = {
