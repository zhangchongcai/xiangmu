<template>
  <div class="table-page">
    ...
    <!-- 弹窗 -->
    <el-dialog title="票券打印" :visible.sync="printVisible" width="32%" :close-on-click-modal="false">
        ...
        <el-form :inline="true" ref="print" label-width="85px" size="small" class="form-print" :model="form" :rules="rules">
          ...
          <el-form-item label="打印设备：" prop="opara">
            <el-select v-model="form.opara" placeholder="请选择" @change="oparaChange">
              <el-option v-for="item in oparaOptions" :key="item.value" :label="item.name" :value="item.code">
              </el-option>
            </el-select>
          </el-form-item>
    ...
    <el-dialog title="票券重打印" :visible.sync="rePrintVisible" width="32%" :close-on-click-modal="false">
      <div class="print-main">
...
<script>
import printModel from 'ccmSrc/dialogs/printModel';
import searchLan from '../../../components/search/index.vue';
import util from 'ccmSrc/util/app.js';
...
export default {
  ...
  methods: {
    ...
    /* 查看是否存在默认票版 */
    isExistDefaultTemplate () {
      this.$ccmList.selectDefault().then(res => {
        if (res.code == 200) {
          this.form.templateName = res.data.name;
          this.form.templateId = res.data.id;
        }
      });
    },
    /**
    ...
    printMethods (type, data) {
      if (typeof (app) !== 'undefined' && app.sendMessage) {
        ...
        this.$confirm('浏览器下不支持打印功能，请运行‘慧影云影院收益管理平台’客户端进行票券打印，如需下载‘慧影云影院收益管理平台’，请点击‘下载’按钮。', '打印提示', {
          title: '打印提示',
          ...
          .then(() => {
            ...
            elink.download = '慧影云影院收益管理平台';
            elink.style.display = 'none';
            elink.href = 'http://www.oristarcloud.com/software/download.html';
     ...
     */
    selecteTemplate () {
      this.$refs.tamplate.openDialog(this.form.templateId);
    },
    /**
