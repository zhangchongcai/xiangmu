          ...
          <el-table-column :key="index" v-if="!item.type && item.hasTemplate" :width="item.width" :label="item.label" :fixed="item.fixed">
            <template slot-scope="scope">
              ...
              <div v-else-if="item.prop == 'control'">
                <el-button type="text" @click="editItem(scope)" style="width:auto">编辑</el-button>
                <el-button type="text" @click="deleteItem(scope)" style="width:auto">删除</el-button>
              </div>
            </template>
          </el-table-column>
        </template>
    ...
    getTableData (param) {
      ...
      this.$ccmList.findCouponList(param).then(data => {
        ...
        if (this.searchBool) {
          if (type == 'warning') {
            pointer.$message({
              message: data.msg ? data.msg : '查询失败',
              type
            });
          }
