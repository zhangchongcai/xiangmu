...
<section class="movie-plan-window" @scroll="scroll" :style="{width:width+'px',height: contentSize.height +'px'}" ref="moviePlanWindow" @mousemove="changeTimeLine">
    <div class="content" :style="{width:contentSize.width+'px',height:contentSize.height+'px'}" @mousedown="addFilm" @mousemove="drawBlock" @mouseup="cleanDrawBlock" ref="content">
        <!-- 过去时间：灰色 -->
                ...
                <ul class="flex-base">
                        ...
                        <div slot="content" class="tool-tips">
                            ...
                            <div v-if="filmItem.originFlag">
                                ...
                                <div><span class="mr30">我的最低价格: {{filmItem.minPrice}}元</span><span class="mr30">竞对最低价格: {{filmItem.cheapestPrice}}元</span><span class="mr30">差价: {{Number(filmItem.minPrice) - Number(filmItem.cheapestPrice)}}元</span></div>
                            </div>
                        </div>
    ...
    methods: {
        ...
        drawBlock(e) {
            ...
            if (this.block.click) {
                ...
                // 鼠标所在元素具体位置
                let position = this.$options.methods.mousePosition(clientX, clientY, parentElement, parentScrollAll, margin_left, true)
                let positionX = position.positionX
                let positionY = position.positionY
                ...
                if (drop_data.length != 0) {
                    ...
                    for (let i = 0; i < plan_rooms.length; i++) {
                        for (let j = 0; j < plan_rooms[i].length; j++) {
                            ...
                            console.log(plan_rooms[i][j].isSelect)
                        }
                    }
        ...
        submitSave(type, cb) {
            ...
            this.$api.savePlanAll(_data).then(res => {
        ...
        deletSinglePlan(e) {
            ...
            if (movie.originFlag) {
                ...
                this.$api.deletePlan(param).then(res => {
                    if (res.code == 200) {
                        plan_rooms[r_index].splice(i_index, 1);
        ...
        subApproves() {
            ...
            this.$api.subApproves(param).then(data => {
                if (data.code == 200) {
                    // 刷新当前排片数据
                    this.$emit('initMoivePlan', {
                        type: null
        ...
        stopOrStartSale(saleStatus, isMore) {
            ...
            if (param.planUids.length != 0) {
                this.$api.saleOpenStop(param).then(data => {
