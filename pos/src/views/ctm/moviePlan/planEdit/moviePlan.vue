...
<section class="planWindow">
    ...
    <div class="window_1" :style="{width: showBan ? 'calc(100% - 183px)' : '100%'}">
        <!-- 影片库 -->
        <filmLibary ref="filmLibary" :films="films" @selectMovie="selectMovie" :width="windowWidth ? windowWidth - filmLibary_deleteWidth : normalWidth - filmLibary_deleteWidth" :mode="mode"></filmLibary>
        <div class="window_2">
            <!-- 影厅库 -->
            <filmRoom ref="filmRoom" :scrollY="scroll.y" :rooms="rooms" :contentSize="contentSize" @selectRoom="selectRoom" :mode="mode"></filmRoom>
            <div class="window_3">
                <!-- 刻度尺 -->
                <filmScale ref="filmScale" @saveChangeScale="saveChangeScale" :scrollX="scroll.x" :scaleConfig="scaleConfig" :width="windowWidth ? windowWidth - filmScale_deleteWidth : normalWidth - filmScale_deleteWidth"></filmScale>
                <!-- 放映计划 -->
                <filmPlan ref="filmPlan" @cantEdition="cantEdition" @setMindData="setMindData" @scroll="moviePlanScorll" @cleanMovie="cleanMovie" :width="windowWidth ? windowWidth - filmPlan_deleteWidth: normalWidth - filmPlan_deleteWidth" :rooms="rooms" :contentSize="contentSize" :selectMovieData="selectMovieData" :baseParam="baseParam" :plan_list="plan_list" :scaleConfig="scaleConfig" @initMoivePlan="initMoivePlan" @copyMovie="selectMovie" :pricePlan="pricePlan" @changeMyData="changeMyData" :mode="mode"></filmPlan>
            </div>
        </div>
        <!-- 标注 -->
        <filmTips ref="filmTips" :width="windowWidth ? windowWidth - filmTips_deleteWidth: normalWidth - filmTips_deleteWidth" :filmTips="filmTips"></filmTips>
    </div>
</section>
</template>
    ...
    methods: {
        ...
        changeMyData() {
            ...
            this.banData.myData.forEach(item => {
                ...
                item.scenePer = (item.scene / planTotal * 100) ? (item.scene / planTotal * 100).toFixed(2) + '%' : '0%' 
                item.goldScenePer = (item.goldScene / goldTotal * 100) ? (item.goldScene / goldTotal * 100).toFixed(2)  + '%' : '0%'
            })
