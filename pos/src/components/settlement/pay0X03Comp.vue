<template>
    <div class="pay-containers">
      ...
      <div class="right">
        <NumberKeyBoard :showPointer="true"  v-model="cashes" @keynumber="showResult"></NumberKeyBoard>
        <div class="btn-container">
            ...
            <span v-show="getUserConfig.allow_fast_checkout == '1' && vipInfo.totalBalance >= payData.notPayAmount && vipInfo.cardTypeCode == 'stored_card'" class="result-btn quickCash" @click="quickPay">快速结账</span>
        </div>
      </div>
    </div>
</template>
...
export default {
   ...
   computed:{
    ...
    watch: {
        ...
        $route() {
            if(this.$route.name == 'toHome'){ //回来结算页面才调用
    ...
    methods: {
        ...
        payIt() {
          if(this.cashes || this.payData.notPayAmount == 0) {
             ...
             else if(this.cashes > this.vipInfo.totalBalance) {
                 this.$message({
                                showClose: true,
                                message: "余额不足",
                                type: 'warning'
