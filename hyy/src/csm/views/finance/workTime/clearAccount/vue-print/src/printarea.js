export default class {
  ...
  init() {
    ...
    this.ele = ele;
    this.settings.startCallback(this.ele);
  ...
  write(PADocument, $ele) {
    ...
    PADocument.write(`<html>${this.getHead()}${this.getBody()}</html>`);
    PADocument.close();
  };
  docType() {
  ...
  getHead() {
    ...
    for (let i = 0; i < document.getElementsByTagName('style').length; i++) {
      try {
        let styleHtml = document.getElementsByTagName('style')[i].outerHTML || '';
        style += styleHtml
      } catch (error) { }
    }
    ...
    return `<head><title>${this.settings.popTitle}</title>${extraHead}${links}${style}</head>`;
  };
  getBody() {
    let htm = this.ele.outerHTML;
    // console.log('htm', htm);
