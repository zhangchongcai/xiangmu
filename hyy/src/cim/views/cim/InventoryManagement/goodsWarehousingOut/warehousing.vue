  ...
  <div class="content">
    <!-- <div class="breadcrumb">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>{{typeText}}领用出库</el-breadcrumb-item>
      </el-breadcrumb>
    </div> -->
    ...
    <cinemal-dialog ref="myCinemalDialog" @onSumit="onCinemalSumit" :dialogFeedbackData="[{cinemaUid:queryData.cinemaUid,cinemaName:queryData.cinemaName}]"></cinemal-dialog>
      ...
      <!-- 选择采购单弹窗 -->
      <purchase-note-dialog ref="allocationDialog" :outCinemaUid="queryData.cinemaUid" :approvalStatus="2" :status="2"
...
import purchaseNoteDialog from "cim/components/allocationDialog/allocationDialog.vue";
export default {
  mixins: [mixin],
  data() {
    return {
        ...
        .then(res => {
          if (res.code == 200) {
            ...
            res.data.detailList.forEach((val,index,arr)=>{
              this.goodList[index].storeoutCount = val.allotCount
        ...
        this.$refs["ruleForm"].validate(valid => {
          if (valid) {
              ...
              if(this.routeQuery.type == "1"){
                this.queryData.dataList.forEach((vval,vindex,varr)=>{
                  delete vval.uid
                })
                this.resStoreInBillRegisterSave(this.queryData,"新增")
              }else if(this.routeQuery.type == "2"){
                if(this.sty == true){
                   this.queryData.dataList.forEach((vaval,vindex,varr)=>{
                    delete vaval.uid
                  })
                }
      ...
      if(this.goodList.length == 0){
        ...
        this.$refs["ruleForm"].validate(valid => {
          if (valid) {
              ...
              // this.queryData.storeType = parseInt(this.queryData.storeType)
              if(this.routeQuery.type == "1"){
                this.queryData.dataList.forEach((val,index,arr)=>{
                  delete val.uid
                })
                this.resStoreInBillRegisterSave(this.queryData,"新增")
              }else if(this.routeQuery.type == "2"){
