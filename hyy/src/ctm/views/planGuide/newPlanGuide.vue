        ...
        <el-form ref="form" label-width="72px">
            ...
            <el-form-item>
                <el-table
                    :data="hallData">
                    <el-table-column
                        :label="item.type == 'h' ? '黄金场' : item.type + '个厅'"
                        v-for="(item, index) in hallList"
                        :key="index"
                        width="110"
                    >
                        <template slot-scope="scope">
                            <span class="table-range-con"><el-input :disabled="showType == 'view'" v-model="scope.row[item.type]" @change="inputChange($event, index)"></el-input></span>
                        </template>
                    </el-table-column>
                </el-table>
            </el-form-item>
...
import { getBaseMovieInfo } from 'ctm/http/interface'
    export default {
        props: {
            addType: {
                type: String,
