    ...
    <div :class="isFullScreen ? 'movie-plan full' : 'movie-plan'">
        <div class="movie-plan-tool">
            <div class="plan-tool-l">
                ...
                <el-button
                    class="cinema-con-button"
                    @click="$refs.frameSingleCinema.listAuthCommCinemas(), singleCinemaVisible = true"
                    :title="innerData.name"
                >{{innerData.name}}</el-button>
            ...
            <div class="plan-tool-r">
                <div
                    class="plan-menu-area"
                    :style="{width: mode ? 'calc(100% - 60px)' : '100%'}"
                >
                    <div class="meun-area-l">
                        <ul class="plan-menu-group">
                            <li
                                class="plan-single-menu"
                                @click="planTicketOffice"
                            >
                ...
                <p class="plan-menu-name">打印</p>
                            ...
                            <li
                                v-if="mode && showType != 3"
                                class="plan-single-menu"
                                @click="saveCheckedPlan"
                            >
        ...
        <el-dialog
            title="智能排片设置"
            :visible.sync="intelligentPlanSetting"
            class="int-plan-setting"
        >
            <el-form
                :model="intSettingForm"
                :rules="rules"
                ref="intSetForm"
            >
            ...
            >
                <el-button
                    @click="intelligentPlanSetting = false"
                    class="dialog-btn-w80"
                >取 消</el-button>
                <el-button
                    type="primary"
                    @click="saveGoldTimeSet"
                    class="dialog-btn-w80"
                >确 定</el-button>
            </div>
        </el-dialog>
        ...
        <el-dialog
            ...
            @close="editBatchCloseDialogCb"
        >
            <!-- 导入价格方案 -->
            <el-dialog
                class="import-price-plan-dialog"
                title="导入价格方案"
                :visible.sync="importPricePlanDialog"
                @close="importCloseDialogCb"
                append-to-body
            >
                <el-form>
                    <el-form-item>
                        ...
                        >
                            ...
                            >
                                <template slot-scope="scope">
                                    <el-button
                                        @click="toPricePlanDetail(scope.row.uid)"
                                        type="text"
                                    >查看</el-button>
                                </template>
                            </el-table-column>
        ...
        <movie-plan
            ref="moviePlan"
            :price-plan="curPricePlan"
            :base-param="baseParam"
            :mode="mode"
            :showPricePlanName="showPricePlanName"
...
export default {
    ...
    methods: {
        ...
        editBatch() {
            ...
            if (selectPlans.length && !this.curSelectPlans.length)
                return this.warning("当前选中的排片计划不可编辑")
        ...
        selectSinglePlan() {
            ...
            priceprogramScan({ id: this.selectPlanData.data.id }).then(res => {
                if (res.code != 200) return this.error(res.msg)
                if (res.code == 200 && res.data) {
                    if (!this.selectPlanData.isShow)
                        this.selectPlanData.isShow = true
