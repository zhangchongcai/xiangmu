<template>
    <div class="movie-plan-continuity">
        <div class="basic-info movie-info" v-if="isEditMode">
            <el-form
                label-width="80px">
        ...
        <div class="basic-info" v-if="isEditMode">
            ...
            <el-checkbox-group v-model="channelMulData" @change="handleCheckedChannelChange">
                <el-checkbox v-for="channel in channelData" @change="singleCheckChange(channel)" :label="channel" :key="channel.uid">{{channel.name}}</el-checkbox>
            </el-checkbox-group>
            <div>
            </div>
        </div>
        <div class="basic-info" v-else>
            ...
            <div style="margin: 15px 0;"></div>
            <el-checkbox-group v-model="channelMulData" v-if ="channelMulData.length">
                <el-checkbox v-for="channel in channelData" @change="singleCheckChange(channel)" :label="channel" :key="channel.uid" disabled>{{channel.name}}</el-checkbox>
            </el-checkbox-group>
...
<script>
import {getmoviePlanDetails, cinemaStockList, initTimeLine, datePlanList, savePlanAll} from 'ctm/http/interface'
    export default {
        data() {
            return {
                isEditMode: false,
        ...
        methods: {
            ...
            channelSelChange(val) {
                ...
                this.checkAll = val.length === this.channelData.length
                this.isIndeterminate = (!!val.length) && (val.length < this.channelData.length)
                this.channelMulData = val
            },
            // 增加影片进入连排
            addMovieData() {
                ...
                selAddData.forEach((item, i) => {
                    ...
                    item.minPrice = item.minPrice ? item.minPrice : 0.00
                    item.percentPrice = item.percentPrice ? item.percentPrice : 0.00
                    item.publisherRate = item.publisherRate ? item.publisherRate : 0.00
                })
                ...
                getmoviePlanDetails({
                    ...
                    if (this.isEditMode) {
                        ...
                        this.$nextTick(() => {
                            ...
                            console.log(this.ticketData.filter(item => item.switchStatus))
                        ...
                        this.isIndeterminate =  this.channelData.some(item => item.switchStatus) && !this.channelData.every(item => item.switchStatus)
                    } else {
                        res.data.ticketList.forEach(item => {
                            item.addFee = parseFloat(item.addFee).toFixed(2)
                            item.price = parseFloat(item.price).toFixed(2)
            ...
            handleCheckAllChange(val) {
                this.channelMulData = val ? this.channelData : []
                this.$refs.channelMultipleTable.clearSelection()
                this.channelMulData.forEach(row => {
            ...
            handleCheckedChannelChange(value) {
                ...
                this.checkAll = checkedCount === this.channelData.length
                this.isIndeterminate = (checkedCount > 0) && (checkedCount < this.channelData.length)
            },
            singleCheckChange(val) {
                this.$refs.channelMultipleTable.toggleRowSelection(val)
            },
            tableFixed(scope) {
