<template>
  <div class="ctm extra-price-list">
        ...
        >
          ...
          >
            <el-radio-group v-if="enabledEdit" v-model="dataForm.status">
              ...
              class="c-view"
              v-if="!enabledEdit"
            >{{dataForm.status==1?'启用': (dataForm.status==0?'禁用':'')}}</span>
          </el-form-item>
          <el-form-item
            label="优先级："
            prop="priority"
        ...
        >
          ...
          >
            ...
            <el-button
              ...
              type="primary"
              plain
              @click="showCinemaModel()"
            >选择影院</el-button>
            <div class="w-600">
              <span
                class="c-view"
                v-if="!enabledEdit"
              >{{dataForm.cinemaNames}}</span>
            </div>
          </el-form-item>
          <el-form-item
            label="适用渠道："
          ...
          >
            <el-select
              v-if="enabledEdit"
              v-model="dataForm.useChannel"
              @change="useChannelChange"
              placeholder="请选择"
            >
              <el-option
                label="不限"
            ...
            <el-input
              v-if="enabledEdit"
              v-show="dataForm.useChannel==0"
              ref="channelNameInput"
              style="margin-left:5px;"
              :readonly="true"
              v-model="dataForm.channelShortName"
              placeholder=""
            ></el-input>
            <el-button
              v-if="enabledEdit"
              ...
              @click.prevent="showChannelModel"
            >请选择渠道</el-button>
            <div class="w-600">
            <span v-if="!enabledEdit && dataForm.useChannel==1" >不限渠道</span>
            <span
              class="c-view"
              v-if="!enabledEdit && dataForm.useChannel==0"
            >{{dataForm.channelNameStr}}</span>
            </div>
          </el-form-item>
          ...
          >
            <el-select
              v-if="enabledEdit"
              v-model="dataForm.useHallType"
              @change="useHallTypeChange"
              placeholder="请选择"
            >
              <el-option
                label="不限"
            ...
            <div class="w-600">
              <span
                class="c-view"
                v-if="!enabledEdit"
              >{{dataForm.useHallType==0?(dataForm.ciPriceHallTypeStr):'不限类型'}}</span>
            </div>
          </el-form-item>
          ...
          >
            <el-select
              v-if="enabledEdit"
              v-model="dataForm.useHall"
              @change="useHallChange"
              placeholder="请选择"
            >
              <el-option
                label="不限"
            ...
            <span
              class="c-view"
              v-if="!enabledEdit && dataForm.useHall==1"
            >不限影厅</span>
            <div
              class="cinema-room m-t-16"
              v-show="dataForm.useHall==0"
            >
              ...
              <div
                v-for="(item,index) in dataForm.ciCinemaAndHallVos"
                :key="index"
                class="flex-wrapper"
              >
                ...
                <div class="td flex-1 s-h-326" :class="(item.selectedHalls && item.selectedHalls.length>4)?'':'f-h-32'">
                  <el-select
                    class="m-v-5"
                    v-if="item.hallList && enabledEdit"
                    size="small"
                    v-model="item.selectedHalls"
                    @change="(val)=>electedHallChange(val,item)"
                    multiple
                    placeholder="请选择"
                  >
                    <el-option
                      v-for="(item,idx) in item.hallList"
                      :key="idx"
                      :label="item.name"
                      :value="item.uid"
                    >
          ...
          >
            ...
            <span
              class="c-view"
              v-if="!enabledEdit"
            >{{dataForm.daterange.length==2?(formatDate(dataForm.daterange[0])+' 至 '+formatDate(dataForm.daterange[1])):''}}</span>
          </el-form-item>
        ...
        >
          ...
          >
            <div v-if="enabledEdit">
              ...
              <el-input-number
                class="w-180"
                v-model="dataForm.priceA"
                :controls="false"
                :min="0"
                :max="dataForm.priceTypeA==1?100:999999999"
                :precision="2"
                placeholder=""
              ></el-input-number>
            ...
            <div v-if="enabledEdit" class="m-t-16">
              ...
              <el-input-number
                v-show="dataForm.priceTypeB==3"
                v-model="dataForm.priceB"
                :controls="false"
                :min="0"
                :precision="2"
                placeholder=""
    ...
    />
        <TableSelect
      :formItems="channelFormItems"
      nameKey="name"
      rowKey="uid"
      :request="channelRequestApi"
      :columns="channelColumns"
      :defaultQuery="channelDefaultQuery"
      :multiSelect="true"
      title="选择渠道"
      :modelVisible="channelModelVisible"
      @close="channelModelVisible=false"
      @select="channelSelect"
      :defaultSelectRows="channelDefaultSelectRows"
    />
...
import { cinemaList,queryChannelList,queryExtraPriceDetail,fetchCinemaList,hallTypeList,fetchCinameHallByType,saveExtraPrice,updateExtraPrice,getEditChannelList } from 'ctm/http/interface'
...
export default {
  ...
  mixins: [fixStepMixin,mixins.cacheMixin],
  data () {
    return {
