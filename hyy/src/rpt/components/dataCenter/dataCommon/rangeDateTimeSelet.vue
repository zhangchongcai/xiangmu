<template>
	...
	<div style="display:inline-block;">
		<el-date-picker
	      	v-model="datetime"
	      	:type="type"
	      	class="input adjustProfixPlace"
	      	range-separator="至"
	      	start-placeholder="开始日期"
	      	end-placeholder="结束日期" @change="dateTimeValueChange" placeholder="请选择操作时间">
	    </el-date-picker>
	</div>
</template>
	...
	export default{
		...
		methods : {
			dateTimeValueChange(val){
				...
				this.newselectValue.queryColValue = result.join(',')
				datacenterBus.$emit('input', this.newselectValue);
				datacenterBus.$emit('dataTime', result.join('至'));
				this.$emit('change');
			}
		}
	}
