      ...
      <ul class="datacenter-query-ul clearfix">
        <template v-for="(item,index) in baseQueryArr">
          ...
          <li v-if="item.controlType === 'TICKET_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <ticket-type
                :cinemaUid="dataQueryObj.cenimaUid"
          ...
          <li v-if="item.controlType === 'WAREHOUSE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <warehouse
                ...
                :cenimaUid="dataQueryObj.cenimaUid"
          ...
          <!-- 精确时间控件 -->
          <li
            v-if="item.controlType === 'DATETIME' && reportCode !== 'T05'"
            :key="index"
            class="date"
          >
            <span class="small-ii DC666 DC-margin-left32">{{ item.queryName + " :" }}</span>
            <date-time-select
              :dateTimeSelectObj="dateTimeSelectObj"
              :queryName="item.queryName"
              :resetStatus="resetStatus"
          ...
          <!-- T05交易时间控件 -->
          <li
            v-if="item.controlType === 'DATETIME' && item.queryColKey ==='transaction_date' && reportCode === 'T05'"
            :key="`transaction_date_${index}`"
            class="date"
          >
            <span class="small-ii DC666 DC-margin-left32">{{ item.queryName + " :" }}</span>
            <transaction-date
              :queryName="item.queryName"
              :transactionDateObj="transactionDateObj"
              :resetStatus="resetStatus"
              @selectTransactionDateDate="getTransactionDateDate"
            ></transaction-date>
          </li>
          <!-- T05放映时间控件 -->
          <li
            v-if="item.controlType === 'DATETIME' && item.queryColKey ==='show_time' && reportCode === 'T05'"
            :key="`show_time_${index}`"
            class="date"
          >
            <span class="small-ii DC666 DC-margin-left32">{{ item.queryName + " :" }}</span>
            <show-time
              ...
              :queryName="item.queryName"
              :resetStatus="resetStatus"
              @selectShowTimeData="getShowTimeData"
            ></show-time>
          </li>
          <!-- 时间控件 -->
          ...
          <li v-if="item.controlType === 'TRANSACTION_ORDER_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <transaction-order
                :resetStatus="resetStatus"
                :transactionOderObj="transactionOderObj"
          ...
          <li v-if="item.controlType === 'DEAL_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <deal-type
                :dealTypeObj="dealTypeObj"
                :resetStatus="resetStatus"
                @selectDealTypeData="getDealTypeData"
              ></deal-type>
            </div>
          </li>
          <!-- 影厅类型 -->
          <li v-if="item.controlType === 'HALL_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <hall-type
                :hallTypeObj="hallTypeObj"
                :resetStatus="resetStatus"
                @selectHallTypeData="getHallTypeData"
              ></hall-type>
            </div>
          </li>
          <!-- 影厅 -->
          ...
          <li v-if="item.controlType === 'CHANNEL'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <channel
                ...
                :resetStatus="resetStatus"
                @selectChannelData="getChannelData"
              ></channel>
            </div>
          </li>
          <!-- 单选影院 -->
          ...
          <li v-if="item.controlType === 'CINEMA_TREE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <multi-cinema
                :multiCinemaObj="multiCinemaObj"
                :resetStatus="resetStatus"
                :queryData="item"
                :multiCinemaLoading="multiCinemaLoading"
                @selectMultiCinemaData="getMultiCinemaData"
                @setMultiCinemaLoading="setMultiCinemaLoading"
              ></multi-cinema>
            </div>
          </li>
          <!-- 会员等级 -->
          ...
          <li v-if="item.controlType == 'MOVIE_RELEASED_VERSION'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <movie-released-version
                :movieReleasedVersionObj="movieReleasedVersionObj"
                :resetStatus="resetStatus"
          ...
          <li v-if="item.controlType === 'SALE_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <sale-type
                ...
                :resetStatus="resetStatus"
                @selectSaleTypeData="getSaleTypeData"
              ></sale-type>
            </div>
          </li>
          <!-- 授权人/收银员 -->
          ...
          <li v-if="item.controlType === 'MER_CLASS'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <mer-class
                :merClassObj="merClassObj"
                :resetStatus="resetStatus"
                @selectMerClassData="getMerClassData"
              ></mer-class>
            </div>
          </li>
          <!-- 商品类型 -->
          <li v-if="item.controlType === 'MER_TYPE'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <mer-type
                :merTypeObj="merTypeObj"
                :resetStatus="resetStatus"
                @selectMerTypeData="getMerTypeData"
              ></mer-type>
            </div>
          </li>
          <!-- 商品 -->
          <li v-if="item.controlType === 'MER_GOODS'" :key="index">
            ...
            <div class="small-ii DC-input208">
              <mer-goods
                :controllerObj="controllerObj"
                :resetStatus="resetStatus"
                :merClassUid="dataQueryObj.merClassUid"
                :queryData="item"
              ></mer-goods>
            </div>
          </li>
          ...
          <li v-if="item.controlType == 'CHECK_SING'" :key="index">
            <combo-son-goods
              :comboSonGoodsObj="comboSonGoodsObj"
              :queryName="item.queryName"
      ...
      <div slot="dialog-content">
        <div class="common-box">
          ...
          <ul class="common-box-query1">
            <draggable
              v-model="dataQueryObj.showQueryArr"
            ...
            >
              <li
                ...
                v-for="(element,index) in dataQueryObj.showQueryArr"
                :key="index"
          ...
          <ul class="common-box-query2">
            <draggable
              v-model="dataQueryObj.hideQueryArr"
              :options="{group:'people'}"
              @start="drag=true"
              @end="drag=false"
            >
              <li
                class="commonbox-query-coment"
                v-for="(element, index) in dataQueryObj.hideQueryArr"
                :key="index"
              >{{ element.queryName }}</li>
            </draggable>
          </ul>
...
import { filterFunc, messageAlertFunc } from "src/rpt/util/commonFunc.js";
...
export default {
  ...
  methods: {
    ...
    getTableValue(value) {
      ...
      let colDataArr = [];
      //处理支付方式查询信息
      if (this.reportCode == "T100") {
        // 支付方式控件是否选择
        if (
          this.dataQueryObj.payTypeValueArr.length != 0 &&
          this.dataQueryObj.payTypeValueArr[0] !== ""
        ) {
          let deleteIndex = [];
        ...
        if (this.isSetTableHeader === true) {
          ...
          if (this.dataQueryObj.payTypeValueArr[0] === "") {
      ...
      if (this.dataQueryObj.searchData.length != 0) {
        this.dataQueryObj.searchData.forEach(element1 => {
          this.baseQueryArr.forEach(element2 => {
            if (element1.queryColKey && element1.queryColKey == element2.queryColKey) {
      ...
      if (this.mergeTableObj.isMerged == true) {
