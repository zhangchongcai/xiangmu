    ...
    <div class="datacenter-btn-right">
      <div class="datacenter-btn-rightul">
        <button
          ...
          @click="handleMergeTable">切换表格</button>
        <!-- 自定义模板下拉菜单 -->
        <el-dropdown
          v-if="dataBtnObj.showStyleList"
          trigger="click"
          @click.native="handleMuneBtnClick"
        ...
        >
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            >
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item
                  ...
                  @mouseenter.native="handleMouseEnter"
                  @mouseleave.native="handleMouseLeave"
                  @click.native="setStyle('default')"><span>默认模板</span></el-dropdown-item>
            ...
            <el-dropdown-item
              ...
              @mouseenter.native="handleMouseEnter"
              @mouseleave.native="handleMouseLeave"
              @click.native="handleSaveClick">保存为新样式</el-dropdown-item>
            <el-dropdown-item 
              style="color:#606266!important;" 
              @mouseenter.native="handleMouseEnter"
              @mouseleave.native="handleMouseLeave"
              @click.native="handleDeleteClick">删除</el-dropdown-item>
            <!-- <el-dropdown-item
              v-if="dataBtnObj.rightGroupArr.length != 0 || dataBtnObj.rightGroupsShow.length != 0"
        ...
        <el-dropdown trigger="click" class="print-export-dropdown">
          ...
          <el-dropdown-menu slot="dropdown" class="print-export-dropmenu">
            <el-dropdown-item 
              ...
              @mouseenter.native="handleMouseEnter"
              @mouseleave.native="handleMouseLeave"
              @click.native="pagesExport">导出</el-dropdown-item>
            <el-dropdown-item
              style="color:#606266!important;" 
              @mouseenter.native="handleMouseEnter"
              @mouseleave.native="handleMouseLeave"
        ...
        <div slot="dialog-content">
          <div class="common-box">
            <ul class="common-box-query1">
              ...
              >
                <li
                  ...
                  v-for="(element,index) in dataBtnObj.nowColArr"
                  :key="`comment1_${index}`"
                >{{ element.label }}</li>
              </draggable>
            </ul>
            ...
            <ul class="common-box-query2">
              <draggable
                v-model="dataBtnObj.nownoColArr"
                :options="{group:'people', ghostClass: 'ghost-item', dragClass: 'drag-item'}"
                :move="moveCol"
              >
                <li
                  class="commonbox-query-coment"
                  v-for="(element, idx) in dataBtnObj.nownoColArr"
                  :key="`comment2_${idx}`"
                >{{ element.label }}</li>
              </draggable>
            </ul>
            ...
            <div class="groupView-left">
              ...
              <div class="groupView-left-main">
                <el-checkbox-group v-model="leftGroupsChecked" @change="handleCheckedCitiesChange">
                  <el-checkbox
                    v-for="(city, index) in dataBtnObj.leftGroupsShow"
                    :label="city"
                    :key="`city${index}`"
...
import { messageAlertFunc } from "src/rpt/util/commonFunc.js";
// 数组取差值
  ...
  methods: {
    handleMergeTable() {
      if (this.mergeTableObj.isMerged) {
        ...
        if (this.dataBtnObj.isOpenedGroupDialog) {
          this.$emit(
            "groupArrEvent",
    ...
    setgroupView: function() {
      ...
      for (var i = 0; i < this.dataBtnObj.leftGroupArr.length; i++) {
        ...
        if (isGroupShow)
          this.dataBtnObj.leftGroupsShow.push(this.dataBtnObj.leftGroupArr[i]);
      }
      ...
      if (
        ...
        this.styleNameList.splice(
          this.styleNameList.findIndex(
            element => element === this.styleNameData
          ),
          1
        );
        this.styleNameData = this.inputStyleName;
