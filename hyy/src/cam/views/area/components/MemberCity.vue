  ...
  methods: {
    ...
    getAmountChart(res){    
      ...
      this.ChartsAmount.rows = res.memberIndex.yAxis.map(item => {
        return {
          日期: item.dateKey,
          会员消费金额: item.memberConsumeAmount.toFixed(2)
        };
      });
    },
    //会员消费占比
    getPropChart(res){
      ...
      this.ChartsProp.rows = res.memberIndex.yAxis.map(item => {
        return {
          日期: item.dateKey,
          会员消费占比: item.memberConsumePercent.toFixed(2)
        };
      });
    },
    getQuantChart(res){
      ...
      this.ChartsOpen.rows = res.memberIndex.yAxis.map(item => {
        return {
          日期: item.dateKey,
          开卡数量: item.newCardCount.toFixed(2)
        };
      });
    },
    getStorageChart(res){
      ...
      this.ChartsProfit.rows = res.memberIndex.yAxis.map(item => {
        return {
          日期: item.dateKey,
          储值金额: item.cardProfitAmount.toFixed(2)
        };
      });
    },
    //router跳转
