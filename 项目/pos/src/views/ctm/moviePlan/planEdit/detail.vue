...
import fixStepMixin from "src/mixins/ctm/fixStepTool"
import { getSchPlanLang, getPriceSystem, savePlanAll, importPricePlan, hallTypeList, priceprogramScan, getmoviePlanDetail } from 'src/http/ctm'
export default {
    data() {
    ...
    methods: {
        dataInit() {
            ...
            if (this.isEditMode) {
                if (!this.$store.state.moviePlan.current_row) {
                    ...
                    setTimeout(() => {
                        this.$router.push({path: '/ctm/moviePlan/layout'})
                    }, 1000)
                    return
                }
            }
        ...
        timeChange(val) {
            ...
            if (this.planHours < 6 && this.formatDateTime(tmpTimeEnd).split(' ')[1].split(':')[0] >= 6) {
                this.planMinute = this.copyMinute
                return this.error('放映结束时间不能超过早上6点!')
            }
            this.movieData.planTime = this.formatDateTime(tmpTimeStart)
        ...
        formatDateTime(timeStamp, noHour) {
            ...
            mm = mm < 10 ? `0${mm}` : mm
            return noHour ? `${y}-${m}-${d}` : `${y}-${m}-${d} ${h}:${mm}`
        },
        // 导入价格方案
        showPricePlanDialog() {
        ...
        toPricePlanDetail(id) {
            this.$router.push({path: '/ctm/ticketPrice_Plan/Scan', query: {id}})
        },
        // ------------------  save & cancle --------------------------
        cancelHandle() {
            this.$confirm('检测到未保存的内容，是否在离开页面前保存修改？', '确认信息', {
                ...
                .catch(action => {
                    ...
                    this.leaveFlag = false
                    this.$router.push({path: '/ctm/moviePlan/layout', query: {date: this.curDate}})
                })
        },
        savePlan() {
