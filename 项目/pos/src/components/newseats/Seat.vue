      ...
      <div v-show="isExtends" class="seat-detail">
        ...
        <div class="seat-contorl">
         ...
         <div class="tickets-type">
           ...
           <span v-if="selPriceItem.length">(已选：{{selPriceItem[0].name + "￥" + selPriceItem[0].price}})</span>
         </div>
         <div class="tickets-container">
          ...
          <div class="tickets-sel-container">
            <div :class="['control-common-style-selected']" v-for="(seleteditem, index) in pagesSeatSelection[moreSel]" :key="'sel' + index">
              <span style="font-size: 12px; color: #333333; font-weight: bold; margin-left: 10px">{{seleteditem.seat_name}}</span>
              <span style="font-size: 12px; color: #ffffff;" class="have-selected-ticket">{{seleteditem.type_price}} <i class="ticket-arrow iconfont iconyoujiantouda"></i></span>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      narrowSeats() {
        ...
        //以 8排  20 座  为基数， 大于这两个基数则进行计算按比例缩小
        if((this.rowYs.length > 8 || this.colXs.length > 20) && !this.isExtends) {
          this.scaleNum =  `${Math.min(8.4/this.rowYs.length, 22/this.colXs.length)}`
