  ...
  <div class="query-dialog-wrapper">
    <div class="input-button">
      <div @mouseenter="showIcon" @mouseleave="hideIcon">
        <el-input v-model="payTypeValue" :disabled="true">
          <i
            v-if="isShowIcon"
    ...
    <div class="my_dialog">
      <el-dialog title="支付方式" :visible.sync="dialigVisible" @close="closeDialog">
        ...
        <div class="choose_table">
          <div>
            ...
            >
              ...
              <el-table-column prop="payTypeCode" label="支付编号" show-overflow-tooltip></el-table-column>
          ...
          <div class="choose_ul">
            ...
            <ul class="ul_body">
              <li v-for="(item,index) in selectedData" :key="index">
                <div class="choose_name">{{item.payTypeName}}</div>
...
export default {
  ...
  methods: {
    handleIconClick() {
      ...
      this.selectedData = [];
      this.selectObj = {};
      this.$emit("selectPayTypeData", this.payTypeValue);
    },
    showIcon() {
      if (this.payTypeValue !== "") this.isShowIcon = true;
    },
    hideIcon() {
    ...
    openDialog() {
      ...
      if (this.cinemaUids.length === 1 && this.cinemaUids[0] === "") {
        ...
        // this.getData(['35054251', '745498'], this.payTypeName, 1, 10);
      }
    },
    setData() {
      this.selectObj = {};
    ...
    //切换分页请求数据
    currentChange(val) {
      this.getData(this.cinemaUids, this.payTypeName, val, 10);
    },
    //清空所选数据
    clearClick() {
    ...
    comfirmClick() {
      ...
      if (this.selectedData.length !== 0) {
        this.selectedData.forEach(element => {
          this.payTypeValue += `${element.payTypeName},`;
          payTypeCode += `${element.payTypeCode},`;
        });
      } else {
        this.payTypeValue = "";
      }
      this.$emit("selectPayTypeData", payTypeCode);
    ...
    //发请求获取数据的方法
    getData(cinemaUids, payTypeName, currentPage, pageSize) {
      this.$rptList
        .getPayTypeData(cinemaUids, payTypeName, currentPage, pageSize)
        .then(res => {
          console.log(res);
          if (res.code === 200) {
