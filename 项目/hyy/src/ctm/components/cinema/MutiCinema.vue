...
export default {
  ...
  created () {
    this.chooseItem = this.innerData;
  },
  mounted () {
    this.getCinemaList();
  },
  methods: {
    ...
    pushItem (item) {
      let tmp_item = this.chooseItem.find((el) => {
        return el.code == item.code
      })
      if (!tmp_item) {
    ...
    removeItem (item) {
      // let item = this.chooseItem.find((el) => {
      //   return el.code == item.code
      // })
      this.chooseItem = this.chooseItem.filter((el) => {
        return el.code != item.code
      })
    },
    // 全选
    ...
    getCinemaList () {
      ...
      authQueryUserCinemas(params).then(res => {
        this.loading = false;
        if (res.data && res.code == 200) {
          this.tableList = res.data.rows
          let rowObj = {};
          this.tableList.forEach(item => {
            rowObj[item.code] = rowObj;
          })
          this.rowObj = rowObj;
