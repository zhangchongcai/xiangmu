  ...
  <div class="movie-price-plan">
    <div class="content-wrap">
      <el-collapse v-model="activeNames" @change="handleChange">
        <el-form ref="rulesSizeForm" :model="sizeForm.ciPriceProgram" label-width="97px" size="small" :rules="rules"
          class="demo-ruleForm">
          ...
          <el-collapse-item title="触发条件" name="2">
            <div class="basic-info">
              <el-form-item label="适用影院：">
                <el-button plain @click="cinemaSelectFun()">请选择影院</el-button><span
                  style="color:#f56c6c;font-size:14px;margin-left:8px" v-for="(item,index) in sizeForm.ciCinemaList"
                  :key="index">{{item.name}}</span>
              </el-form-item>
              <el-form-item label="适用影厅：" prop="ciPriceHallType">
                <el-select v-model="sizeForm.ciPriceProgram.ciPriceHallType" placeholder="请选择影厅"
                  @change='ciPriceHallTypeChange' multiple>
                  <el-option label="不限影厅" value="limit"
                    :disabled="sizeForm.ciPriceProgram.ciPriceHallType[0] != 'limit' && sizeForm.ciPriceProgram.ciPriceHallType.length>0">
                  </el-option>
                  <el-option :disabled="sizeForm.ciPriceProgram.ciPriceHallType[0] == 'limit'"
                    v-for="item in HallTypeList" :key="item.value" :label="item.keyName" :value="item.keyCode">
                  </el-option>

                </el-select>
              </el-form-item>
              <el-form-item label="有效日期：" prop="tiemStart_end">
                <el-date-picker v-model="sizeForm.ciPriceProgram.tiemStart_end" value-format="yyyy-MM-dd"
                  type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
              </el-form-item>
              <el-form-item label="适用时段：" prop="weekDay_1">
                <span class="xinhao"></span>
                <div class="time-interval" v-for='(item, index) in sizeForm.tmList' :key="index">

                  <div class="time-interval-1">
                    <el-checkbox :indeterminate="item.isIndeterminate_week" v-model="item.weekDay_checkAll"
                      @change="handleCheckWeekAllChange($event,index)">工作日</el-checkbox>
                    <div class="time-interval-week">
                      <el-checkbox v-model="item.monday" @change="handleCheckedWeekChange($event,index)" true-label="1"
                        false-label="0">周一</el-checkbox>
                      <el-checkbox v-model="item.tuesday" @change="handleCheckedWeekChange($event,index)" true-label="1"
                        false-label="0">周二</el-checkbox>
                      <el-checkbox v-model="item.wednesday" @change="handleCheckedWeekChange($event,index)"
                        true-label="1" false-label="0">周三</el-checkbox>
                      <el-checkbox v-model="item.thursday" @change="handleCheckedWeekChange($event,index)"
                        true-label="1" false-label="0">周四</el-checkbox>
                      <el-checkbox v-model="item.friday" @change="handleCheckedWeekChange($event,index)" true-label="1"
                        false-label="0">周五</el-checkbox>
                    </div>

                  </div>
                  <div class="time-interval-2">
                    <el-checkbox :indeterminate="item.isIndeterminate_weekend" v-model="item.weekendDay_checkAll"
                      @change="handleCheckWeekendAllChange($event,index)">周末</el-checkbox>
                    <div class="time-interval-weekend">
                      <el-checkbox v-model="item.saturday" @change="handleCheckedWeekendChange($event,index)"
                        true-label="1" false-label="0">周六</el-checkbox>
                      <el-checkbox v-model="item.sunday" @change="handleCheckedWeekendChange($event,index)"
                        true-label="1" false-label="0">周日</el-checkbox>
                    </div>
                  ...
                  <div class="time-interval-3" ref='timeDom'>
                    <div class="time-wrap">
                      <el-time-picker v-model="item.startTm" placeholder="请输入时间" @change="findTime($event,index)">
                      </el-time-picker>
              ...
              <!-- <el-form-item label="适用影片：" prop="movieCodeList">
                <el-select v-model="sizeForm.ciPriceProgram.movieCodeList" placeholder="请选择影片" @change='movieListChange'
                  ...
                  <el-option :disabled="sizeForm.ciPriceProgram.movieCodeList[0] == 'limit'"
                    v-for="(item,key) in movieList" :key="key" :label="item.movieName" :value="item.movieCode">
                  </el-option>
             ...
             <el-form-item label="适用影片：">  <!-- 黎的版本  -->
              <el-radio-group v-model="useMovie" @change="changeCinema">
                <el-radio :label="1">不限影片</el-radio>
                <el-radio :label="2">指定影片</el-radio>
              </el-radio-group>
              <el-input v-model="chooseCinemas" v-show="isSome" @focus="chooseCinema" clearable class="cinema">
              </el-input>
            </el-form-item>
            <getCinema ref="chooseCinema"  @searchUser="searchUser" @chooseUser="chooseUser" @changeCurrentPage="changeCurrentPage"></getCinema>
            </div>
          </el-collapse-item>
            ...
            <div class="basic-info price-system" style="margin-bottom:30px">
              <el-tabs v-model="activeTab" type="card" @tab-click="handleClick">
                <el-tab-pane label="柜台/自助" name="first">
                  ...
                  <el-form-item label="适用渠道：" style="margin-bottom: 0;" prop="price_Channel_1">
                    <el-checkbox-group v-model="price_Channel_1" @change='handleCheckedChannel_change_1'>
                      <el-checkbox v-for="chanel in channelsList_1" :key="chanel.uid" :label="chanel.uid">
                        {{chanel.name}}</el-checkbox>
                    </el-checkbox-group>
                  </el-form-item>
                  <el-form-item label="适用版本：" style="margin-bottom: 0;">
                    <el-checkbox class="check-all-type" v-model="disVersion_checkAll_1"
                      :indeterminate="disVersion_isIndeterminate_1" @change="disVersion_handleCheckAllChange_1">全部版本
                    </el-checkbox>
                    <el-checkbox-group v-model="price_disVersion_1" @change='handleCheckedDisVersion_Change'>
                      <el-checkbox v-for="(item, index) in disVersionList" :key="index"
                        :label="item.keyCode + ',' + item.keyName">{{item.keyName}}</el-checkbox>
                    </el-checkbox-group>
                  </el-form-item>
                  <el-form-item label="适用票类：">
                    <el-checkbox-group v-model="price_tickettype" @change='handleCheckedDisTickettype_Change'>
                      <template v-for="(item, i) in tickettypeList">
                        <el-checkbox :disabled="item.name == '成人票' " :key="i" :label="item.uid+','+item.name">
                          {{item.name}}
                        </el-checkbox>
                      </template>
                    ...
                    <div>
                      ...
                      <el-table ref="multipleTable1" :data="ttVoList" @selection-change="handleSelectionTtVoListChange">
                        <el-table-column type="selection" width="60">
                        </el-table-column>
                        <el-table-column prop="ttName" label="票类" width="100">
                        </el-table-column>
                        <el-table-column prop="movieVersionName" label="影片版本" width="100">
                        </el-table-column>
                        <el-table-column prop="price" label="价格">
                          <template slot-scope="scope">
                            <el-select v-model="scope.row.feeFlag">
                              <el-option v-for="(item, index) in scope.row.sel" :key="index" :value="item.value"
                                :label="item.label">
                              </el-option>
                            </el-select>
                            <span class="input-con">
                              <el-input type="number" maxlength="9" v-model="scope.row.price"
                                @change="priceChange1(scope.row.price,scope.$index)"></el-input><i
                                class="rmb-hover">¥</i>
                            </span>
                          </template>
                        </el-table-column>
                ...
                <el-tab-pane label="线上网售" name="second">
                  ...
                  <el-form-item label="适用渠道">
                    <el-checkbox-group v-model="price_Channel_2" @change="handleCheckedChannel_change_2">
                      <el-checkbox v-for="(channel, i) in channelsList_2" :key="i" :label="channel.uid" name="type">
                        {{channel.name}}</el-checkbox>
                    </el-checkbox-group>
                  </el-form-item>
                  <el-form-item label="适用版本：" style="margin-bottom: 0;">
                    <el-checkbox class="check-all-type" v-model="disVersion_checkAll_2"
                      :indeterminate="disVersion_isIndeterminate_2" @change="disVersion_handleCheckAllChange_2">全部版本
                    </el-checkbox>
                    <el-checkbox-group v-model="price_disVersion_2" @change="handleCheckedDisVersion_Change_2">
                      <el-checkbox v-for="(item, index) in disVersionList" :key="index"
                        :label="item.keyCode + ',' + item.keyName">{{item.keyName}}</el-checkbox>
                    </el-checkbox-group>
                  </el-form-item>
                  <div>
                    ...
                    <el-table class="edit-cichannel-table" :data="priceNetSale" ref='multipleTable2'
                      ...
                      <el-table-column prop="price" label="渠道结算票价">
                        <template slot-scope="scope">
                          ...
                          <span class="input-con">
                            <el-input type="number" maxlength="9" v-model="scope.row.price"
                              @change="priceChange2(scope.row.price,scope.$index)"></el-input><i class="rmb-hover">¥</i>
                          </span>
                        </template>
                      </el-table-column>
                    </el-table>
              ...
              <div class="table-bottom-menu">
                <div class="prcie-promise">
                  <el-checkbox label="允许包厢单卖" name="type" v-model="sizeForm.ciPriceProgram.permitSaleBox"></el-checkbox>
                </div>
                <div class="prcie-promise">
                  <el-checkbox label="允许营销活动折扣、会员折扣" name="type" v-model="sizeForm.ciPriceProgram.permitDiscount">
                  </el-checkbox>
                </div>
    ...
    <div class="price-edit-button-wrap">
      <div class="btn-area">
        <el-button type="primary" @click="priceprogramUpdate('rulesSizeForm')">保存</el-button>
        <el-button @click="cancelFun">取消</el-button>
      </div>

    </div>
    <fixStepTool :stepData="stepData.stepList"></fixStepTool>
    <cinemaSelect ref="checkNode" :selectCinemaStatus='selectCinemaStatus' :getCinemaTreeList='getCinemaTreeList'
      @getSelectedCinema='getSelectedCinema' @selectCinemaClose="selectCinemaClose"
      :ciCinemaList='sizeForm.ciCinemaList'></cinemaSelect>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      chooseUser(arr) {
        ...
        this.chooseCinemas = this.chooseCinemas.slice(0,this.chooseCinemas.length-1)
      },
      chooseCinema(){
        ...
        if(!this.cinemaUids ){
          ...
          this.$refs.chooseCinema.title = "选择影片"  //  关联代码 getMovieList
        }
      },
        ...
        self.$ctmList
          ...
          .then(ret => {
            if (ret.data) {
              ...
              self.$nextTick(_ => {
                self.$refs.checkNode.initCheckedTreeNode(self.sizeForm.ciCinemaList);
              });
            }
          })
          .catch(() => {
      ...
      deleFun(item, index) {
        ...
        if (this.sizeForm.tmList.length == 1) {
          this.$message({
            message: "必须保留一个时间段",
            type: "warning"
          });
      ...
      ttVoList_Fun() {
        ...
        self.ttVoList.forEach(item => {
          obj[`${item.movieVersionName}-${item.ttName}`] = JSON.parse(JSON.stringify(item))
        })
        ...
        self.price_disVersion_1.forEach((item, i) => {
          ...
          if (self.price_tickettype.length > 0) {
            self.price_tickettype.forEach((item2, i2) => {
              ...
              let obj1 = obj[`${item.split(",")[1]}-${item2.split(",")[1]}`]

              if (obj1) {
                resultItem.price = obj1.price
                resultItem.feeFlag = obj1.feeFlag
      ...
      priceNetSale_Fun() {
        ...
        self.priceNetSale.forEach(item => {
          obj[`${item.movieVersionName}`] = JSON.parse(JSON.stringify(item))
        })
      ...
      getMovieList(current=1,movieName="") {
      ...
      ciPriceHallTypeChange() {
        ...
        if (self.sizeForm.ciPriceProgram.ciPriceHallType.length == 1 && self.sizeForm.ciPriceProgram.ciPriceHallType[
          0] == 'limit') {
      ...
      priceprogramUpdate(rulesSizeForm) {
        ...
        this.$refs[rulesSizeForm].validate(valid => {
          if (valid) {
            ...
            // ciPriceProgram.useMovie = self.sizeForm.ciPriceProgram.useMovie;
            ciPriceProgram.id = self.sizeForm.ciPriceProgram.id;
            ciPriceProgram.uid = self.sizeForm.ciPriceProgram.uid;
            ...
            console.log(
              "self.ciPriceSaleChannelVoList法国红酒黑寡妇",
              ciPriceSaleChannelVoList
            );
            ...
            //判断适用时段是否选择
            if (!self.sizeForm.tmList.every((item, index) => {
            ...
            //判断价格不能为0
            if (self.formPriceNetSale.some((item, index) => {
                return (item.price == 0)
              })) {
              self.$message({
                message: '价格不能为0',
            ...
            if (self.formTtVoList.some((item, index) => {
                return (item.price == 0)
              })) {
              self.$message({
                message: '价格不能为0',
        ...
        self.$ctmList
          ...
          .then(ret => {
            ...
            if (ret.data) {
              ...
              // 处理影票
              console.log(result)
              this.useMovie = (result.ciPriceProgram && result.ciPriceProgram.useMovie) || "" 
              if (this.useMovie == "2") this.isSome = true
              let arr = result.schBashMovieList?result.schBashMovieList:[]
             if(arr.length>0){
                arr.forEach(item => {
                this.chooseCinemas += item.movieName + ",";
              });
              ...
              self.getMovieList()
              self.sizeForm.ciPriceProgram.tiemStart_end = result.ciPriceProgram.startDy ? [result.ciPriceProgram
                .startDy, result.ciPriceProgram.endDy
              ] : []
              //时间段处理
              ...
              if (arr.length > 0) {
                ...
                self.sizeForm.ciPriceProgram.movieCodeList = ['limit']
                self.sizeForm.schBashMovieList = []
              }
              //处理影厅数据
              if (result.ciPriceHallType.length > 0) {
                ...
                self.sizeForm.schBashHallTypeList = []
                self.sizeForm.ciPriceProgram.ciPriceHallType = ['limit']
              }
              ...
              if (result.ciPriceSaleChannelVoList.length > 0) {
                result.ciPriceSaleChannelVoList.forEach((item, index) => {
                  if (item.feeFlag == 0) {
                    ...
                    console.log(' self.走走组左走走走走走1 ', self.price_Channel_1)
                    self.ciPriceSaleChannelVoList_1.push(item)
                  } else if (item.feeFlag == 2) {
                    self.price_Channel_2.push(item.uid)
                    self.ciPriceSaleChannelVoList_2.push(item)
                    console.log(' self.走走组左走走走走走2 ', self.price_Channel_2)
                  }
                })
              }
              //处理影片版本和适用票类---线上
              let resultItempriceNetSale = []
              result.priceNetSale.forEach((item, index) => {
                // console.log('item99999999999999999999',item)
                resultItempriceNetSale.push(item.movieVersion + ',' + item.movieVersionName)

              })
              if (resultItempriceNetSale.length > 0) {
                self.disVersion_checkAll_2 = resultItempriceNetSale.length == self.disVersionList.length;
                self.disVersion_isIndeterminate_2 =
                  resultItempriceNetSale.length > 0 && resultItempriceNetSale.length < self.disVersionList.length;
              }
              self.price_disVersion_2 = resultItempriceNetSale
              // ********************************************************
              //处理影片版本和适用票类---柜台
              let resultTtVoList = []
              let resultTickettype = []
              result.ttVoList.forEach((item, index) => {
                resultTtVoList.push(item.movieVersion + ',' + item.movieVersionName)
                resultTickettype.push(item.ttUid + ',' + item.ttName)
              })
              if (resultTtVoList.length > 0) {
                self.disVersion_checkAll_1 = resultTtVoList.length == self.disVersionList.length;
                self.disVersion_isIndeterminate_1 =
                  resultTtVoList.length > 0 && resultTtVoList.length < self.disVersionList.length;
              }

              self.price_disVersion_1 = [...new Set(resultTtVoList)]
              self.price_tickettype = [...new Set(resultTickettype)]
              //*************************************************************
              //包厢或者折扣
              ...
              // self.$refs.multipleTable.toggleRowSelection(self.ttVoList);


              console.log('result.ciPriceProgram.status', self.sizeForm.ciPriceProgram.status)
            }
    ...
    watch:{
      useMovie(val){
        ...
        // console.log(this.sizeForm.schBashMovieList)
      }
    },
    components: {
