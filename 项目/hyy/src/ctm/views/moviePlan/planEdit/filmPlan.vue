                    ...
                    <ul class="flex-base">
                            ...
                            <div slot="content" class="tool-tips">
                                ...
                                <div v-if="filmItem.originFlag">
                                    ...
                                    <div class="plan-tip"><span class="mr30">人次：{{filmItem.privateSaleSeatCount ? filmItem.privateSaleSeatCount : 0}}人</span> <span class="mr30"> 票房：0元</span> <span class="mr30">上座率：{{filmItem.seatPrecent}}%</span></div>
                                    <div class="plan-tip"><span class="mr30">我的最低价格: {{filmItem.minPrice}}元</span><span class="mr30">竞对最低价格: {{filmItem.cheapestPrice}}元</span><span class="mr30">差价: {{Number(filmItem.minPrice) - Number(filmItem.cheapestPrice)}}元</span></div>
                                </div>
                            </div>
...
<script>
import {savePlanAll, deletePlan, subApproves, saleOpenStop, approvePassPlan} from 'ctm/http/interface'
export default {
    props: {
        pricePlan: {
            type: Object,
        ...
        approvePassPlan(type) {
            ...
            if (isHasSelect) {
                if (planUids.length != 0) {
                    ...
                    approvePassPlan(param).then(data => {
                        // 刷新页面
                        this.$emit('initMoivePlan', {
                            type: null
                        })
