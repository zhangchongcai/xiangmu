<template>
    <div :class="isFullScreen ? 'movie-plan full' : 'movie-plan'">
                    ...
                    <el-form-item>
                        <el-table
                            ...
                            <el-table-column
                                label="影厅类型">
                                <template slot-scope="scope">
                                    {{scope.row.useHallType ? '适用于所有影厅' : '适用于指定影厅'}}
                                </template>
                            </el-table-column>
                            <el-table-column
                                width="80"
                        ...
                        >
                         ...
                         <el-table-column>
                             <template slot-scope="scope">
                                ...
                                <span class="input-con" v-if="scope.row.selValue == 1">
                                    价格：<el-input type="number" maxlength="9" v-model="scope.row.price"></el-input>元+增值服务费: {{scope.row.addFee}} 元 = 最终售价: {{((Number(scope.row.price) * 100 + Number(scope.row.addFee) * 100) / 100).toFixed(2)}}元
                                </span>
                            </template>
                         </el-table-column>
                    </el-table>
        ...
        <frame-singlecinema
            ...
            <div slot="footerId" class="footer-button-con">
...
export default {
    ...
    data() {
        var turnoverCountValidate = (rule, value, callback) => {
           setTimeout(() => {
                if (value <= 0 || value > 20 || !/^\+?[1-9][0-9]*$/.test(value)) {
        ...
        return {
            ...
            intSettingForm: {
                timeSetValue:  [new Date(2019, 6, 8, 18, 30), new Date(2019, 6, 8, 21, 30)],
                timeSetValue1:  [new Date(2019, 6, 8, 18, 30), new Date(2019, 6, 8, 21, 30)],
                timeSetValue2:  [new Date(2019, 6, 8, 18, 30), new Date(2019, 6, 8, 21, 30)],
                timeSetValue3:  [new Date(2019, 6, 8, 18, 30), new Date(2019, 6, 8, 21, 30)],
                timeSetRadio: 0,
                turnoverCount: 1
            },
            rules: {
    ...
    mounted() {
        ...
        this.dateInit()
        if (!(this.$store.state.planCinema.cinemaUid && this.$store.state.planCinema.cinemaName)) {
            this.getUserInfo()
        } else {
    ...
    methods: {
            ...
            getRefCinema({
                ...
                if (result.code == 200 && result.data) {
                    if (result.data.length) {
                        this.$router.push({path: 'planReference', query: {cinemaUid: this.uidCinema, consultCinemaCode: this.$refs.moviePlan.consultCinemaCode, planDate: this.baseParam.planDate, goldTimeStart: this.$refs.moviePlan.goldTimeStart, goldTimeEnd: this.$refs.moviePlan.goldTimeEnd}})
                    } else {
        ...
        toPricePlanDetail(id) {
            this.$router.push({path: '/ticket//Scan', query: {id}})
        },
         // 表格单选事件
        selectedRow(val) {
        ...
        setBatchPirce() {
            let data = {
                ...
                planMovieList: this.curSelectPlans.map(plan => {
                    let item = {
                        ...
                        "priceProgramName":this.selectPlanData.isShow ? this.selectPlanData.programName : this.curPricePlan.priceProgramName,
                        "priceProgramUid":this.selectPlanData.isShow ? this.selectPlanData.priceProgramUid : this.curPricePlan.priceProgramUid,
            ...
            setBatchPirce(data).then(res => {
                ...
                if (res.code == 200 && res.data.length) {
                    let planData = JSON.parse(JSON.stringify(this.$refs.moviePlan.$refs.filmPlan.plan_rooms))
                    planData.forEach((plan, i) => {
                        plan.forEach(cPlan => {
                            res.data.some(item => {
                                if (item.uidHall == cPlan.hallUid && `${cPlan.startTime.hours}:${cPlan.startTime.minute}` == item.showTimeStart.split(' ')[1]) {
                                    ...
                                    cPlan.originFlag = {
                                        ...
                                        positionY: this.$refs.moviePlan.$refs.filmPlan.innerPoistionY(i)
                                    }
                                    cPlan.plan_text = '新建'
