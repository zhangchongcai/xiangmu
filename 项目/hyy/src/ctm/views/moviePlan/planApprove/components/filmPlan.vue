...
export default {
    ...
    mounted() {
        this.$eventHub.$on('selectRoomByHall', this.selectRoomByHall)
    ...
    watch: {
        ...
        plan_rooms: {
            handler(data, old_data) {
                ...
                this.$eventHub.$emit('getPlanRooms', data)
    ...
    methods: {
        ...
        filmMouseDown(e) {
            ...
            if(selectMovie.approveStatus !== 'WAIT_APPROVE') {
                return
            }
            ...
            this.changeHallStatus()

        },
        /* 改变影厅选中状态 */
        changeHallStatus() {
            let isIndeterminateData = []
            let checkedHallData = []
            let allCheckedCount = 0
            let allHalls = 0

            this.plan_rooms.forEach( (item, index) => {
                let data = item.filter( innerItem => {
                    return innerItem.approveStatus === 'WAIT_APPROVE'
                })
                allHalls += data.length
                let checkedCount = 0
                ...
                if(checkedCount === 0) {
                    // 全部未选中状态
                    isIndeterminateData.push(false)

                }else if(checkedCount > 0 && checkedCount < data.length) {
                    // 半选中状态
                    isIndeterminateData.push(true)

                }else if(checkedCount === data.length) {
                    // 全选状态
                    isIndeterminateData.push(false)
                    checkedHallData.push(index)

                }
            })
            let checkAll = false
            let isIndeterminate = false
            if(allCheckedCount < allHalls && allCheckedCount > 0) {
                checkAll = false
                isIndeterminate = true
            }else if(allCheckedCount === 0) {
                checkAll = false
                isIndeterminate = false
            }else if(allCheckedCount === allHalls) {
                checkAll = true
                isIndeterminate = false
            }
            this.$eventHub.$emit("handleCheckAll", checkAll, isIndeterminate)
            this.$emit("updateIsIndeterminate", isIndeterminateData)
            this.$eventHub.$emit('updateCheckedHall', checkedHallData)
        ...
        selectRoomByHall(currentHalls) {
            ...
            currentHalls.forEach( item => {
                plan_rooms[item].forEach( innerItem => {
        ...
        selectAll() {
            ...
            for (let i = 0; i < plan_rooms.length; i++) {
                for (let j = 0; j < plan_rooms[i].length; j++) {
                    ...
                    c_item.rowIndex = i
                    c_item.positionY = this.innerPostionY(this, i)
                    c_item.itemIndex = j
                    if (dropStatus) {
        ...
        submitSave(type, cb) {
            ...
            this.plan_rooms.forEach((row, rowIndex) => {
                ...
                row.filter(plan => plan.approveStatus !== 'WAIT_APPROVE' && plan.approveStatus !== 'APPROVED').map(plan =>{
                    return {
                        ...
                        "mustRightSeat": plan.mustRightSeat != undefined ? plan.mustRightSeat : 0,
                        "planDate": this.baseParam.planDate,
                        "planTimeStart": `${this.baseParam.planDate} ${plan.startTime.hours}:${plan.startTime.minute}`,
                        "planTimeEnd": `${this.baseParam.planDate} ${plan.endTime.hours}:${plan.endTime.minute}`,
            ...
            this.$ctmList.savePlanAll(_data).then(res => {
                if (res.code != 200) return this.success(res.msg)
                if (res.data.length) {
        ...
        deletSinglePlan(e) {
            ...
            if (movie.originFlag) {
                ...
                this.$ctmList.deletePlan(param).then(res => {
                    if (res.code == 200) {
                        plan_rooms[r_index].splice(i_index, 1);
        ...
        deletSelectPlan(e) {
            ...
            if (cantDelet) {
                this.warnning('当前所选场次存在可售或未完全退票，部分不可删除')
            }
            // 隐藏控制器
            this.hideControl()
        },
        ...
        goToPlanDetail(type) {
            ...
            this.$router.push({
                path: `/ctm/moviePlan/detail?mode=${type}&uid=${uid}`
            })
        },
        /* 控制器 - 查看影片*/
        viewMoive() {
        ...
        copyMoive() {
            ...
            let movie = JSON.parse(JSON.stringify(this.control.movie))
            let deleteArr = ['planUid', 'startTime', 'endTime', 'positionX', 'positionY', 'rowIndex', 'itemIndex', 'originFlag', 'planStatus', 'plan_text', 'saleStatus', 'sale_text', 'approveStatus', 'approve_text', 'hallCode', 'hallName', 'hallUid', 'saveError', 'sameTimeTip']
        ...
        subApproves() {
            ...
            this.$ctmList.subApproves(param).then(data => {
                if (data.code == 200) {
                    // 刷新当前排片数据
                    this.$emit('initMoivePlan', {
                        type: null
        ...
        stopOrStartSale(saleStatus, isMore) {
            ...
            if (param.planUids.length != 0) {
                this.$ctmList.saleOpenStop(param).then(data => {
        ...
        approvePassPlan(type) {
            ...
            if (isHasSelect) {
                if (planUids.length != 0) {
                    ...
                    this.$ctmList.approvePassPlan(param).then(data => {
                        // 刷新页面
                        this.$emit('initMoivePlan', {
                            type: null
                        })
