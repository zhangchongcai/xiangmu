export default {
    ...
    updated: function() {
        let _this = this
        if (document.querySelectorAll(this.stepData.stepEl).length) {
            window.setTimeout(() => {
    ...
    methods: {
        calcScrollTop(el, arr) {
            ...
            if ( !stepEles || !stepEles.length) return
            arr.forEach((item, i) => {
        ...
        changeToolActive(arr = this.stepData.stepList) {
            ...
            arr.forEach((step, i) => {
                if (i > 0) {
                    if (
                        scrolled <= step.scrollTop &&
                        scrolled > arr[i - 1].scrollTop
                    ) {
                        step.isactive = true
                    } else {
                        step.isactive = false
    ...
    destroyed() {
		this.$eventHub.$off("changeScroll")
	}
}