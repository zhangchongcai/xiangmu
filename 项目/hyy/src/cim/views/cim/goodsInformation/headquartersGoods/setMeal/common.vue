...
<!--    </div>-->
            ...
            <el-row>
              <div class="form-item-box left">
                <el-form-item
                        prop="price"
                        label="售价"
                        :rules="[{ required: routeQuery.type==3 ? false : true, message: '售价不能为空',trigger: 'change' }]"
                >
                  <span v-if="routeQuery.type==3">{{queryData.price}}</span>
                  <el-input v-else class="price-inp" v-model="queryData.price"></el-input> 元
                </el-form-item>
              </div>
              <div class="form-item-box left">
    ...
    <cinemal-dialog ref="myCinemalDialog" @onSumit="onCinemalSumit" :multiple="multipleCinemal"
                    :check="routeQuery.type==3 && queryData.saleCinema!=1" :dialogFeedbackData="queryData.cinemas"
                    :title="cinemalDialogTittle"></cinemal-dialog>
    <!-- 选择渠道弹窗 -->
    <channel-dialog ref="myChannelDialog" @onSumit="onChanneSumit"
                    :dialogFeedbackData="queryData.channels"
