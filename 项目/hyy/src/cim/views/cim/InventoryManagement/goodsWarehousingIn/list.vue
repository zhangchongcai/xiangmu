      ...
      >
        ...
        <el-form-item label="单据类型">
          <el-select v-model="queryData.billType" @change="billTypeEvent()">
            ...
            <el-option label="领用退回入库" value="4"></el-option>
            <el-option label="盘点赔偿入库" value="5"></el-option>
            <el-option label="调拨入库" value="6"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="单据状态">
          <el-select v-model="queryData.status">
            ...
            <el-option label="已执行入库" value="3"></el-option>
          </el-select>
        </el-form-item>
      ...
      <div class="common-new-built">
        <el-dropdown @command="newAddhandleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="1">领用退回入库</el-dropdown-item>
            <el-dropdown-item command="2">盘点赔偿入库</el-dropdown-item>
            <el-dropdown-item command="3">调拨入库</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
      <div>
  ...
  methods: {
    ...
    billTypeEvent(){
      if(this.queryData.billType == ""){
        this.queryData.billTypeList = ["4","5","6"]
      }else{
    ...
    seetable(row){
      console.log(row)
      this.resStoreInDetail(row,3)
    },
    // 修改操作
    edirtable(row){
      this.resStoreInDetail(row,2)
    },
    // 提交操作
    tjtable(row){
    ...
    resCheckBillToPage(row){
      this.$cimList.goodsWarehousingIn
        .storeInBillRegisterGetList(row)
        .then(res => {
          if (res.code === 200) {
