    ...
    <div class="bodyer">
      <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
        ...
        <el-collapse  v-model="activeNames" class="_edit-member-integral-content">
          ...
          <el-collapse-item title="购买影票" name="0" class="get-rules">
            <div class="pay-ticket-content">
              ...
              <el-form-item 
                ...
                prop="ticketChannelObj.checkedAccessChannels"
              >
                ...
                <el-checkbox-group v-model="formData.ticketChannelObj.checkedAccessChannels" @change="handleCheckedTicketChange($event)">
                  <el-checkbox v-for="channel in formData.ticketChannelObj.checkedAccessChannelsOptions" :label="channel" :key="channel" @change="handleCheckSingleTicket($event,channel)">{{channel}}</el-checkbox>
                </el-checkbox-group>
              </el-form-item>
              <!-- 内容 -->
              <div class="label" v-for="(item,key) in formData.ticket" :key="key" >
                <template v-if="formData.ticketChannelObj.checkedAccessChannels.indexOf(key)!=-1">
                  ...
                  <ul class="label-content">
                    <template  v-for="(childItem,childIndex) in item">
                      <li class="label-content-item" v-if="!childItem.isDelete" :key="childItem.id">
                        ...
                        <el-form-item class="form-inline" :rules="rules.memberLevelUpper" :prop="'ticket.'+key+'.'+childIndex">
                          <el-input   style="margin-right:24px;" v-model="childItem.memberLevelUpper"  clearable></el-input>
                        </el-form-item>
                    ...
                    <div class="label-add"><el-button type="text" @click="addItem('ticket',key)">添加</el-button></div>
                  </ul>
                </template>
              </div>
            </div>
          ...
          <el-collapse-item title="购买卖品" name="1" class="get-rules">
              ...
              <el-form-item 
                ...
                :rules="{ required: true, validator: checkChannelListSelected, trigger: 'change'}" 
                class="channel-selection"
                prop="goodsChannelObj.checkedAccessChannels"
              >
                ...
                <template v-if="formData.goodsChannelObj.checkedAccessChannels.indexOf(key)!=-1">
                  ...
                  <ul class="label-content">
                   <template  v-for="(childItem,childIndex) in item">
                    ...
                    <div class="label-add"><el-button type="text" @click="addItem('goods',key)">添加</el-button></div>
                  </ul>
                </template>
              </div>
            </div>
        ...
        <div class="footer-btn-wrap">
          <el-button type="primary" @click="submitForm('formData')" class="_el-btn-custom _member-add-edit-save-btn">保存</el-button>
...
export default {
  ...
  methods:{
    ...
    handleCheckedTicketChange(value) {
      ...
      console.log("checkedAccessChannelsOptions", this.formData.ticketChannelObj.checkedAccessChannelsOptions.length)
      this.formData.ticketChannelObj.checkAll = checkedCount === this.formData.ticketChannelObj.checkedAccessChannelsOptions.length;
      this.formData.ticketChannelObj.isIndeterminate = checkedCount > 0 && checkedCount < this.formData.ticketChannelObj.checkedAccessChannelsOptions.length;
      console.log("checkAll", this.formData.ticketChannelObj.checkAll)
      console.log(this.formData.ticketChannelObj.checkedAccessChannels)
      console.log(this.formData.ticketChannelObj.checkedAccessChannelsOptions)
    },
    // 卖品-监听变化
    handleCheckedGoodsChange(value) {
      let checkedCount = value.length;
      console.log("checkedCount",checkedCount)
      console.log("checkedAccessChannelsOptions", this.formData.goodsChannelObj.checkedAccessChannelsOptions.length)
      this.formData.goodsChannelObj.checkAll = checkedCount === this.formData.goodsChannelObj.checkedAccessChannelsOptions.length;
      this.formData.goodsChannelObj.isIndeterminate = checkedCount > 0 && checkedCount < this.formData.goodsChannelObj.checkedAccessChannelsOptions.length;
      console.log("checkAll", this.formData.goodsChannelObj.checkAll)
      console.log(this.formData.goodsChannelObj.checkedAccessChannels)
      console.log(this.formData.goodsChannelObj.checkedAccessChannelsOptions)
    },
    //影票-单选按钮
    handleCheckSingleTicket(val,channel){
      console.log("handleCheckSingleTicket",val)
    ...
    handleCheckSingleGoods(val,channel){
      console.log("handleCheckSingleGoods",val)
      console.log("channel",channel)
      if(!val){
        //子模块全部删除
    ...
    handleCheckAllTicket(val) {
      console.log("全选",val)
      this.formData.ticketChannelObj.checkedAccessChannels = val ? JSON.parse(JSON.stringify(this.formData.ticketChannelObj.checkedAccessChannelsOptions)) : [];
    ...
    handleCheckAllGoods(val) {
      console.log("全选",val)
      this.formData.goodsChannelObj.checkedAccessChannels = val ? JSON.parse(JSON.stringify(this.formData.goodsChannelObj.checkedAccessChannelsOptions)) : [];
    ...
    async channelList() {
      ...
      console.log(params)
      await this.$crmList.channelList(params)
      .then(res => {
        if(res){
          res.forEach(element => {
            this.formData.ticketChannelObj.checkedAccessChannelsOptions.push(element.desc)
            this.formData.goodsChannelObj.checkedAccessChannelsOptions.push(element.desc)
          });
          this.formData.allChannelObj=JSON.parse(JSON.stringify(res))
          console.log("this.formData.allChannelObj",this.formData.allChannelObj)
        }
      })
      .catch(err => {
        console.log(err);
    ...
    resetFormDataRenderData(data){
      ...
      if(this.formData.goodsChannelObj.checkedAccessChannels.length == this.formData.allChannelObj.length){
        this.formData.goodsChannelObj.checkAll=true
      }
      if(this.formData.ticketChannelObj.checkedAccessChannels.length == this.formData.allChannelObj.length){
        this.formData.ticketChannelObj.checkAll=true
      }
      console.log("this.formData.allChannelObj.length:",this.formData.allChannelObj.length)
      console.log("this.formData.ticketChannelObj.checkedAccessChannels.length:",this.formData.ticketChannelObj.checkedAccessChannels.length)
      console.log("this.formData.goodsChannelObj.checkedAccessChannels.length:",this.formData.goodsChannelObj.checkedAccessChannels.length)
      console.log("goodsChannelObj.checkAll:",this.formData.goodsChannelObj.checkAll)
      console.log("ticketChannelObj.checkAll:",this.formData.ticketChannelObj.checkAll)
      
      console.log("this.formData",this.formData)
      console.log("this.ticketChannelObj.checkedAccessChannels", this.formData.ticketChannelObj.checkedAccessChannels)
      console.log("this.goodsChannelObj.checkedAccessChannels", this.formData.goodsChannelObj.checkedAccessChannels)
    ...
    deleteRegion(target,key){
      ...
      console.log("key",key)
      // this.formData[target][key]=false
      console.log(this.formData)
      this.formData[`${target}ChannelObj`].checkedAccessChannels.forEach((item,index) => {
        if(key==item){
          this.formData[`${target}ChannelObj`].checkedAccessChannels.splice(index,1)
        }
      });
      //全部模块清除时取消全选
      if(this.formData[`${target}ChannelObj`].checkedAccessChannels.length==0){
    ...
    deleteItem(target,key,childIndex){
      ...
      console.log("isDelete",this.formData[target][key][childIndex])
    },
    //添加每条
    addItem(target,key){
      console.log("key",key)
      ...
      console.log("ChannelObj",this.formData[`${target}ChannelObj`].checkedAccessChannelsOptions)
      this.formData.allChannelObj.forEach(item=>{
        console.log(item.desc+"----------"+key)
        if(item.desc==key){
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          console.log("commitParams",this.commitParams)
          //请求提交
    ...
    commitIntegralRulesList(params){
      console.log(params)
      this.$crmList.commitIntegralRulesList(params)
      .then(res => {
        this.$message({
          message: '保存成功',
          type: 'success',
