  ...
  <div class="member-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      ...
      <el-form-item label="会员姓名：" prop="userName">
        <el-input v-model="formData.userName" @blur="()=>{formData.userName = formData.userName.trim()}"
    ...
    <div class="member-list-table _m-member-table-custom">
      <el-table :data="tableDataList" stripe style="width: 100%;" :empty-text="tipMessage">
        ...
        <el-table-column fixed="right" label="操作" min-width="120">
          <template slot-scope="scope">
            <span @click="handleChangeStatus(scope.row.cardNo,'frozen')" class="change-status-btn"
              v-if="scope.row.status=='normal'">冻结</span>
            <span @click="handleChangeStatus(scope.row.cardNo,'normal')" class="change-status-btn"
              v-if="scope.row.status=='frozen'">解冻</span>
    ...
    <!-- 分页 end -->
    <!-- 冻结、解冻的dialog -->
    <el-dialog title="" :visible.sync="frozenDialog" width="320px" :show-close="false"
