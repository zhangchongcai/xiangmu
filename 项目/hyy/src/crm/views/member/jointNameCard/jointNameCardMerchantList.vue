<template>
  <div class="jointNameCard-merchant-list">
    <el-form :inline="true" :model="searchData" ref="searchData" class="search-data-wrap _member-search-area-custom">
      ...
      <el-table :data="tableDataList" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column fixed="right" label="操作" min-width="140">
          <template slot-scope="scope">
            ...
            <el-button @click="handleChangeStatus(0,scope.row)" type="text" size="small" class="operation-button"
              v-if="scope.row.status== 1?true:false">停售</el-button>
            <el-button @click="handleChangeStatus(1,scope.row)" type="text" size="small" class="operation-button"
              v-if="scope.row.status== 0?true:false">开售</el-button>
            <el-button @click="handleEmitCard(scope.row)" type="text" size="small" class="operation-button"
              v-if="scope.row.status== 1?true:false">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="page-wrap">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
        :current-page="searchData.current-0" :page-size="searchData.size-0"
        layout="total, sizes, prev, pager, next, jumper" :page-sizes="[20 , 50 , 100]" :total="total-0"></el-pagination>
    </div>
    <!-- 分页 end -->
    <!-- 开售、停售的dialog -->
