  ...
  <div class="_member-tags-list">
    ...
    <div class="member-tags-table _m-member-table-custom">
      <el-table :data="tableData" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column prop="posVisible" label="是否在POS上显示" min-width="120">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.posVisible" :active-value="1" :inactive-value="0"
              @change="handleIsShowPos($event,scope.row)">
            </el-switch>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="时间间隔设置" class="__time-interval-dialog" :visible.sync="timeIntervalDialog">
      <el-form :model="timeIntervalData" ref="timeIntervalData" class="_time-interval-dialog-form-data-wrap">
        <el-tooltip class="item" effect="dark" :visible-arrow="false" content="设置同一手机号接收多条短信之间的间隔时间"
        ...
        <el-form-item class="btn-wrap">
          <el-button type="primary" size="medium" @click="handleSureTimeIntervsl('timeIntervalData')"
    ...
    <el-dialog title="发送短信" class="__send-msg-dialog" :visible.sync="sendMsgDialog" @close="handleSendMsgDialogClose">
      <el-form :model="sendMsgData" ref="sendMsgData" class="_send-msg-dialog-form-data-wrap">
        <el-form-item label="短信内容：" class="_send-msg-lable" prop="content"
          ...
          <el-input type="textarea" :class="isPreview?'_send-msg-textarea':''" resize="none" :rows="7" placeholder="请输入"
            v-model="sendMsgData.content" :readonly="isPreview?true:false">
          </el-input>
        </el-form-item>
        <el-form-item class="btn-wrap">
          <template v-if="!isPreview">
            <el-button type="primary" size="medium" @click="handlePreviewMsg('sendMsgData')"
              class="_el-btn-custom _member-add-edit-save-btn">预览</el-button>
            <el-button size="medium" @click="handleCanclePreviewMsg" class="_el-btn-custom">取消</el-button>
          </template>
          <template v-if="isPreview">
            <el-button type="primary" size="medium" @click="handleSureSendMsg"
              class="_el-btn-custom _member-add-edit-save-btn">确认发送</el-button>
    ...
    <!-- 删除标签的dialog -->
    <el-dialog title="" :visible.sync="deleteLabelDialog" width="320px" :show-close="false"
