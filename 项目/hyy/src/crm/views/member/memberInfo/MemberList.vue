  ...
  <div class="member-list">
    <el-form :inline="true" :model="formData" ref="formData" class="form-data-wrap _member-search-area-custom">
      ...
      <el-form-item label="会员ID/手机号：" prop="memberIdOrPhone">
        <el-input v-model="formData.memberIdOrPhone"
          @blur="()=>{formData.memberIdOrPhone = formData.memberIdOrPhone.trim()}" placeholder="会员ID/手机号" clearable>
        </el-input>
      </el-form-item>
      <el-form-item label="会员等级：" prop="levelId">
        <el-select v-model="formData.levelId" placeholder="全部等级" clearable>
          <el-option v-for="item in memberLevelRuleVOList" :key="item.levelNo" :label="item.levelName"
      ...
      <el-table :data="records" stripe style="width: 100%" :empty-text="tipMessage">
        ...
        <el-table-column fixed="right" label="操作" min-width="120">
          <template slot-scope="scope">
            ...
            <span @click="handleChangeHistory(scope.row)"
              style="color:#3B74FF;cursor: pointer;fontSize:12px;margin-left:10px;">变更历史</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="page-wrap">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
        :current-page="currentPage-0" :page-size="formData.size - 0" layout="total, sizes, prev, pager, next, jumper"
