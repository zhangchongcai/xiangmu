<template>
<section class="flex-base">
    <div class="scale-window" v-bind:style="{width: width +'px'}">
        <ul class="content flex-base" v-bind:style="{left:'-'+scrollX +'px'}">
            <li class="time-item flex-base" v-for="(time_item,time_item_index) in timeLine" v-bind:key="time_item_index">
    ...
    <div class="chaneg-scale flex-base">
...
export default {
    ...
    methods: {
        ...
        changeScale() {
            ...
            let precisionTips = '拖动精度栏请输入一个介于1和60之间的值';
            let timeIntervalFlage = false;
            let timeIntervalTips = '场次间隔栏请输入一个介于1和99之间的值';

            if (1 <= dragPrecision && dragPrecision <= 60) {
                precisionFlage = true;
            }
            if (1 <= timeInterval && timeInterval <= 99) {
